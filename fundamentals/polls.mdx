...
- **Create a poll**: Submit a question and options to start a poll.
- **Vote**: Let users cast votes on the poll options.
- **Fetch results**: Retrieve current voting results for a poll.

## 1. Creating a Poll

In order to create a poll, you need to submit the following details:

- **Question**: The poll question users will vote on.
- **Options**: An array of options to choose from.
- **Receiver**: UID or GUID of the target user or group.
- **Receiver type**: Specify `user` or `group`.
