---
title: "Pin Message"
description: "Pin, unpin, and fetch pinned CometChat messages in one-on-one or group conversations using the Pin Message extension."
---
...
CometChat.callExtension('pin-message', 'GET', URL).then(response => {
    // {pinnedMessages: []}
})
.catch(error => {
    // Error occured
