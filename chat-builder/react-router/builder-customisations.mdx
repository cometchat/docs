...
description: "Customize React Router UI Kit Builder components by adjusting CometChat app props, styling, group messages, and behavior."
---

Your settings file, `src/builder/cometchat-builder-settings.json`, handles feature toggles, layout and theming — see [UI Kit Builder Settings](/chat-builder/react-router/builder-settings). This page covers what to do when you need more than a setting.

---

## **App-Level Customizations**

[React UI Kit v7](/ui-kit/react/overview) has no `CometChatApp` component. The app shell is `src/App.tsx`, which is yours to edit, and it renders `CometChatHome` — where the app-level props live.
...
Which one applies depends on `layout.chatType` when the sidebar is hidden — see [Builder Settings](/chat-builder/react-router/builder-settings#3-layout-layout).
...
<Note>
  **Upgrading from v6?** `showGroupActionMessages` and `autoOpenFirstItem` were
  ...
  what `CometChatApp` did is settings plus your own `App.tsx`.
</Note>
