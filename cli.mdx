...
`npx @cometchat/skills-cli <command>` (as shown throughout this page), or
...
- **Several apps** — it lists them and asks you to pick one, or to enter `n` to
  ...
  `provision list` and then `provision use --app-id <id>` (it prints the Auth
...
| `add [--ide <agent>] [--no-mcp]` | Install the skills for your project's platform and an AI agent (default: Claude Code), and connect the CometChat Docs MCP (`--no-mcp` skips it) |
...
<CardGroup cols={2}>
  ...
  <Card title="Chat & Messaging" icon="comments" href="/chat-call">
