...
- A conversation can hold up to **100 pinned messages**. Pins made as the app (without `onBehalfOf`) and pins made on behalf of a user are counted separately, so each has its own limit of 100.
- A user can save up to **100 messages**. Saved messages are private to the user who saved them.
...
| [Unpin Message](/rest-api/messages/unpin-message)                   | `DELETE` | `/messages/{id}/pin`        | Remove a pinned message                          |
...
| [Mark as Interacted](/rest-api/messages/mark-message-as-interacted) | `PATCH`  | `/messages/{id}/interacted` | Mark elements of an interactive message as interacted |
