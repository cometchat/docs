...
If your users or developers are behind a corporate firewall, proxy, or VPN, allow the CometChat
domains so chat, calling, AI agents, moderation, and media work — all outbound over HTTPS/WSS (plus a
few ports for realtime and calling). See the full, current list of domains, ports, and a wildcard
shortcut in [Network Allowlist](/network-allowlist).
