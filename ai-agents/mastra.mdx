...
app.use(express.json());
    <Accordion title="Run & Deploy Your Mastra Agent">
      ...
      <ol>
        <li><code>npx mastra dev</code> (or your dev script) – starts local API, often on <code>http://localhost:4111/api</code>.</li>
        <li>Test quickly:
          <pre><code className="language-bash">curl -X POST http://localhost:4111/api/agents/AGENT_ID/generate -H "Content-Type: application/json" -d '{"messages":[{"role":"user","content":"ping"}]}'</code></pre>
        </li>
        <li>If you only exposed a custom Express route (e.g. <code>/mastra/agent</code>) ensure you deploy the same path and use that full URL in CometChat.</li>
      </ol>
      <h4>Expose Publicly for CometChat (Temporary)</h4>
      <p>Use a tunneling tool while iterating:</p>
      <pre><code className="language-bash"># example (choose one)
  ngrok http 4111
  cloudflared tunnel --url http://localhost:4111
  loca.lt --port 4111</code></pre>
      <p>Copy the forwarded HTTPS URL and append your agent route (e.g. <code>/api/agents/chef/generate</code>).</p>
      <h4>Production Deployment Patterns</h4>
      <ul>
        <li><b>Serverless (Vercel / Netlify):</b> Export a handler that instantiates (or reuses) the agent per invocation.</li>
        <li><b>Container (Fly.io / Render / ECS):</b> Run your Express (or Mastra CLI) process; enable health probes.</li>
        <li><b>Edge:</b> Keep tools pure & stateless or externalize state (DB / vector store) since edge runtimes are ephemeral.</li>
      </ul>
      <h4>Vercel API Route Example</h4>
      <pre><code className="language-ts">// api/agents/chef/generate.ts
  ...
  export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
      ...
      <ul>
        <li>Rate limit (e.g. IP + user) to prevent abuse.</li>
        <li>Add auth (bearer token or signed JWT) if endpoint is not meant to be public.</li>
        <li>Log tool invocations (id + latency) for observability.</li>
      </ul>
      <h4>Copy Values into CometChat</h4>
      <p>Use the final public HTTPS URL + path as <b>Deployment URL</b> and the internal agent key (e.g. <code>chef</code>) as <b>Mastra Agent ID</b>.</p>
