...
npx mastra dev
```
Quick ping (replace if you changed the id):
```bash
...
Use a tunnel to test the agent from CometChat before full deployment:
```bash
ngrok http 4111
cloudflared tunnel --url http://localhost:4111
loca.lt --port 4111
```
Append the route: `/api/agents/chef/generate` → copy resulting HTTPS URL into the CometChat Dashboard as the Deployment URL.
...
### CometChat Mapping
Use the final public HTTPS URL + `/api/agents/chef/generate` as the <b>Deployment URL</b> and the key <code>chef</code> as the <b>Mastra Agent ID</b> in the Dashboard.
