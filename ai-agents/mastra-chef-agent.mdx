...
<AccordionGroup>
  <Accordion title="Local run & ping test">
    <p><b>Local run:</b></p>
```bash
npx mastra dev
```
    <p><b>Ping test</b> (replace if you changed the id):</p>
```bash
...
```
  ...
  <Accordion title="Temporary public tunnel">
    <p>Use a tunnel to test the agent from CometChat before full deployment:</p>
```bash
ngrok http 4111
cloudflared tunnel --url http://localhost:4111
loca.lt --port 4111
```
    <p>Append the route <code>/api/agents/chef/generate</code> → copy resulting HTTPS URL into the Dashboard as the Deployment URL.</p>
  </Accordion>
  <Accordion title="Vercel serverless example">
    <Tabs>
      <Tab title="TypeScript">
        <p><code>api/agents/chef/generate.ts</code></p>
```ts
...
```
  ...
  <Accordion title="Production patterns">
    <ul>
      ...
      <li><b>Container:</b> Long‑lived process (Express/Fastify); add health checks & logging.</li>
      <li><b>Edge:</b> Keep tools stateless; push persistence (memory/db) to external services.</li>
    </ul>
  </Accordion>
  <Accordion title="Security essentials">
    <ul>
      <li>Rate‑limit requests (per IP + per user).</li>
      <li>Require auth (Bearer/JWT) for non-public use.</li>
      <li>Log tool calls (name, duration, success/error) for observability.</li>
    </ul>
  </Accordion>
  <Accordion title="CometChat mapping">
    Use the final public HTTPS URL + <code>/api/agents/chef/generate</code> as the <b>Deployment URL</b> and the key <code>chef</code> as the <b>Mastra Agent ID</b>.
  </Accordion>
</AccordionGroup>
...
## Step 7 — Configure in CometChat 

1. Open the [CometChat Dashboard](https://app.cometchat.com/).
2. Go to your App → AI Agents.
...
## Step 8 — Customize in the Chat Builder

1. From AI Agents in the CometChat dashboard
2. Select the "AI Agents" tab and Getting Started.
3. Select "Customize and Deploy"
4. Customize the agent’s appearance on Chat Builder
