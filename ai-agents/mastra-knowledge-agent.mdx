...
export const knowledgeAgent = new Agent({
  ...
  instructions: [
    ...
    'Always call the kb-retrieve tool first to gather context before answering.',
    'If the docs do not contain the answer respond exactly: "I\'t have that in my notes."',
    'Answer succinctly. Provide a Sources list if any docs were used.'
  ].join('\n'),
...
// (Optional) retain scaffolded weather agent if it still exists
// import { weatherAgent } from '../agents/weather-agent';

export const mastra = new Mastra({
  agents: {
    knowledge: knowledgeAgent,
    // weather: weatherAgent, // uncomment if keeping the scaffold example
  },
});
