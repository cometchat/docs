...
> Scaffold note: `npx create-mastra@latest` currently generates a sample **weather** agent at `src/mastra/agents/weather-agent.ts` plus a tool & workflow under `src/mastra/tools` and `src/mastra/workflows`. In this guide you'll **replace** those with a knowledge agent. You can delete the weather files or keep them while adding the new ones.
...
**`src/mastra/tools/docs-tool.ts`** (choose ONE of the `source` configurations; you can later switch to an array):
```ts
...
**`src/mastra/agents/knowledge-agent.ts`** (new file):
```ts
...
export const knowledgeAgent = new Agent({
  ...
  instructions: `You are a precise documentation assistant.\nAlways call the 'docs' tool first to fetch relevant passages before answering.\nRespond concisely (3-6 sentences).\nEnd with: Sources: <list headings/URLs>.\nIf the answer is not in the docs, say so clearly and do not speculate.`,
  model: openai('gpt-5'),
  tools: { docs: docsTool },
});
```
