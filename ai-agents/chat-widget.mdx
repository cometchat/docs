...
- A Chat Widget variant configured in Widget Builder.
...
## Step 1 - Publish & Embed

In the builder, open the **Widget** panel and click **Get Embed Code**. This publishes your variant and then shows an **App Credentials** panel — copy all four values:
...
You will also need your **Agent ID**, from **AI Agents** in the Dashboard.

<Note>
  `variantID` is optional. If you leave it out, the widget uses the first
  available variant for the app.
</Note>
...
By default the widget opens a fresh conversation with the agent every time it loads. Add `loadLastAgentConversation` to continue where the user left off instead:

```js
const COMETCHAT_WIDGET_CONFIG = {
  // …
  defaultChatID: "<YOUR_AGENT_ID>",
  loadLastAgentConversation: true,
};
```

<Note>
  This only has an effect for a user the widget can recognise on their next
  visit — `uid` or `authToken` mode. In guest mode the widget stores the
  generated UID in a cookie, so clearing cookies or switching browser starts a
  new conversation.
</Note>
