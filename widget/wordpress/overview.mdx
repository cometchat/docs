...
The Widget Builder controls theme, colors, and typography with no code — set those in its **Styles** tab. For anything it does not expose, open the **Custom Code** tab and target the widget's own `cometchat-*` component classes with your own **Custom CSS** (and Custom JS). See [Customize the widget with CSS](/widget/wordpress/integration#customize-the-widget-with-css) in the Integration guide.
