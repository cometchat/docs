...
The Widget Builder includes a **Custom Code** tab where you can add **Custom CSS** (and Custom JS) that ships with your widget.

Open it from the dashboard: **Chat & Messaging → Get Started / Integrate → No Code → WordPress → Widget Builder → Custom Code** tab.

<Note>
  Set your brand color, text colors, font and font size in the Builder's **Styles** tab, not here.
  Those are first-class controls, and the Builder applies them as inline styles that a stylesheet
  rule cannot cleanly override. Custom CSS is for the things the Builder does not expose.
</Note>

Custom CSS is automatically scoped under the widget's root, so top-level selectors like `:root` or
`body` never match — write ordinary component selectors instead. The scoping also adds specificity
for you, so these rules win without `!important`.

Example — square off the message bubbles and the search bar:
...
### What to know before you write Custom CSS

**Component classes are the stable target.** Every component carries a `cometchat-*` block class
with BEM elements under it — `.cometchat-message-bubble__body`, `.cometchat-conversations__item`,
`.cometchat-message-composer__send-button`. Inspect the widget to find the one you want. Prefer a
plain property (spacing, radius, borders, `display`) over redefining a `--cometchat-*` variable:
variables are resolved higher up the tree and some are composed into other variables before your
rule is reached, so a variable override can appear to apply while changing nothing.

**At-rules do not work here.** `@font-face`, `@media` and `@import` are dropped: every top-level
selector is scoped automatically, and that scoping makes an at-rule invalid, so the browser
discards it silently. Stick to declarations inside ordinary selectors.

**Dialogs and call screens sit outside `.cometchat-root`.** Confirmation dialogs, the call screen
and context menus are rendered at the widget's outer element so they can cover the chat. Their own
component classes still work — just do not nest your selector under `.cometchat-root`.
