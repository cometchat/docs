...
Add the CometChat Widget with a single snippet in Wix’s **Custom Code** settings. Pick the sign-in flow that matches your site: guests, auto-created users by ID, or backend-issued auth tokens.
...
Add it as **site-level custom code**, not as an Embed HTML element.

1) In your Wix dashboard, go to **Settings → Custom Code** and click **+ Add Custom Code**.
2) Paste the snippet from your chosen option below.
3) Set **Add Code to Pages** to **All pages**, and **Place Code in** to **Body – end**.
4) Give it a name, click **Apply**, then **Publish** your site.
5) Open the published site — custom code does not run in the editor preview — and confirm the widget appears.

<Warning>
  **Do not use Add → Embed Code → Embed HTML for this.** That element renders your code inside a
  separate Wix iframe, and browsers do not grant camera or microphone access to an embedded frame
  unless the page embedding it says so. Wix's does not, so voice and video calls fail there with a
  permission error even after the visitor has allowed access — the chat works, the call does not.

  Site-level custom code runs in the page itself, where both are permitted.
</Warning>
...
<script>
  ...
  const startWidget = () => {
    // Custom code injects no markup, so the widget's mount point has to be created here.
    if (!document.getElementById("cometChatMount")) {
      const mount = document.createElement("div");
      mount.id = "cometChatMount";
...
- **Widget not appearing?** Custom code only runs on the published site, never in the editor preview — publish, then reload. Check the browser console for the `chat-embed@1.x.x` script and for errors from it.
- **Calls fail with a permission error?** The snippet is almost certainly in an Embed HTML element rather than Custom Code. See the warning above.  
