...
<Note>
**Push notifications are not part of the card.** When a card is sent, the *message's* `data.text` carries the push/preview text — taken from the **Notification** tab's body in Card Builder, falling back to `fallbackText`. The card JSON itself has no notification field.
</Note>
