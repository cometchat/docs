...
You can create multiple channels of the same type. For example:
- "Promotions Feed" (`in_app`) — for marketing notifications
- "Alerts Feed" (`in_app`) — for transactional alerts

Templates reference a specific channel instance by its `channelId`.
...
| `push` channels per app | No hard limit |
