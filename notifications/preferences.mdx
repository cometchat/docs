---
title: "Preferences"
description: "Configure CometChat notification preferences for push, email, and SMS defaults, user overrides, muted conversations, and DND."
---
...
- **Notify for subscribed threads** *(new replies only)* — the user is notified only about replies in threads they are *subscribed* to. A user becomes subscribed to a thread by sending its parent message, replying in it, being @-mentioned in it, or explicitly following it — and can unfollow to stop. Useful in busy conversations where users only want updates on threads they are part of. In the SDK this maps to `RepliesOptions.SUBSCRIBE_TO_SUBSCRIBED_THREADS`.
- **Notify when my message is quoted** *(quoted replies only)* — the sender of the original message is notified whenever someone quote-replies to their message, independent of threads or mentions. In the JavaScript & React Native SDKs this maps to `QuotedRepliesOptions.SUBSCRIBE_TO_QUOTES_ON_OWN_MESSAGES`.
...
**Replies** use the `RepliesOptions` enum, accessible as `CometChatNotifications.RepliesOptions`. It supports four values:

- `DONT_SUBSCRIBE` — don't notify for replies.
- `SUBSCRIBE_TO_ALL` — notify for all replies.
- `SUBSCRIBE_TO_MENTIONS` — notify only for replies that mention the user.
- `SUBSCRIBE_TO_SUBSCRIBED_THREADS` — notify only for replies in threads the user has subscribed to.

`SUBSCRIBE_TO_SUBSCRIBED_THREADS` pairs with [Thread Subscription](/sdk/javascript/thread-subscription): a user is subscribed to a thread when they start it, reply in it, are @-mentioned in it, or follow it explicitly.

</Note>

<Note>

**Quoted replies** use the dedicated `QuotedRepliesOptions` enum (JavaScript & React Native SDK), accessible as `CometChatNotifications.QuotedRepliesOptions`. It supports four values:

- `DONT_SUBSCRIBE` — don't notify for quoted replies.
- `SUBSCRIBE_TO_ALL` — notify for all quoted replies.
- `SUBSCRIBE_TO_MENTIONS` — notify only for quoted replies that mention the user.
- `SUBSCRIBE_TO_QUOTES_ON_OWN_MESSAGES` — notify only when someone quotes the user's own messages.

Pass a `QuotedRepliesOptions` value to `setQuotedRepliesPreference()` (not `RepliesOptions`, which has no `SUBSCRIBE_TO_QUOTES_ON_OWN_MESSAGES` option).
...
groupPreferences.setQuotedRepliesPreference(QuotedRepliesOptions.DONT_SUBSCRIBE);
...
oneOnOnePreferences.setQuotedRepliesPreference(QuotedRepliesOptions.DONT_SUBSCRIBE);
