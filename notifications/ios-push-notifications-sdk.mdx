...
<Warning>
  **Known limitation in 1.0.0 and below — Swift Package Manager only.** The package manifest does
  not declare its dependencies, so adding this package on its own resolves the push binary and
  nothing else, and the build fails with `no such module 'CometChatSDK'`.

  Until this is fixed, add these two packages to your project as well:

  ```
  https://github.com/cometchat/chat-sdk-ios     (4.1.5 or later)
  https://github.com/cometchat/calls-sdk-ios    (5.0.0 or later)
  ```

  You will not hit this if your project already depends on both — which is why an app that
  already uses CometChat Chat builds fine. CocoaPods is unaffected: the pod declares both and
...
extension MyRootCoordinator: CometChatPushNotificationsDelegate {
    ...
    // and start the session (generate a token, then join). See "Start the call" below.
...
### Start the call in `presentCallScreen`

`presentCallScreen` hands you a Chat SDK `Call`, but `joinSession` takes Calls SDK session
settings. The two SDKs name the call type differently: the `Call`'s `callType` is a
`CometChat.CallType` (`.audio` / `.video`), while the session wants a `SessionType`
(`.voice` / `.video`). Map one to the other on every call. See
[Join Session](/calls/ios/join-session) for the container and the other session settings.

```swift
func presentCallScreen(for call: Call, sessionId: String) {
    // Present your call screen first. callViewContainer is a view on it,
    // laid out by frame (see Join Session).
    ...
    CometChatCalls.joinSession(
        ...
        onSuccess: { _ in },
        onError: { error in print("Join failed: \(error?.errorDescription ?? "")") }
    )
}
```

<Warning>
  Don't hardcode `.setSessionType(.video)` here. The join samples elsewhere use `.video`, but
  copied into this callback it answers every voice call with the camera on.
</Warning>
