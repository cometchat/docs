...
```json
{
  ...
  "parentId": "176001" // Optional - only sent for threaded notifications (thread reply, or reaction on a threaded message); the parent message's ID, for deep linking into the thread.
}
```
