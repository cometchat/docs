...
| Event | What happens |
| --- | --- |
| **VoIP push arrives** | The package reports it to CallKit immediately, in every app state, as iOS requires. App open with `ringInForeground: false`: it reports the call and ends it at once, and your in-app screen rings instead. |
...
The `aps.badge` field is set by CometChat server-side, so iOS updates the badge when the push is delivered.
