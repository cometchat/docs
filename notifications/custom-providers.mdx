...
description: "Route CometChat push notification payloads to a custom webhook provider with credentials, delivery rules, and payload handling."
---
...
```json
{
  ...
  "data": {
    ...
    "notificationDetails": {
      ...
      "parentId": "176001", // Optional - sent only for threaded notifications (a reply in a thread, or a reaction on a threaded message). The parent message's ID, so you can deep link straight into the thread instead of the conversation.
      "type": "chat",
...
```json
{
  ...
  "data": {
    ...
    "notificationDetails": {
      ...
      "parentId": "176001", // Optional - sent only when the reaction is on a threaded message; the parent message's ID, for deep linking into the thread.
      "type": "chat",
      "sentAt": "1741847453000",
      "unreadMessageCount": 5, // Optional - only present when "Include badge count in payload" is enabled
      "custom": {Custom JSON} // Custom JSON object is added in case it is configured in the preferences.
