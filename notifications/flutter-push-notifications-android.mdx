...
- CometChat dashboard setup (enable push, add an FCM provider) with screenshots.
- Firebase/FCM wiring (service account, `google-services.json`, Gradle).
...
- Flutter 3.3.0+ / Dart 3.10.8+, and **Kotlin 2.0 or newer** in your Android build (required by `notification_voip_plugin` 2.1.0+).
...
## 1. Enable push and add an FCM provider (CometChat Dashboard)
...
Keep the provider ID — you'll pass it to `registerToken`.

## 2. Prepare Firebase and credentials

1. In the Firebase Console, register your Android package name (the same as `applicationId` in `android/app/build.gradle`) and download `google-services.json` into `android/app/`.
...
Store your CometChat and provider values somewhere your app can read them (for example a small `AppCredentials` class):
...
## 3. Add the plugin and configure Gradle
...
`cometchat_sdk` and `notification_voip_plugin` are pulled in transitively. Run:
...
### 4.2 The `incomingCallMain` entrypoint
...
### 4.3 Route call navigation once the app is running
...
## 5. Request permission and register the FCM token
...
## 6. Notification taps and navigation
...
## 7. OEM permissions for lock-screen calls
...
### 8.1 Enable unread badge count on the dashboard
...
### 8.2 Update and clear the badge
