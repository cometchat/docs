...
- Prerequisite: the shared dashboard + Firebase setup in the [Getting Started](/notifications/push-getting-started) guide.
...
<Info>
**Complete the [Getting Started](/notifications/push-getting-started) guide first** — enable Push Notifications, add your FCM provider, and configure Firebase (`google-services.json` + service account JSON). This guide covers only the React Native app wiring.
</Info>
...
### 2.1 Configure Firebase with Android credentials
...
### 2.2 Configure required permissions in `AndroidManifest.xml` as shown.
...
### 2.4 Unregister FCM token on logout
...
## 3. VoIP call notifications

These steps are Android-only—copy/paste and fill your IDs.

### 3.1 Add CallKeep services to `android/app/src/main/AndroidManifest.xml`
...
### 3.2 Background handler for call pushes (`index.js`)
...
### 3.5 Wire `App.tsx` to init VoIP + consume pending accepts
...
### 3.7 Local notification helper (`LocalNotificationHandler.ts`)
...
### 5.4 Handle Badge Count in Foreground Messages
...
### 5.5 Display Local Notification with Badge Count
...
### 5.6 Clear Badge When App Becomes Active
...
### 5.8 Clear Badge on Fresh Install / No Logged-In User
...
### 5.9 Clear Badge in Login Listener (Safety Net)
