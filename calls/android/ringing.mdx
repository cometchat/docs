...
- **Placing calls:** if the caller backgrounds the app while an outgoing call is ringing, an accept or reject may not be delivered — the caller UI should reconcile the call state when the app returns to the foreground.
</Warning>
