...
- **Placing calls:** if the caller backgrounds the app while an outgoing call is ringing, the connection stays alive for about 30 seconds before disconnecting — an accept or reject that happens after that window is not delivered, and the caller UI should reconcile the call state when the app returns to the foreground.
</Warning>
