...
<Warning>
**Ringing events require a foreground app in the default socket mode**

Call signaling events (`onIncomingCallReceived`, `onOutgoingCallAccepted`, and the other `CallListener` callbacks) are delivered over the Chat SDK's WebSocket. In the default automatic socket mode, that connection is **disconnected when the app goes to the background** (see [Connection Behaviour](/sdk/android/connection-behaviour)), and events that fire while it is down are not redelivered on reconnect. In practice:

- **Receiving calls:** an app in the background will not receive `onIncomingCallReceived`. To ring reliably when the app is backgrounded or killed, integrate [push notifications](/notifications/android-push-notifications) for incoming calls.
- **Placing calls:** in the default automatic mode the socket disconnects immediately when the app goes to the background, so an accept or reject can be missed as soon as the caller backgrounds the app. In manual mode the connection stays alive for up to 30 seconds in the background (until the ping timeout — see [Connection Behaviour](/sdk/android/connection-behaviour)), leaving a brief window. Either way, the caller UI should reconcile the call state when the app returns to the foreground.
</Warning>
