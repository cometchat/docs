...
Hides the transcription start/stop item from the control panel's **More** menu. Set to `true` to remove it, for example when transcription is controlled programmatically.
...
Hides the closed-caption (Show/Hide Captions) item from the control panel's **More** menu. Even when set to `false`, the item only appears while transcription is running.
