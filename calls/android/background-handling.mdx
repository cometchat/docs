...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <application>
        <!-- REQUIRED. The SDK ships the service class but does NOT declare it for you.
             Without this entry CometChatOngoingCallService.launch() fails at runtime with
...
<Warning>
  The `<service>` element above is not optional. `CometChatOngoingCallService` lives in the SDK, but an
  Android app must declare every service it starts in its OWN manifest — the SDK's manifest merge does not
  add it. Skipping it produces a silent dead end: the call keeps running in the foreground, no notification
  appears, and the session dies as soon as the user leaves the app.
</Warning>
...
class CallActivity : AppCompatActivity() {
    ...
    private fun joinCall() {
        ...
        val container = findViewById<RelativeLayout>(R.id.callContainer)
...
public class CallActivity extends AppCompatActivity {
    ...
    private void joinCall() {
        ...
        RelativeLayout container = findViewById(R.id.callContainer);
...
- [Session Status Listener](/calls/android/events#session-events) - Listen for session events
