...
To fetch all call logs in your web application, you should use the `CallLogRequestBuilder`. This builder allows you to customize the call log fetching process according to your needs.
...
| `setCallType(callType: 'video' or 'audio')`                                                          | Sets the type of calls to fetch (video or audio).            |
| `setCallStatus(callStatus: 'ongoing' or 'busy' or 'rejected' or 'cancelled' or 'ended' or 'missed')` | Sets the status of calls to fetch (ongoing, busy, etc.).     |
...
| `setGuid(guid: string)`                                                                              | Sets the GUID of the group whose call logs to fetch.         |
