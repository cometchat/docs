...
sidebarTitle: "Events"
sdk_version: "5.x"
description: "Handle CometChat Calls SDK v5 events on JavaScript for call lifecycle, participants, media state, errors, and real-time updates."
---
...
Fired when the SDK fails to establish the connection to the session. The listener receives a `ConnectionError` payload.

| Property | Type | Description |
|----------|------|-------------|
| `name` | String | Name/type of the connection error |
...
Fired when the remote participant's connection is lost during a peer-to-peer call.

```javascript
CometChatCalls.addEventListener("onRemoteConnectionLost", () => {
  console.log("Remote participant's connection was lost");
});
```

### Remote Connection Restored

Fired when the remote participant's connection is restored during a peer-to-peer call.

```javascript
CometChatCalls.addEventListener("onRemoteConnectionRestored", () => {
  console.log("Remote participant's connection was restored");
});
```
