...
sdk_version: "5.x"
description: "Configure CometChat Calls SDK v5 ringing on React Native for incoming calls, outgoing calls, call alerts, and accept or reject flows."
---
...
<Info>
  **Quick Reference for AI Agents & Developers**
  ...
  // Chat SDK = signaling. Calls SDK = media.
  const call = new CometChat.Call(uid, CometChat.CALL_TYPE.VIDEO, CometChat.RECEIVER_TYPE.USER);
  await CometChat.initiateCall(call);                       // ring them

  CometChat.addCallListener('LISTENER_ID', new CometChat.CallListener({
    onIncomingCallReceived: (call) => { /* show your accept/reject UI */ },
  }));

  const accepted = await CometChat.acceptCall(sessionId);   // on accept
  const { token } = await CometChatCalls.generateToken(accepted.getSessionId());
  // → render <CometChatCalls.Component callToken={token} />
  ```
  ...
  * ⚠️ This rings the callee **only while their app is open** — waking a closed app needs [VoIP Calling](/docs/calls/react-native/voip-calling)
...
async function startCallSession(call: any) {
  ...
  try {
    ...
    // Subscribe to session events; addEventListener returns an unsubscribe function
    const unsubscribe = CometChatCalls.addEventListener('onSessionLeft', () => {
      console.log('Call ended');
    });
    
    return { token, sessionSettings, unsubscribe };
  } catch (error) {
...
function CallManager({ children }: { children: React.ReactNode }) {
  ...
  const startSession = async (call: Call) => {
    try {
      ...
      // Clear active call when the session is left
      CometChatCalls.addEventListener('onSessionLeft', () => {
        setActiveCall(null);
      });

      setActiveCall({ token, sessionSettings });
      setIncomingCall(null);
    } catch (error) {
        ...
        <Modal visible={true} animationType="slide">
          <View style={styles.callContainer}>
            <CometChatCalls.Component
              ...
              sessionSettings={activeCall.sessionSettings}
            />
          </View>
        </Modal>
      )}
