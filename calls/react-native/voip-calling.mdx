...
<Info>
  **Quick Reference for AI Agents & Developers**
  ...
  // providerId is the Provider ID from Dashboard → Notifications.
  ...
  * **Prerequisite** → [Ringing](/calls/react-native/ringing) covers the foreground half
</Info>
  ...
  try {
    await CometChatNotifications.registerPushToken(
      ...
      platform === 'ios'
        ...
        : PushPlatforms.FCM_REACT_NATIVE_ANDROID,
      PROVIDER_ID,                                 // Dashboard → Notifications → Provider ID
    );
...
<Note>
  ...
  (for example `APNS_PROVIDER_ID`). Pass it so the token is bound to the right provider.
</Note>
