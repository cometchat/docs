...
sdk_version: "5.x"
description: "Migrate to CometChat Calls SDK v5 on React Native with updated setup, APIs, call sessions, layouts, and feature changes."
---
...
- **Simplified initialization** — pass plain objects to `init()` instead of using builder classes
...
In React Native, `startSession()` is replaced by rendering `<CometChatCalls.Component>`. There is no `joinSession()` method — generate a call token, then render the component to join.
...
<Tab title="v5">
```jsx
// 1. Generate the call token
const { token } = await CometChatCalls.generateToken(sessionId);

// 2. Render the call UI to join the session
...
<Warning>
  **TypeScript: the deprecated listener methods are not on the exported type.** The compatibility layer
  above runs correctly, but `addCallEventListener` and `removeCallEventListener` are not declared on the
  exported `CometChatCalls` class, so a TypeScript project cannot use this incremental path — it fails to
  compile with `TS2551: Property 'addCallEventListener' does not exist on type 'typeof CometChatCalls'`.
  Since `npx react-native init` creates a TypeScript app by default, most projects should migrate their
  listeners to `addEventListener()` in one step rather than running the two side by side.
</Warning>
...
| `CometChatCalls.CallSettingsBuilder` | Pass plain `SessionSettings` object to `<CometChatCalls.Component>` |
| `CometChatCalls.OngoingCallListener` | `CometChatCalls.addEventListener()` |
| `CometChatCalls.startSession()` | Render `<CometChatCalls.Component>` (no `joinSession` in React Native) |
| `CometChatCalls.endSession()` | `CometChatCalls.leaveSession()` |
