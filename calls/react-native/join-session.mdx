...
<Info>
  **Quick Reference for AI Agents & Developers**
  ...
  // 1. a token for this session (init + login must already have resolved)
  const { token } = await CometChatCalls.generateToken(sessionId);

  // 2. joining IS rendering — React Native has NO joinSession()
  <View style={{ flex: 1 }}>
  ...
  * **Related** → [Session Settings](/calls/react-native/session-settings) · [Events](/calls/react-native/events) · [Actions](/calls/react-native/actions)
</Info>
...
<Note>
  **Two equivalent ways to remove a listener.** `addEventListener` **returns an unsubscribe function** and
  also accepts an optional `AbortSignal`:

  ```ts
  static addEventListener(eventType, listener, options?: { signal?: AbortSignal }): () => void
  ```

  Call the returned function, or pass `{ signal }` and `abort()` the controller — one `AbortController`
  removes every listener registered with its signal, which is why the examples here use it. Neither
  supersedes the other; other pages show the unsubscribe form.
</Note>
