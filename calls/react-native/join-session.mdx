  ...
  useEffect(() => {
    async function initializeCall() {
      try {
        ...
        setCallSettings(sessionSettings);
      } catch (error) {
...
| `callSettings` | SessionSettings | No | Configuration object (see [Session Settings](/calls/react-native/session-settings)) |
  ...
  useEffect(() => {
    async function initializeCall() {
      try {
        ...
        CometChatCalls.addEventListener('onSessionLeft', handleCallEnd);
        CometChatCalls.addEventListener('onSessionTimedOut', handleCallEnd);
        CometChatCalls.addEventListener('onParticipantJoined', (user) => {
          console.log('User joined:', user.name);
        });
        CometChatCalls.addEventListener('onParticipantLeft', (user) => {
