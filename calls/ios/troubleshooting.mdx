...
description: "Troubleshoot CometChat Calls SDK v5 on iOS for setup, authentication, permissions, call launch, media, and session issues."
sdk_version: "5.x"
---
...
| App crashes with `-[AppDelegate window]: unrecognized selector` when a call starts | Scene-based app with no `window` property on the `AppDelegate` | Add `var window: UIWindow?` to your `AppDelegate`. See [Setup](/calls/ios/setup#declare-a-window-on-your-appdelegate) |
...
| CallKit UI not showing | Incoming calls are not reported to CallKit | If you use the [iOS Push Notifications SDK](/notifications/ios-push-notifications-sdk), do not add your own `CXProvider` or `PKPushRegistry` delegate: the SDK is already both, and a second one competes for the same callbacks. Check that `registerForVoIPPushes()` is called and that you are testing on a physical device. If you wired CallKit by hand, report the incoming call from your `CXProvider`. See [VoIP Calling](/calls/ios/voip-calling) |
