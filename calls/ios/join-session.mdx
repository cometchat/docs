...
sdk_version: "5.x"
description: "Join CometChat Calls SDK v5 sessions on iOS with session IDs, call tokens, participant setup, and in-call UI launch."
---
...
<Warning>
  **Planning to use [in-app Picture-in-Picture](/calls/ios/picture-in-picture)? Don't pin the container with constraints.**
  `PiPViewCoordinator` moves and resizes the container by setting its `frame`. Constraints on the
  container override that frame at the next layout pass, so the call stays full-screen: the call UI
  switches to its PiP layout, but there is no mini-call to drag and no exit button. Lay out the
  container by frame instead:

  ```swift
  let callViewContainer = UIView(frame: view.bounds)
  callViewContainer.autoresizingMask = [.flexibleWidth, .flexibleHeight]
  view.addSubview(callViewContainer)
  ```

  The same applies to a storyboard container: give it no constraints, so Interface Builder uses its autoresizing mask.
</Warning>
...
| `onError` | Closure | Closure called on failure. Can also fire before a successful join; see [Error Handling](#error-handling) |
...
<Warning>
In Calls SDK 5.0.x, `joinSession` can call `onError` with `INIT ERROR` ("CometChatCalls in not Initialised properly") once or twice and then join anyway, calling `onSuccess`. This happens on joins that follow a fresh `login` in the same app run, even when `init` and `login` both succeeded. `joinSession(sessionID:)` checks twice, so it can report the error twice.

Don't end the call, show an alert, or retry on `onError` alone. If `init` and `login` both succeeded, log an `INIT ERROR` and wait. Treat the join as successful when `onSuccess` or `onSessionJoined` fires. Handle other error codes as real failures.
</Warning>
