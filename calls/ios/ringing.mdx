...
sdk_version: "5.x"
description: "Configure CometChat Calls SDK v5 ringing on iOS for incoming calls, outgoing calls, call alerts, and accept or reject flows."
---
...
Both SDKs must be initialized and have the same user logged in before any code on this page runs. Do it in this order:

1. **Chat SDK init** — `CometChat.init(appId:appSettings:onSuccess:onError:)`. See [Chat SDK Setup](/sdk/ios/setup).
2. **Chat SDK login** — `CometChat.login(...)`. See [Chat SDK Authentication](/sdk/ios/authentication-overview). `initiateCall`, `acceptCall`, `rejectCall` and the call listener all run on this session.
3. **Calls SDK init** — `CometChatCalls(callsAppSettings:onSuccess:onError:)`. See [Setup](/calls/ios/setup).
4. **Calls SDK login** — `CometChatCalls.login(...)`. See [Authentication](/calls/ios/authentication). `joinSession` fails without it.
5. **Register the call listener** — `CometChat.addCallListener(_:_:)`, then initiate or accept calls.

Each SDK must finish its `init` before its `login`. Wait for each `onSuccess` before starting the next step:
...
CometChat.init(appId: appId, appSettings: appSettings, onSuccess: { _ in
    ...
    // 2. Log the user in to the Chat SDK
    CometChat.login(UID: "cometchat-uid-1", authKey: "AUTH_KEY", onSuccess: { user in

        // 3. Initialize the Calls SDK
        let callAppSettings = CallAppSettingsBuilder()
            .set(appID: appId)
            .set(region: region)
            .build()

        CometChatCalls(callsAppSettings: callAppSettings, onSuccess: { _ in

            // 4. Log the same user in to the Calls SDK, reusing the Chat SDK's session token
            CometChatCalls.login(authToken: CometChat.getUserAuthToken() ?? "", onSuccess: { callsUser in
                // 5. Register your call listener, then initiate or accept calls
            }, onError: { error in
                print("Calls SDK login failed: \(error.errorDescription)")
            })
        }, onError: { error in
            print("Calls SDK initialization failed: \(error?.errorDescription ?? "")")
        })
    }, onError: { error in
        print("Chat SDK login failed: \(error.errorDescription)")
    })
}, onError: { error in
    print("Chat SDK initialization failed: \(error.errorDescription)")
})
```
</Tab>
</Tabs>
...
<Warning>
`login(UID:authKey:)` is for development only. In production, log in to the Chat SDK with an auth token from your backend. See [Chat SDK Authentication](/sdk/ios/authentication-overview).
</Warning>
