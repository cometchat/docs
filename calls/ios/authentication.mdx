...
sdk_version: "5.x"
description: "Configure authentication for CometChat Calls SDK v5 on iOS with app credentials, logged-in users, and secure call access."
---
...
The SDK also provides `login(UID:authKey:onSuccess:onError:)` (`loginWithUID:authKey:onSuccess:onError:` in Objective-C). It takes the same key and behaves exactly like `login(UID:apiKey:onSuccess:onError:)`, so you can use whichever label matches the rest of your code:
