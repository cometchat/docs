...
<Warning>
  **The container must be laid out by frame, not Auto Layout.** `PiPViewCoordinator` positions the
  view by setting its `frame`, `center` and `autoresizingMask`, and never touches constraints. If the
  container has `translatesAutoresizingMaskIntoConstraints = false` and is pinned to its superview,
  as in the constraint snippet on [Join Session](/calls/ios/join-session#container-setup), those constraints win at the
  next layout pass. `onPictureInPictureLayoutEnabled` still fires, but the call stays full-screen,
  with no controls and no exit button. Give this view no constraints: the example below never adds any to `callContainer`, and the coordinator sets its frame and autoresizing mask itself.
</Warning>
