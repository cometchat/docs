...
sidebarTitle: "Events"
sdk_version: "5.x"
description: "Handle CometChat Calls SDK v5 events on iOS for call lifecycle, participants, media state, errors, and real-time updates."
---
...
<Note>
`CallSession` holds listeners **weakly**. It does not keep your listener alive, so the object you register must be owned elsewhere, such as the view controller itself (as in the examples below) or a stored property. A listener created inline, like `addSessionStatusListener(MyListener())`, is deallocated right away and never receives an event. A deallocated listener is dropped automatically. Call `removeSessionStatusListener`, `removeParticipantEventListener`, etc. when you want events to stop before the listener goes away.
</Note>
...
<Note>
  ...
  started from another client, but has no API to start one locally.
</Note>
...
| `.REAR` | Rear-facing (main) camera is active |
</Accordion>
</AccordionGroup>
