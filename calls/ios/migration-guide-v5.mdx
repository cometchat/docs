...
sdk_version: "5.x"
description: "Migrate to CometChat Calls SDK v5 on iOS with updated setup, APIs, call sessions, layouts, and feature changes."
---
...
<Note>
v5 listeners use weak references internally, so they are automatically cleaned up when the listener object is deallocated. This also means the SDK does not keep a listener alive: store `mySessionListener` and the others in a property (or register a view controller that conforms to the protocol). A listener passed in without another owner is deallocated immediately and receives no events. You can also explicitly remove listeners using `removeSessionStatusListener(_:)`, `removeParticipantEventListener(_:)`, etc.
</Note>
