...
`CallLogsBuilder` is nested inside `CallLogsRequest`, so in Swift you create it as `CallLogsRequest.CallLogsBuilder()`. Configure the request using the builder methods:
...
| `initiator` | [CallEntity](#callentity) | User who initiated the call |
| `receiver` | [CallEntity](#callentity) | User or group that received the call |
...
`initiator` and `receiver` are declared as `CallEntity`, the base class. At runtime each one is a `CallUser` or a `CallGroup`. Cast to the subclass to read its identifier and image:

| Class | Property | Type | Description |
|-------|----------|------|-------------|
| `CallEntity` | `name` | String | Display name. Inherited by both subclasses |
| `CallUser` | `uid` | String | User ID |
| `CallUser` | `avatar` | String? | Avatar URL |
| `CallGroup` | `guid` | String | Group ID |
| `CallGroup` | `icon` | String? | Group icon URL |

`receiverType` tells you which subclass `receiver` is. To show who a call was with, use the group for group calls. For one-to-one calls, use whichever user is not the logged-in user:

```swift
callLogRequest.fetchNext(onSuccess: { callLogs in
    let myUID = CometChatCalls.getLoggedInUser()?.uid

    for callLog in callLogs {
        switch callLog.receiver {
        case let group as CallGroup:
            // Group call: the group is the other side
            print("Group: \(group.name) (\(group.guid))")
        case let receiver as CallUser:
            // One-to-one call: whichever user isn't me
            let other = receiver.uid == myUID ? callLog.initiator : receiver
            if let user = other as? CallUser {
                print("User: \(user.name) (\(user.uid)), avatar: \(user.avatar ?? "none")")
            }
        default:
            break
        }
