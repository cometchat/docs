...
Monitor audio mode changes using `MediaEventListeners`:
...
final mediaEventListener = _MediaEventListener();
callSession?.addMediaEventsListener(mediaEventListener);
```

<Warning>
Flutter listeners are not lifecycle-aware, and `removeMediaEventsListener` matches on the
instance you passed in. Keep the listener in a field — a listener constructed inline can
never be removed — and call `callSession?.removeMediaEventsListener(mediaEventListener)` from your widget's
`dispose()`.
</Warning>
