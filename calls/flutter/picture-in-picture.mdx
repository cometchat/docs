...
sdk_version: "5.x"
description: "Configure picture-in-picture for CometChat Calls SDK v5 on Flutter to keep video calls visible while users multitask."
---
...
Monitor PiP mode transitions using `LayoutListeners` to update your UI accordingly:
...
class _LayoutListener extends LayoutListeners {
  ...
  // override only what you need (onCallLayoutChanged, onParticipantListVisible, onParticipantListHidden).
}

CallSession? callSession = CallSession.getInstance();

callSession?.layoutListener = _LayoutListener();
```
