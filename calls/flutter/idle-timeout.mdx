...
sdk_version: "5.x"
description: "Configure CometChat Calls SDK v5 idle timeout on Flutter to handle inactive participants and call session cleanup."
---
...
Listen for the `onSessionTimedOut` callback using `SessionStatusListeners` to handle when the session ends due to idle timeout:
...
class _SessionStatusListener extends SessionStatusListeners {
  ...
  // override only what you need (onSessionJoined, onSessionLeft, onConnectionLost, onConnectionRestored, …).
}
...
<Note>
Flutter listeners are not lifecycle-aware, and `removeSessionStatusListener` matches on the
instance you passed in. Keep the listener in a field — a listener constructed inline can
never be removed — and call `callSession?.removeSessionStatusListener(sessionStatusListener)` from your widget's
`dispose()`.
</Note>
