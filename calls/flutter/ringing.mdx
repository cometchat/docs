...
// In dispose()
callSession?.removeButtonClickListener(buttonClickListener);
...
```dart
class _CallEndListener with CallListener {
  _CallEndListener(this.context);
...
final callEndListener = _CallEndListener(context);
CometChat.addCallListener(listenerID, callEndListener);
```

<Note>
`CallListener` is a mixin with no constructor, so it cannot be instantiated directly. Apply it
to your own class with `with CallListener` and override only the callbacks you need. Keep a
reference to the instance — `CometChat.removeCallListener(listenerID)` is what detaches it.
</Note>
