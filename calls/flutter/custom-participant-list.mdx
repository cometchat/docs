...
class _CallScreenState extends State<CallScreen> {
  ...
  late final ParticipantEventListeners _participantListener;
  ...
  void _setupParticipantListener() {
    _participantListener = _ParticipantEventListener();
  ...
  void _onPinParticipant(Participant participant) async {
    ...
    final uid = participant.uid;
    if (uid == null) return;
    await CallSession.getInstance()?.pinParticipant(uid);
  }
...
class _CallScreenState extends State<CallScreen> {
  ...
  void _setupParticipantListener() {
    _participantListener = _ParticipantEventListener2();
    CallSession.getInstance()?.addParticipantEventListener(_participantListener);
  }
