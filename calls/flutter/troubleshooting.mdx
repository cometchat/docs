...
description: "Troubleshoot CometChat Calls SDK v5 on Flutter for setup, authentication, permissions, call launch, media, and session issues."
sdk_version: "5.x"
---
...
| Android build fails | `minSdkVersion` too low | Set `minSdkVersion` to 26 or higher in `android/app/build.gradle` |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `generateCallToken()` fails | User not logged in or invalid session ID | Ensure `login()` completes before generating a token |
...
| `joinSession()` throws error | Invalid or expired token | Generate a fresh token using `generateCallToken()` before joining |
