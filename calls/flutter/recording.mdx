...
final recordingMediaListener = _MediaEventListener();
callSession?.addMediaEventsListener(recordingMediaListener);

// In dispose()
callSession?.removeMediaEventsListener(recordingMediaListener);
```
...
final recordingParticipantListener = _ParticipantEventListener();
callSession?.addParticipantEventListener(recordingParticipantListener);

// In dispose()
callSession?.removeParticipantEventListener(recordingParticipantListener);
```
...
never be removed — and call `callSession?.removeParticipantEventListener(recordingParticipantListener)` from your widget's
`dispose()`.
The same applies to the media events listener above.
</Note>
