...
Use `MediaEventListeners` to keep your UI synchronized with the actual call state.
...
void _setupMediaEventsListener() {
  _mediaListener = _MediaEventListener(),
  );
  CallSession.getInstance()?.addMediaEventsListener(_mediaListener);
}
```
...
Use `SessionStatusListeners` to handle session end events:
...
```dart
class _SessionStatusListener2 extends SessionStatusListeners {
  @override
  void onSessionLeft() {
          if (mounted) Navigator.of(context).pop();
        }
...
class _MediaEventListener2 extends MediaEventListeners {
  @override
  void onAudioMuted() {
          setState(() => isAudioMuted = true);
        }
...
class _CallScreenState extends State<CallScreen> {
  ...
  late final SessionStatusListeners _sessionListener;

  late final MediaEventListeners _mediaListener;
  ...
  void _setupMediaEventsListener() {
    _mediaListener = _MediaEventListener2(),
    );
    CallSession.getInstance()?.addMediaEventsListener(_mediaListener);
  }

  void _setupSessionStatusListener() {
    _sessionListener = _SessionStatusListener2(),
    );
    CallSession.getInstance()?.addSessionStatusListener(_sessionListener);
  }
  ...
  @override
  void dispose() {
    CallSession.getInstance()?.removeMediaEventsListener(_mediaListener);
