...
sdk_version: "5.x"
description: "Migrate to CometChat Calls SDK v5 on Flutter with updated setup, APIs, call sessions, layouts, and feature changes."
---
...
- **Granular event listeners** — 5 focused listener classes (`SessionStatusListeners`, `ParticipantEventListeners`, `MediaEventListeners`, `ButtonClickListeners`, `LayoutListeners`) instead of one monolithic `CometChatCallsEventsListener`
...
Key differences:
- v5 replaces `generateToken()` with `generateCallToken()`, which takes no `authToken` parameter (it uses the token cached by `loginWithAuthToken()`). `generateToken()` still exists but is deprecated.
...
class _LayoutListener extends LayoutListeners {
  ...
  // override only what you need (onCallLayoutChanged, onPictureInPictureLayoutEnabled, onPictureInPictureLayoutDisabled).
}

class _ButtonClickListener extends ButtonClickListeners {
  // every other callback already defaults to a no-op on the abstract class —
  // override only what you need (onLeaveSessionButtonClicked, onToggleAudioButtonClicked, onToggleVideoButtonClicked, onSwitchCameraButtonClicked, …).
}
...
class _SessionStatusListener extends SessionStatusListeners {
  // every other callback already defaults to a no-op on the abstract class —
  // override only what you need (onSessionJoined, onSessionLeft, onSessionTimedOut, onConnectionLost, …).
}
...
|----------|-------------|----------|
| `onCallEnded()` | `SessionStatusListeners` | `onSessionLeft()` |
| `onCallEndButtonPressed()` | `ButtonClickListeners` | `onLeaveSessionButtonClicked()` |
| `onSessionTimeout()` | `SessionStatusListeners` | `onSessionTimedOut()` |
| `onUserJoined(RTCUser)` | `ParticipantEventListeners` | `onParticipantJoined(Participant)` |
| `onUserLeft(RTCUser)` | `ParticipantEventListeners` | `onParticipantLeft(Participant)` |
| `onUserListChanged(List<RTCUser>)` | `ParticipantEventListeners` | `onParticipantListChanged(List<Participant>)` |
| `onAudioModeChanged(List<AudioMode>)` | `MediaEventListeners` | `onAudioModeChanged(AudioMode)` |
| `onCallSwitchedToVideo(CallSwitchRequestInfo)` | *Removed* | — |
| `onUserMuted(RTCMutedUser)` | `ParticipantEventListeners` | `onParticipantAudioMuted(Participant)` |
| `onRecordingToggled(RTCRecordingInfo)` | `MediaEventListeners` | `onRecordingStarted()` / `onRecordingStopped()` |
...
|----------|-------|-------------|
| `SessionStatusListeners` | `onConnectionLost()` | Network interrupted |
| `SessionStatusListeners` | `onConnectionRestored()` | Network restored |
| `SessionStatusListeners` | `onConnectionClosed()` | Connection permanently closed |
| `ParticipantEventListeners` | `onParticipantVideoResumed()` | Participant turned camera on |
| `ParticipantEventListeners` | `onParticipantVideoPaused()` | Participant turned camera off |
| `ParticipantEventListeners` | `onParticipantHandRaised()` | Participant raised hand |
| `ParticipantEventListeners` | `onParticipantHandLowered()` | Participant lowered hand |
| `ParticipantEventListeners` | `onParticipantStartedScreenShare()` | Participant started screen share |
| `ParticipantEventListeners` | `onParticipantStoppedScreenShare()` | Participant stopped screen share |
| `ParticipantEventListeners` | `onDominantSpeakerChanged()` | Active speaker changed |
| `MediaEventListeners` | `onCameraFacingChanged()` | Camera switched front/back |
| `ButtonClickListeners` | Various button events | Individual button click tracking |
| `LayoutListeners` | `onCallLayoutChanged()` | Layout type changed |
| `LayoutListeners` | `onPictureInPictureLayoutEnabled/Disabled()` | PiP state changed |

---

## Call Logs

`CallLogRequest` itself is unchanged; auth is now handled by `CometChatCalls.login()` instead of
passing the auth token manually. Note that `CallLogRequestBuilder` is a **top-level class** with
public **fields** — it takes cascaded assignments, not `setX()` calls.
...
| `RTCUser` | `Participant` |
| `RTCMutedUser` | `ParticipantEventListeners.onParticipantAudioMuted(Participant)` |
| `RTCRecordingInfo` | `MediaEventListeners.onRecordingStarted()` / `onRecordingStopped()` |
