...
| CSS | Add `css-variables.css` to `angular.json` styles array or import in global `styles.css` via `node_modules/@cometchat/chat-uikit-angular/styles/css-variables.css` |
...
<Warning>
**Your Auth Key can authenticate as any user on your app. Never commit it, and never ship it
in a production bundle.**

`ng new` scaffolds `src/environments/` as ordinary tracked source, so a `git commit -a` after
this step puts the key in your repository history. Keep it out of version control from the
start:
...
Commit a template with placeholder values so teammates know what to fill in, and keep the real
values in the gitignored file:

```ts title="src/environments/environment.example.ts"
// Committed. Copy to environment.ts and fill in real values.
...
Before you go live, move to the [Auth Token flow](/ui-kit/angular/methods#login-using-auth-token):
your server mints a token per user, and the Auth Key never reaches the browser.
</Warning>

```ts title="src/environments/environment.ts"
// Gitignored. Development only — see the warning above.
export const environment = {
  production: false,
  cometchat: {
    appId: "APP_ID",       // Replace with your App ID
    region: "REGION",      // Replace with your Region
    authKey: "AUTH_KEY",   // Replace with your Auth Key — never commit this
  },
};
```
...
<Note>
Use the full `node_modules` path shown above. The package-specifier form —
`@import '@cometchat/chat-uikit-angular/styles/css-variables.css'` — does **not**
resolve: the package's `exports` map does not expose the `./styles/*` subpath, so
the build fails with `Could not resolve`.
</Note>
</Tab>
</Tabs>
...
<Card title="Build Conversation List + Message View" href="/ui-kit/angular/angular-conversation" icon="comments">
...
<Card title="Build One-to-One / Group Chat" href="/ui-kit/angular/angular-one-to-one-chat" icon="message">
  Step-by-step guide to build this layout
</Card>
...
<Card title="Build Tab-Based Chat" href="/ui-kit/angular/angular-tab-based-chat" icon="table-columns">
  Step-by-step guide to build this layout
</Card>
