...
| SDK requirement | A Chat SDK build exposing `CometChat.subscribeToThread` (4.2.0 or later) |
...
|:---|:---|:---|
| Thread header control | Icon-only bell in the thread header's top bar | State-labelled icon; tooltip and accessible name say "Subscribe to thread" / "Unsubscribe from thread" |
...
Replying to a thread, or being @mentioned in one, auto-subscribes the user server-side. The Chat SDK emits no subscription events, so nothing arrives to relay: the UI Kit works the change out itself from the message it already has — a reply you authored, or one whose mentioned users include you — and publishes `ccThreadSubscriptionChanged` so both surfaces update without a refetch. No extra write is made; the server has already recorded it.
