...
| Selector | `<cometchat-user-item>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatUserItemComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `isActive` · `isSelected` · `isFocused` · `tabIndex` · `contextMenuOptions` |
| Key outputs | `itemClick` · `itemSelect` · `contextMenuOpen` · `contextMenuOptionClick` |
