...
| Selector | `<cometchat-reactions>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatReactionsComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `message` · `reactionsRequestBuilder` · `alignment` · `hoverDebounceTime` |
| Key outputs | `reactionClick` · `reactionListItemClick` |
| View slots | — |
