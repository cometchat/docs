...
| Standalone | Standalone — add `CometChatAIAssistantChat` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `streamingSpeed` · `aiAssistantTools` · `loadLastAgentConversation` · `suggestedMessages` |
| Key outputs | `backClick` |
| View slots | `greetingTemplate` — override via `<ng-template>` |
| Other inputs | 2 `hide*`/`show*`/`disable*` refinements — see the props table below |
...
<Info>
  **Live Preview** — default AI assistant chat preview.
  [Open in Storybook ↗](https://storybook.cometchat.io/angular/?path=/docs/components-ai-ai-assistant-chat--docs)
</Info>

<iframe
  src="https://storybook.cometchat.io/angular/iframe.html?id=components-ai-ai-assistant-chat--default&viewMode=story&shortcuts=false&singleStory=true"
  className="w-full rounded-xl"
  loading="lazy"
  style={{height: "420px", border: "1px solid #e0e0e0"}}
