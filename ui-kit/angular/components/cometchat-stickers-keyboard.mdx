...
| Selector | `<cometchat-stickers-keyboard>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatStickersKeyboardComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `errorStateText` · `emptyStateText` · `autoFocus` · `trapFocus` |
| Key outputs | `stickerClick` · `closeKeyboard` |
| View slots | — |
