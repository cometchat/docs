...
| Selector | `<cometchat-incoming-call>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatIncomingCallComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `call` · `customSoundForCalls` · `onAccept` · `onDecline` · `onError` |
| Key outputs | `callAccepted` · `callDeclined` · `error` |
| View slots | `itemView` · `titleView` · `subtitleView` · `leadingView` · `trailingView` · `acceptButtonView` · `declineButtonView` — override via `<ng-template>` |
