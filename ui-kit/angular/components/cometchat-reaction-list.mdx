...
| Selector | `<cometchat-reaction-list>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatReactionListComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `message` · `reactionsRequestBuilder` |
| Key outputs | `itemClick` · `empty` |
| View slots | — |
