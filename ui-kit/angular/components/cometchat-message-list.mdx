...
| Pin permission | No client-side role gate. Pin is offered to every member and the server decides, refusing with `ERR_ACTION_NOT_ALLOWED`. Saving is private to the user and has no gate |
