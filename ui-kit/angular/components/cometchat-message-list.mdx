...
| Standalone | Standalone — add `CometChatMessageListComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `group` · `parentMessageId` · `messagesRequestBuilder` · `reactionsRequestBuilder` · `isAgentChat` · `loadLastAgentConversation` · `textFormatters` |
| Key outputs | `error` · `threadRepliesClick` · `reactionClick` · `reactionListItemClick` · `smartReplyClick` · `conversationStarterClick` · `messagePrivatelyClick` · `replyClick` |
| View slots | `hideModerationView` · `emptyView` · `errorView` · `loadingView` · `headerView` · `footerView` · `bubbleFooterView` · `appendView` · `listItemTemplate` · `emptyStateTemplate` · `errorStateTemplate` · `loadingStateTemplate` — override via `<ng-template>` |
