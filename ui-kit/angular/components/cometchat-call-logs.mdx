...
| Selector | `<cometchat-call-logs>` — the class name is NOT a valid tag |
...
| Prerequisites | `CometChatUIKit.initFromSettings()` resolved, `login()` complete, **and** the Calls SDK finished loading — see the readiness warning below |
...
<Warning>
**Calls SDK readiness.** `@cometchat/calls-sdk-javascript` is an optional peer and is loaded
lazily, so it is not available the instant your app boots. If `<cometchat-call-logs>` is in
your **initial** template, it can mount before the SDK resolves and fail with:

```
TypeError: Cannot read properties of null (reading 'CallLogRequestBuilder')
```

The component drops into its error state, and its **Retry button does not refetch** — the
error does not clear until the page is reloaded. This is a known kit defect, not a
configuration problem on your side.

Until it is fixed, render call logs behind a real user action — a Calls tab the user opens,
or an `@if` gate — rather than in the first template that mounts. Do **not** gate on
`CometChatUIKit.callingReady`: it defaults to an already-resolved promise, so awaiting it
guarantees nothing.
</Warning>
