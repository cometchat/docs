...
| Selector | `<cometchat-message-composer>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatMessageComposerComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `group` · `parentMessageId` · `mentionsUsersRequestBuilder` · `mentionsGroupMembersRequestBuilder` · `placeholderText` · `initialComposerText` · `text` |
| Key outputs | `textChange` · `sendButtonClick` · `error` · `closePreview` · `attachmentAdded` · `attachmentRemoved` · `mentionSelected` |
| View slots | `headerView` · `footerView` · `sendButtonView` · `auxiliaryButtonView` · `secondaryButtonView` · `attachmentIconView` · `voiceRecordingIconView` · `emojiIconView` · `errorView` · `listItemTemplate` · `emptyStateTemplate` · `errorStateTemplate` · `loadingStateTemplate` — override via `<ng-template>` |
| Other inputs | 25 `hide*`/`show*`/`disable*` refinements — see the props table below |
| Prerequisites | `CometChatUIKit.initFromSettings()` resolved **and** `login()` complete before render ([Integration](/ui-kit/angular/integration)) |
| Constraints | Fills its container — an ancestor without a resolved height collapses it to 0px. Bind `[user]` **or** `[group]`, never both |
| Related | [Components overview](/ui-kit/angular/components/components-overview) · [Theming](/ui-kit/angular/customization/theming) · [LLM index](/ui-kit/angular/llms-angular-v5) |
...
Attachments are enabled by default through the attachment button. With
`enableMultipleAttachments` on (the default), selected files are staged in a tray
below the input, where they can be reviewed, removed, or added to before being sent
together as a single batch. See [Multiple Attachments](#multiple-attachments).
...
@Component({
  ...
  template: `
    <div class="chat-layout">
      <!-- Do NOT bind (itemClick) in state-service mode. The UI Kit only
           auto-sets the active conversation when itemClick is not observed. -->
...
`enableMultipleAttachments` is `true` by default. In this mode the composer opens a
**staging tray** below the input instead of sending files immediately. Files begin
uploading the moment they're staged — not when you press **Send** — so each tile shows
live upload progress and, because the attachments are already uploaded by send time, the
batch is delivered instantly.

- **Stage** — picking files from an attachment option (or dragging and dropping them)
  adds them to the tray as tiles that show upload progress. The user can keep adding
  files across multiple picks, remove a tile, retry a failed upload, or preview an
  image or video before sending.
- **Batch send** — pressing send delivers the staged files as a batch. Files are
  grouped **by type** and sent as one media message per type — for example, three
  images and two files are sent as **two** messages (one image message with three
  attachments and one file message with two), not five. Every message in the batch
  shares a `metadata.batchId`, and the composer text is attached as the caption on the
  **last** message only.
- **Grouped rendering** — the message list renders each batched message with the
  matching multi-attachment bubble — [Images](/ui-kit/angular/message-bubble-styling#image-bubble),
  [Videos](/ui-kit/angular/message-bubble-styling#video-bubble),
  [Audios](/ui-kit/angular/message-bubble-styling#audio-bubble), or
  [Files](/ui-kit/angular/message-bubble-styling#file-bubble) — and groups messages that
  share a `batchId` into one visual block.

The maximum number of attachments per batch comes from your app's
`CometChat.getMaxAttachmentCount()` setting (default `10`). If a selection would exceed
the remaining slots, none of it is staged and a toast is shown. Use the `(error)`
output to surface send or validation failures:
...
Set `enableMultipleAttachments` to `false` to restore the legacy behavior, where each
selected file is sent immediately with no staging tray. This is a send-side setting only —
incoming messages that carry several attachments still render with the multi-attachment
bubbles. See [Media Message Rendering](/ui-kit/angular/components/cometchat-message-bubble#media-message-rendering).

**Threaded replies** — multiple attachments work inside threads. Set `parentMessageId`
on the composer and the staged uploads (and the resulting batch) are scoped to that
thread. See [Threaded Replies](#threaded-replies).
...
Completed recordings are tagged with `audioType: "voice_note"` and render as a
[Voice Note Bubble](/ui-kit/angular/message-bubble-styling#voice-note-bubble), separately
from attached audio files, which render as an
[Audios Bubble](/ui-kit/angular/message-bubble-styling#audio-bubble).
