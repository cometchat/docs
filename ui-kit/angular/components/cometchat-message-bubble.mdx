...
| Selector | `<cometchat-message-bubble>` — the class name is NOT a valid tag |
| Mounting | Rendered **by** `<cometchat-message-list>` when a matching message arrives — do **not** mount it yourself. Import it only to override it via a view slot. |
| Key inputs | `group` · `message` · `reactionsRequestBuilder` · `alignment` · `options` · `quickOptionsCount` · `textFormatters` · `dateFormat` |
| Key outputs | `optionClick` · `replyPreviewClick` · `avatarClick` · `threadRepliesClick` · `reactionClick` · `reactionListItemClick` · `mediaToggle` · `messageActionsOpen` |
| View slots | `leadingView` · `headerView` · `replyView` · `contentView` · `bottomView` · `footerView` · `statusInfoView` · `threadView` · `hideThreadView` · `bubbleView` · `hideStatusInfoView` · `hideModerationView` — override via `<ng-template>` |
| Other inputs | 9 `hide*`/`show*`/`disable*` refinements — see the props table below |
| Prerequisites | A rendered `<cometchat-message-list>` — the bubble has no standalone lifecycle |
| Constraints | Its message type is set by the sender; the list picks the bubble, not you |
...
| Message type | Bubble |
| --- | --- |
| `image` | [`CometChatImagesBubble`](/ui-kit/angular/message-bubble-styling#image-bubble) |
| `video` | [`CometChatVideosBubble`](/ui-kit/angular/message-bubble-styling#video-bubble) |
| `file` | [`CometChatFilesBubble`](/ui-kit/angular/message-bubble-styling#file-bubble) |
| `audio` tagged as a voice note | [`CometChatVoiceNoteBubble`](/ui-kit/angular/message-bubble-styling#voice-note-bubble) |
| `audio` (attached file) | [`CometChatAudiosBubble`](/ui-kit/angular/message-bubble-styling#audio-bubble) |

Most of these delegate the media rendering to their single-attachment counterpart — Images to [`CometChatImageBubble`](/ui-kit/angular/components/cometchat-image-bubble), Videos to [`CometChatVideoBubble`](/ui-kit/angular/components/cometchat-video-bubble), Files to [`CometChatFileBubble`](/ui-kit/angular/components/cometchat-file-bubble), and Voice Note to [`CometChatAudioBubble`](/ui-kit/angular/components/cometchat-audio-bubble). Audios is the exception: it draws its own player rows rather than delegating. All of them add the batch-aware shared width and the `isFirstInBatch` / `isLastInBatch` grouping that makes a batch read as one visual block. The legacy bubbles stay exported for direct use; the message bubble no longer routes to them.
...
| `setGlobalView` | `part: BubblePart, view: TemplateRef<any>` | Set a single view part (e.g. `'statusInfoView'`) for all message types |
| `setGlobalViews` | `views: Partial<Record<BubblePart, TemplateRef<any>>>` | Set multiple view parts for all message types at once |
...
Media messages route to the multi-attachment bubbles — see [Media Message Rendering](#media-message-rendering):

- [CometChatImagesBubble](/ui-kit/angular/message-bubble-styling#image-bubble) - Renders `image` messages
- [CometChatVideosBubble](/ui-kit/angular/message-bubble-styling#video-bubble) - Renders `video` messages
- [CometChatAudiosBubble](/ui-kit/angular/message-bubble-styling#audio-bubble) - Renders `audio` messages that are attached files
- [CometChatFilesBubble](/ui-kit/angular/message-bubble-styling#file-bubble) - Renders `file` messages
- [CometChatVoiceNoteBubble](/ui-kit/angular/message-bubble-styling#voice-note-bubble) - Renders `audio` messages tagged as voice notes

All except `CometChatAudiosBubble` delegate to a single-attachment counterpart. The message bubble no longer routes to these directly, but they remain available for standalone use:
