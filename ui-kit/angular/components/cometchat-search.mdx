...
| Selector | `<cometchat-search>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatSearchComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `guid` · `uid` · `conversationsRequestBuilder` · `messagesRequestBuilder` · `searchIn` · `searchFilters` · `initialSearchFilter` · `defaultSearchText` |
| Key outputs | `backClick` · `conversationClick` · `messageClick` · `searchError` |
| View slots | `conversationItemView` · `conversationLeadingView` · `conversationTitleView` · `conversationSubtitleView` · `conversationTrailingView` · `messageItemView` · `messageLeadingView` · `messageTitleView` · `messageSubtitleView` · `messageTrailingView` · `initialView` · `loadingView` · `emptyView` · `errorView` — override via `<ng-template>` |
| Other inputs | 4 `hide*`/`show*`/`disable*` refinements — see the props table below |
...
<iframe
  src="https://storybook.cometchat.io/angular/iframe.html?id=components-cometchatsearch--default&viewMode=story&shortcuts=false&singleStory=true"
