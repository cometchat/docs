...
| Selector | `<cometchat-users>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatUsersComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `usersRequestBuilder` · `searchRequestBuilder` · `searchKeyword` · `sectionHeaderKey` · `activeUser` · `selectionMode` · `options` |
| Key outputs | `itemClick` · `select` · `error` · `empty` · `selectionChange` |
...
| Other inputs | 8 `hide*`/`show*`/`disable*` refinements — see the props table below |
