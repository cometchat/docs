...
| Selector | `<cometchat-group-member-item>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatGroupMemberItemComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `member` · `isActive` · `isSelected` · `isFocused` · `tabIndex` · `contextMenuOptions` |
| Key outputs | `itemClick` · `itemFocus` · `contextMenuOptionClick` |
