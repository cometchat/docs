...
<Note>
  The icon is state-labelled — a plain bell when following, a struck-through bell when not — while the tooltip and accessible name say what tapping will do: "Subscribe to thread" or "Unsubscribe from thread". Those are the same two strings the action-sheet option uses, so the two surfaces never word the same action differently.
</Note>
