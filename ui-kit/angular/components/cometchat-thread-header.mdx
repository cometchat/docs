...
| Selector | `<cometchat-thread-header>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatThreadHeaderComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `parentMessage` · `replyCount` |
| Key outputs | `closeClick` · `backClick` |
| View slots | — |
