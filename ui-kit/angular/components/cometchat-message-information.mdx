...
| Selector | `<cometchat-message-information>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatMessageInformationComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `message` · `dateTimeFormat` · `textFormatters` |
| Key outputs | `closeClick` |
| View slots | — |
