...
| Selector | `<cometchat-conversations>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatConversationsComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `conversationsRequestBuilder` · `activeConversation` · `textFormatters` · `selectionMode` · `lastMessageDateTimeFormat` · `options` · `customSoundForMessages` · `slots` |
| Key outputs | `itemClick` · `select` · `error` · `searchBarClick` · `contextMenuOpen` · `contextMenuClose` · `scrollToTop` · `scrollToBottom` · `selectionChange` |
| View slots | `headerView` · `menuView` · `loadingView` · `emptyView` · `errorView` · `searchView` · `itemView` · `leadingView` · `titleView` · `subtitleView` · `trailingView` · `listItemTemplate` · `emptyStateTemplate` · `errorStateTemplate` · `loadingStateTemplate` — override via `<ng-template>` |
