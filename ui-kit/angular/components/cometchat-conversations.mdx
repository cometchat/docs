...
| Stitching | Wire `(itemClick)` → derive the subject with `conversation.getConversationWith()` → bind it to `<cometchat-message-header>` / `<cometchat-message-list>` / `<cometchat-message-composer>`. Pass the same value back as `[activeConversation]` or the open row never highlights. Prefer the exported `ChatStateService` over a local field — see [New Chat](/ui-kit/angular/guides/new-chat). |
