...
| Events | Pinning and unpinning publish nothing. This component re-orders itself directly, so no event is needed here — but a list of your own has none to subscribe to either. Read `conversation.isPinned()` when you render to stay in sync |
