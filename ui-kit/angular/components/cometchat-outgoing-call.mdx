...
| Selector | `<cometchat-outgoing-call>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatOutgoingCallComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `call` · `customSoundForCalls` · `onError` |
| Key outputs | `callCanceled` · `error` |
| View slots | `titleView` · `subtitleView` · `avatarView` · `cancelButtonView` — override via `<ng-template>` |
| Other inputs | 1 `hide*`/`show*`/`disable*` refinements — see the props table below |
