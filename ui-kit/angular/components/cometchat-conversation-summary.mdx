...
| Selector | `<cometchat-conversation-summary>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatConversationSummaryComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `getConversationSummary` · `closeCallback` |
| Key outputs | — |
| View slots | — |
