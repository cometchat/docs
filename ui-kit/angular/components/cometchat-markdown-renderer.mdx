...
| Selector | `<cometchat-markdown-renderer>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatMarkdownRenderer` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `text` · `streaming` |
| Key outputs | `imageClick` |
| View slots | — |
