...
| Selector | `<cometchat-group-item>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatGroupItemComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `group` · `isActive` · `isSelected` · `isFocused` · `tabIndex` · `contextMenuOptions` |
| Key outputs | `itemClick` · `itemSelect` · `contextMenuOpen` · `contextMenuOptionClick` · `itemFocus` |
