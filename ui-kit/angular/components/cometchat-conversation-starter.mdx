...
| Selector | `<cometchat-conversation-starter>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatConversationStarterComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `group` |
| Key outputs | `starterClick` |
| View slots | — |
