...
<Note>
[`CometChatMessageBubble`](/ui-kit/angular/components/cometchat-message-bubble#media-message-rendering) renders this bubble for **every** `image`-type message — there is no flag to opt out. The deprecated single-attachment [`CometChatImageBubble`](/ui-kit/angular/components/cometchat-image-bubble) is what this bubble delegates to internally and remains available as a standalone component, but the message bubble no longer routes to it.
</Note>

## Automatic Rendering

In the standard chat flow you do **not** instantiate this component yourself. [`CometChatMessageList`](/ui-kit/angular/components/cometchat-message-list) renders [`CometChatMessageBubble`](/ui-kit/angular/components/cometchat-message-bubble), which routes every `image`-type message to `CometChatImagesBubbleComponent`.
...
| --- | --- | --- |
| Routed by the message bubble | No — standalone use only | Default for image messages |
