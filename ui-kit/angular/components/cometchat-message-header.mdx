...
| Selector | `<cometchat-message-header>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatMessageHeaderComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `group` · `callSettingsBuilder` · `summaryGenerationMessageCount` · `lastActiveAtDateTimeFormat` |
| Key outputs | `backClick` · `itemClick` · `searchClick` · `conversationSummaryClick` · `error` · `voiceCallClick` · `videoCallClick` |
| View slots | `headerView` · `itemView` · `leadingView` · `titleView` · `subtitleView` · `trailingView` · `backButtonView` · `auxiliaryButtonView` · `listItemTemplate` · `emptyStateTemplate` · `errorStateTemplate` · `loadingStateTemplate` — override via `<ng-template>` |
| Other inputs | 7 `hide*`/`show*`/`disable*` refinements — see the props table below |
