...
| Standalone | Standalone — add `CometChatGroupMembersComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `group` · `groupMemberRequestBuilder` · `searchRequestBuilder` · `selectionMode` · `searchKeyword` · `options` |
| Key outputs | `error` · `itemClick` · `selectionChange` · `empty` |
| View slots | `headerView` · `menuView` · `loadingView` · `errorView` · `emptyView` · `itemView` · `subtitleView` · `trailingView` · `leadingView` · `titleView` · `listItemTemplate` · `emptyStateTemplate` · `errorStateTemplate` · `loadingStateTemplate` — override via `<ng-template>` |
...
| Constraints | Fills its container — an ancestor without a resolved height collapses it to 0px |
