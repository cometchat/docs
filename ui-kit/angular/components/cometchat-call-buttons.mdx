...
| Standalone | Standalone — add `CometChatCallButtonsComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `group` · `callSettingsBuilder` · `onVoiceCallClick` · `onVideoCallClick` · `onError` · `outgoingCallDisableSoundForCalls` · `outgoingCallCustomSoundForCalls` |
| Key outputs | `error` |
| View slots | `voiceCallButtonView` · `videoCallButtonView` — override via `<ng-template>` |
| Other inputs | 3 `hide*`/`show*`/`disable*` refinements — see the props table below |
