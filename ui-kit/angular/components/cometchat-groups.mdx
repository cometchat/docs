...
| Selector | `<cometchat-groups>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatGroupsComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `groupsRequestBuilder` · `searchRequestBuilder` · `activeGroup` · `selectionMode` · `options` |
| Key outputs | `itemClick` · `select` · `error` · `selectionChange` |
| View slots | `headerView` · `menuView` · `loadingView` · `emptyView` · `errorView` · `itemView` · `leadingView` · `titleView` · `subtitleView` · `trailingView` · `listItemTemplate` · `emptyStateTemplate` · `errorStateTemplate` · `loadingStateTemplate` — override via `<ng-template>` |
| Other inputs | 5 `hide*`/`show*`/`disable*` refinements — see the props table below |
...
The component auto-updates `ChatStateService` (via `setActiveGroup`) only when `(itemClick)` is **not** observed. For this service-driven mode, leave `(itemClick)` unbound so the automatic selection stays enabled.
...
@Component({
  ...
  template: `
    <div class="chat-layout">
      <!-- Do not bind (itemClick) so ChatStateService is updated automatically -->
...
Because this example binds `(itemClick)`, the automatic `ChatStateService` selection is suppressed, so you must drive selection manually. Here the `onGroupClick` handler stores the group in `activeGroup` and passes `[group]="activeGroup"` into each downstream component.
