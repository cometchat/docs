...
| Selector | `<cometchat-smart-replies>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatSmartRepliesComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `user` · `group` · `message` · `keywords` · `delayDuration` |
| Key outputs | `replyClick` · `closeClick` |
| View slots | — |
