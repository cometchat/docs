...
| Selector | `<cometchat-conversation-item>` — the class name is NOT a valid tag |
| Standalone | Standalone — add `CometChatConversationItemComponent` to the consuming component's `imports: []` or it renders **nothing**, silently |
| Key inputs | `conversation` · `isActive` · `isSelected` · `isFocused` · `tabIndex` · `typingIndicator` · `loggedInUser` · `dateFormat` |
| Key outputs | `itemClick` · `itemSelect` · `avatarClick` · `titleClick` · `subtitleClick` · `timestampClick` · `badgeClick` · `contextMenuOpen` · `contextMenuOptionClick` · `contextMenuKeyboardOpen` |
| View slots | `leadingView` · `titleView` · `subtitleView` · `trailingView` — override via `<ng-template>` |
