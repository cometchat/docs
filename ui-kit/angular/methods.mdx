...
CometChatUIKit.logout()
  .then(() => {
    // Session ended and local state cleared.
  })
  .catch((error) => {
    // See the warning below — handle this, do not swallow it.
    console.error('CometChat logout failed', error);
  });
```

<Warning>
**Always handle a rejected `logout()`.** Local cleanup — detaching listeners, clearing the
cached user, emitting `null` on `loggedInUser$` — runs only when the underlying SDK call
**succeeds**. If it rejects (for example with `USER_NOT_LOGED_IN`), the UI Kit still reports
the previous user from `getLoggedinUser()`, and `initFromSettings()` rehydrates that session
on the next reload — so the sign-out silently does not stick.

Until this is addressed in the kit, treat a failed logout as a hard failure in your own code:
block the redirect, surface the error, and offer a retry. If you must proceed anyway, clear
your own session state and force a full page reload so nothing is rehydrated from memory.
</Warning>
