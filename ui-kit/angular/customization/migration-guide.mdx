...
v5 changes the dependency set. Install the new required peer **before** upgrading the kit —
`@cometchat/cards-angular` is new in v5, and `npm install` fails on a peer-dependency error
if it is missing.

```sh
# Required in v5
npm install @cometchat/chat-uikit-angular @cometchat/cards-angular

# Removed in v5 — these v4 packages no longer exist
npm uninstall @cometchat/uikit-elements @cometchat/uikit-resources @cometchat/uikit-shared
```

| Package | v4 | v5 |
| ------- | -- | -- |
| `@cometchat/chat-uikit-angular` | Required | Required |
| `@cometchat/chat-sdk-javascript` | Required | Required |
| `@cometchat/cards-angular` | — | **Required (new)** |
| `dompurify` | — | Required |
| `@cometchat/calls-sdk-javascript` | Optional | Optional (calling only) |
| `@cometchat/uikit-elements` | Required | Removed |
| `@cometchat/uikit-resources` | Required | Removed |
| `@cometchat/uikit-shared` | Required | Removed |
...
Do **not** hand-roll a `matchMedia` listener when you migrate. v5 ships `ThemeService`,
which already reads `prefers-color-scheme` on startup via `initFromPreference()`, keeps
following the OS while the app is open, and writes `data-theme` on `<html>` through
Angular's `DOCUMENT` token (so it is SSR-safe). Adding your own listener creates a second
theme source that drifts out of sync with `ThemeService.currentTheme()`.
...
export class AppComponent {
  ...
  // Nothing to wire up: ThemeService applies the OS theme in its own
  // constructor and keeps following it. Call setTheme('dark' | 'light')
  // or toggleTheme() only when you expose a manual switch.
  toggleTheme(): void {
...
<Warning>
`ThemeService` does **not** persist a manual choice — `setTheme()` and `toggleTheme()`
do not write to `localStorage`, so after a reload the app returns to the OS theme. If
your product needs a sticky preference, persist it yourself and re-apply it after the
service initialises. See [Theming](/ui-kit/angular/customization/theming).
</Warning>
...
- [ ] Update packages: `npm install @cometchat/chat-uikit-angular @cometchat/cards-angular` (`@cometchat/cards-angular` is a new required peer in v5)
...
- [ ] Import CSS variables in global styles: `@import 'node_modules/@cometchat/chat-uikit-angular/styles/css-variables.css'` (the package-specifier form does not resolve)
