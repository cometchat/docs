...
| Angular | v17, v18, v19, v20, v21 — matches the published peer range `>=17.0.0 <22.0.0`. Angular 22 is not yet installable; see below. |
...
<Note>
**Angular version support.** The kit's published peer range is
`@angular/core >=17.0.0 <22.0.0`, so v17 through v21 install cleanly. Angular 17 is within
the range and works, though it is not part of our regular test matrix.

**Angular 22 is not installable yet.** `npm install` fails with `ERESOLVE` against the
current peer range. A range widened to `<23.0.0` is merged on the kit side but has not been
published; this page will list v22 as supported in the release that ships it.
</Note>
