...
<Note>
  A conversation pin is private and multi-device: it rearranges only that user's list, so these arrive on their own devices and nowhere else. `CometChatConversations` re-orders itself without them — subscribe when you keep a list of your own.
</Note>
