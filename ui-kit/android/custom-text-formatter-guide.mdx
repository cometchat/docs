...
// V6: build the list yourself. The composer auto-adds a default CometChatMentionsFormatter,
// and setTextFormatters REPLACES the list — include the mentions formatter to keep @mentions.
messageComposer.setTextFormatters(
    listOf(CometChatMentionsFormatter(this), HashTagFormatter())
)
```
</Tab>
<Tab title="Jetpack Compose">
...
val textFormatters = listOf(CometChatMentionsFormatter(context), HashTagFormatter())
