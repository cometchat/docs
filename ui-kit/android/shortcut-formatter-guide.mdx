...
// V6: build the list yourself — setTextFormatters REPLACES the default list, so include the
// mentions formatter if you want @mentions to keep working.
cometChatMessageComposer.setTextFormatters(
    listOf(CometChatMentionsFormatter(this), ShortCutFormatter())
)
```
</Tab>
...
// V6: build the list yourself — setTextFormatters REPLACES the default list.
val textFormatters = listOf(CometChatMentionsFormatter(context), ShortCutFormatter())
