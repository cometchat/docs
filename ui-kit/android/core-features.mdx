...
<Info>
**Secure Media Access:** CometChat uses a File Access Token (FAT) to authenticate all media requests, ensuring only authorized users can access images, videos, audio files, and documents. The UI Kit handles this automatically for all built-in components like `MessageList`, `ImageBubble`, `VideoBubble`, and `FileBubble`. If you're building custom media handling—such as displaying media in a custom view or integrating with third-party image libraries like Glide or Coil—you can configure header-based token transmission for enhanced security. See [Secure Media Access](/ui-kit/android/methods#secure-media-access) for implementation details.
</Info>
