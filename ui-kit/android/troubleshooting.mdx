...
| Wrong import for `CometChatUIKit` | Using old package path | Use `import com.cometchat.uikit.core.CometChatUIKit` for both `chatuikit-kotlin` and `chatuikit-compose` modules |
...
| Compose theme not applying (`chatuikit-compose`) | Not wrapping in `CometChatTheme` composable | In `chatuikit-compose`, wrap your CometChat composables inside `CometChatTheme { }` to apply theming |
...
| Sticker button disappears after setting auxiliary view | Auxiliary button overridden | Retrieve default auxiliary buttons via `CometChatUIKit.getDataSource().getAuxiliaryOption()` and include them in your custom layout before calling `setAuxiliaryButtonViewListener()` |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Event not received | Not collecting from the correct `CometChatEvents` SharedFlow | Check the [Events](/ui-kit/android/events) page for exact SharedFlow names and collect in a coroutine scope |
