...
lifecycleScope.launch {
    CometChatEvents.messageEvents.collect { event ->
        when (event) {
            is CometChatMessageEvent.MessageSent -> { /* Handle message sent */ }
            is CometChatMessageEvent.MessageEdited -> { /* Handle message edited */ }
            is CometChatMessageEvent.MessageDeleted -> { /* Handle message deleted */ }
        }
    }
}
...
composer.setOnSendButtonClick { text ->
    // Handle send — `text` is the composed message text
}
```
...
// Implement the repository interface (com.cometchat.uikit.core.domain.repository)
class MyConversationRepository : ConversationListRepository {
    override suspend fun getConversations(/* see the interface for parameters */): List<Conversation> {
        // Your custom data source
    }
    // also: deleteConversation(...), markAsDelivered(...), hasMoreConversations()
}

// Inject the repository through the factory, then hand the ViewModel to the component
val factory = CometChatConversationsViewModelFactory(
    repository = MyConversationRepository()
)
val viewModel = ViewModelProvider(this, factory)[CometChatConversationsViewModel::class.java]
