...
<Note>
Thread subscription is **enabled by default from v6.1.0**. On **v6.0.x** it is opt-in: nothing on this page renders until you pass `UIKitSettings.setEnableThreadSubscription(true)` to `CometChatUIKit.init()`. That setter is deprecated in v6.1.0 but still works, and is the only app-wide way to turn the feature off — see [setEnableThreadSubscription](/ui-kit/android/methods#init).
</Note>

Thread subscription gives users Slack-style control over thread noise: they can **subscribe** to a thread to be notified about its replies, or **unsubscribe** from one to mute it. The **server** also subscribes users automatically when they send a message (to that message's own thread), reply in a thread, or are @-mentioned in a reply — these happen server-side, so they apply to any app on the SDK, and the UI Kit reflects them as soon as they occur. Subscribing explicitly is how users opt in to a conversation they haven't participated in yet.
