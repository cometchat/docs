...
<Warning>
**Requires the Calls SDK AND calling to be initialized — this component CRASHES otherwise.**
`CometChatCallLogs` builds a `CallLogRequest` in its ViewModel's `init {}`, with **no availability
check**, so simply placing it on a screen is fatal unless both conditions hold. Verified on device:

| Calls artifact | `uiKit.enableCalling` | Result |
|---|---|---|
| absent | anything | `java.lang.NoClassDefFoundError: …CallLogRequest$CallLogRequestBuilder` |
| present | `false` | `java.lang.RuntimeException: Please call the CometChatCalls.init() method …` |
| present | `true` | renders (empty/error state when there are no logs) |
...
```
```json
// app/src/main/assets/cometchat-settings.json — this is what makes the UI Kit init CometChatCalls
{ "uiKit": { "enableCalling": true } }
```

The component **compiles fine** in every case — the failure is runtime-only, so a build cannot warn
you. The same applies to the other calling components.
</Warning>
...
CometChatCallLogs(
    style = CometChatCallLogsStyle.default().copy(
        ...
        itemStyle = CometChatCallLogsListItemStyle.default().copy(
