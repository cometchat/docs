...
|------|-------------|-------------|
| `CometChatEvents.messageEvents` | `CometChatMessageEvent` | Message sent, edited, deleted, read, reactions |
...
| `CometChatEvents.threadEvents` | `CometChatThreadEvent` | Thread subscription state changes |

## API reference

### Message Events

`CometChatEvents.messageEvents` emits `CometChatMessageEvent` sealed class instances when messages are sent, edited, deleted, or read.

**Event types:**

| Event | Description |
| ----- | ----------- |
| `CometChatMessageEvent.MessageSent(message, status)` | Triggered when a message is sent. Status can be `inProgress` or `sent`. |
| `CometChatMessageEvent.MessageEdited(message, status)` | Triggered when a message is edited. Status can be `inProgress` or `sent`. |
| `CometChatMessageEvent.MessageDeleted(message)` | Triggered when a message is deleted. |
| `CometChatMessageEvent.MessageRead(message)` | Triggered when a message is read. |
| `CometChatMessageEvent.LiveReaction(icon)` | Triggered when a live reaction is sent. |
| `CometChatMessageEvent.InteractionGoalCompleted(message)` | Triggered when an interaction goal is completed. |
| `CometChatMessageEvent.MessagePinned(message)` | Triggered when a message is pinned. |
| `CometChatMessageEvent.MessageUnpinned(message)` | Triggered when a message is unpinned. |
| `CometChatMessageEvent.MessageSaved(message)` | Triggered when the logged-in user saves a message. |
| `CometChatMessageEvent.MessageUnsaved(message)` | Triggered when the logged-in user unsaves a message. |
...
lifecycleScope.launch {
    CometChatEvents.messageEvents.collect { event ->
        when (event) {
            ...
            is CometChatMessageEvent.LiveReaction -> {
                val icon = event.icon
                // Show live reaction animation
            }
            is CometChatMessageEvent.InteractionGoalCompleted -> { /* Handle goal completion */ }
            else -> { /* Other events: MessagePinned, MessageSaved, ReactionAdded, TypingStarted, etc. */ }
        }
    }
}
```
...
| Event | Description |
| ----- | ----------- |
| `CometChatThreadEvent.SubscriptionChanged(parentMessageId: Long, subscribed: Boolean)` | Triggered when the logged-in user's subscription state for a thread changes. |
...
lifecycleScope.launch {
    CometChatEvents.threadEvents.collect { event ->
        when (event) {
            is CometChatThreadEvent.SubscriptionChanged -> {
                // Update your subscription control for event.parentMessageId
                // and stamp the flag onto the message object(s) you hold:
...
| Event | Description |
| ----- | ----------- |
| `CometChatUIEvent.ShowPanel(id, alignment, view)` | Triggered to show an additional UI panel with custom elements. |
| `CometChatUIEvent.HidePanel(id, alignment)` | Triggered to hide a previously shown UI panel. |
| `CometChatUIEvent.ActiveChatChanged(id, message, user, group)` | Triggered when the active chat changes. |
| `CometChatUIEvent.OpenChat(user, group)` | Triggered to open a chat with a specific user or group. |
...
lifecycleScope.launch {
    CometChatEvents.uiEvents.collect { event ->
        when (event) {
            ...
            is CometChatUIEvent.CardActionClicked -> {
                val message = event.message            // CardMessage or AIAssistantMessage
                val action = event.actionEvent as CometChatCardActionEvent
                // action.action, action.elementId, action.cardJson
                // Handle the card / agent-card action tap
            }
            else -> { /* no-op */ }
        }
    }
...
fun UIEventsHandler() {
    LaunchedEffect(Unit) {
        CometChatEvents.uiEvents.collect { event ->
            when (event) {
                is CometChatUIEvent.ShowPanel -> { /* Show custom UI panel */ }
                is CometChatUIEvent.HidePanel -> { /* Hide custom UI panel */ }
                is CometChatUIEvent.ActiveChatChanged -> { /* React to active chat change */ }
                is CometChatUIEvent.OpenChat -> { /* Navigate to chat */ }
                is CometChatUIEvent.CardActionClicked -> { /* Handle the card agent-card action tap */ }
                else -> { /* no-op */ }
            }
        }
    }
