...
| Event | Description |
| ----- | ----------- |
| `CometChatThreadEvent.SubscriptionChanged(parentMessageId, subscriptionState, source)` | Triggered when the user's subscription state for a thread changes. `subscriptionState` is a `ThreadSubscriptionState` (`SUBSCRIBED` / `NOT_SUBSCRIBED` / `UNKNOWN`). |
