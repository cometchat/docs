...
```kotlin lines
messageComposer.setOnSendButtonClick { text ->
    // Custom send logic — `text` is the composed message text
}
```
...
This is composer-side only: received multi-attachment messages still render with the grouped bubbles (see [Message List](/ui-kit/android/message-list)).
...
The individual staged-tile visuals and the per-type message bubbles are covered in [Message Bubble Styling](/ui-kit/android/message-bubble-styling).
...
<Note>
If you override the auxiliary button with `setAuxiliaryButtonViewListener()`, your view REPLACES the kit's default auxiliary buttons (stickers / AI / extensions). V6 exposes no API to retrieve and re-add those defaults — `CometChatUIKit.getDataSource()` existed in V5 and was removed — so override this slot only when you want to own it completely.
</Note>
...
```kotlin lines
// The auxiliary slot takes a ViewHolder LISTENER, not a View — the kit calls
// createView() with the current chat target so the slot can react to it.
messageComposer.setAuxiliaryButtonViewListener(object : MessageComposerViewHolderListener {
    override fun createView(context: Context, user: User?, group: Group?): View {
        val linearLayout = LinearLayout(context)
        linearLayout.orientation = LinearLayout.HORIZONTAL

        // NOTE: this REPLACES the auxiliary slot — the kit's default auxiliary buttons
        // (stickers / AI / extensions) are not rendered while an override is set. There is no
        // V6 API to fetch them (`CometChatUIKit.getDataSource()` was removed after V5), so
        // override this slot only when you intend to own it entirely.

        // Add your custom button
        val customButton = ImageView(context)
...
### Rich-Text Toolbar Trailing Buttons

Append your own buttons at the trailing end of the rich-text formatting toolbar — for snippet inserters, template pickers, AI actions, or custom styling. The UI Kit renders its own divider between the built-in formatting buttons and your content; your buttons join the toolbar's horizontal scroll and follow RTL automatically.

Your button receives a **`ComposerInputController`** — a live handle to read and mutate the composer input:

| Member | Description |
| --- | --- |
| `text: String` | Current plain text of the input. |
| `selection: IntRange` | Current selection; `start == end` means a collapsed caret. |
...
| `replaceSelection(replacement)` | Replace the selection; degrades to insert when the caret is collapsed. |
    ...
    object : RichTextToolbarTrailingViewListener {
        override fun createView(
            ...
            return ImageButton(context).apply {
                setImageResource(R.drawable.ic_snippet)
                setOnClickListener {
                    input.insertAtCursor("Thanks for reaching out! ")
                }
            }
        }
    }
...
CometChatMessageComposer(
    ...
    trailingToolbarContent = { input ->
        IconButton(onClick = {
            ...
            Icon(painterResource(R.drawable.ic_snippet), contentDescription = "Snippet")
        }
        IconButton(onClick = {
            input.toggleFormat(RichTextFormat.BOLD)
        }) {
            Icon(painterResource(R.drawable.ic_bold), contentDescription = "Bold")
        }
    }
)
```

The slot is a `RowScope` lambda, so you can emit multiple buttons.
...
The trailing section lives **inside** the rich-text toolbar — it is not rendered when the toolbar is hidden or the rich-text editor is disabled. The `ComposerInputController` is live only while the composer is mounted; don't retain it beyond your button's lifecycle.
...
<CardGroup cols={2}>
  ...
  <Card title="Message Template" icon="puzzle-piece" href="/ui-kit/android/message-template">
    Customize message bubble structure
  </Card>
  <Card title="Component Styling" icon="paintbrush" href="/ui-kit/android/component-styling">
