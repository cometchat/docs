...
| Prerequisite | Complete [Kotlin Integration](/ui-kit/android/getting-started-kotlin) or [Jetpack Compose Integration](/ui-kit/android/getting-started-jetpack) Steps 1–3 first |
...
<Warning>
**`enableEdgeToEdge()` alone is not enough.** It draws your layout edge-to-edge, so the UI Kit
toolbar renders **under the status bar** and the title collides with the clock. Give the layout root
an `android:id` and consume the insets (add `Type.ime()` on the message screen so the composer rides
above the keyboard):
