...
The header renders a [thread subscription](/ui-kit/android/guide-threaded-messages#thread-subscription) bell as a trailing control on the reply-count bar. It flips optimistically on tap, reverts with a toast on failure, and stays in sync with the message list's Subscribe/Unsubscribe option automatically.
