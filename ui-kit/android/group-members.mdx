...
## Member Permissions (who can act on whom)

`CometChatGroupMembers` **enforces a permission matrix before it shows the long-press menu**. If the
logged-in user is not allowed to act on the tapped member, no menu appears at all — this is built in,
not something you wire.

| Logged-in user is… | Can open the menu on… | Cannot act on |
| --- | --- | --- |
| **Owner** (`loggedInUser.uid == group.owner`) | any member except themselves | — |
| **Admin** (`group.scope == "admin"`) | participants and moderators | other admins, the owner |
| **Moderator** (`group.scope == "moderator"`) | participants only | moderators, admins, the owner |
| **Participant** | nobody — the menu never opens | everyone |

Additionally, in the scope-change sheet a **moderator cannot assign the `admin` role** — that option is
rendered disabled.

Scope values come from `CometChatConstants`: `SCOPE_ADMIN` (`"admin"`), `SCOPE_MODERATOR`
(`"moderator"`), `SCOPE_PARTICIPANT` (`"participant"`).

<Warning>
**Parameter order differs between the two ownership APIs** — a common source of silent bugs:

```kotlin
CometChat.updateGroupMemberScope(UID, GUID, scope, listener)   // UID first
CometChat.transferGroupOwnership(GUID, UID, listener)          // GUID first
```
</Warning>
...
// Get selected
val selected = groupMembers.getSelectedMembers()

// Or observe selection as it changes
groupMembers.setOnSelection { members -> /* members: List<GroupMember> */ }
...
See [ViewModel & Data](/ui-kit/android/customization-viewmodel-data) for ListOperations, state observation, and custom repositories.
...
<CardGroup cols={2}>
  ...
  <Card title="ViewModel & Data" icon="database" href="/ui-kit/android/customization-viewmodel-data">
