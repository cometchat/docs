...
When your app handles media files (images, videos, audio, documents), CometChat uses a File Access Token (FAT) to authenticate requests. This ensures only authorized users can access media content.

The UI Kit automatically handles secure media loading for all built-in components. No additional configuration is required when using components like `MessageList`, `ImageBubble`, `VideoBubble`, and `FileBubble`.

| Mode | Description |
| ---- | ----------- |
| `EMBEDDED` | Token is appended to the URL as a query parameter (`?fat=xxx`). This is the default. |
| `HEADER_BASED` | Token is sent via HTTP headers, preventing exposure in server logs. More secure for production. |

<Note>
If you're building custom media handling outside the UI Kit components, you'll need to handle secure media access yourself. See [Secure Media Access](/ui-kit/android/methods#secure-media-access) for implementation details.
</Note>
