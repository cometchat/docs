...
| `HEADER_BASED` | Token is sent via HTTP headers instead of the URL. This prevents token exposure in server logs, browser history, and network monitoring tools. More secure for production apps. |
...
CometChat.init(this, "YOUR_APP_ID", appSettings, object : CometChat.CallbackListener<String>() {
    override fun onSuccess(successMessage: String?) {
        CometChatUIKit.init(this@MainActivity, uiKitSettings, object : CometChat.CallbackListener<String>() {
            override fun onSuccess(s: String?) {
                Log.d(TAG, "CometChat UI Kit initialized with secure media")
            }
    ...
    override fun onError(e: CometChatException?) {
        Log.e(TAG, "CometChat SDK init failed: ${e?.message}")
    }
})
```
</Tab>
...
CometChat.init(this, "YOUR_APP_ID", appSettings, new CometChat.CallbackListener<String>() {
    ...
    public void onSuccess(String successMessage) {
        CometChatUIKit.init(MainActivity.this, uiKitSettings, new CometChat.CallbackListener<String>() {
            @Override
            public void onSuccess(String s) {
                Log.d(TAG, "CometChat UI Kit initialized with secure media");
            }
    ...
    @Override
    public void onError(CometChatException e) {
        Log.e(TAG, "CometChat SDK init failed: " + e.getMessage());
    }
});
```
</Tab>
...
**Option 1: Header-Based Requests (for OkHttp, MediaPlayer)**

Use `SecureMediaHelper.createSecureOkHttpClientBuilder()` to create an OkHttpClient with the FAT header injected:
...
**Option 2: Presigned URLs (for Glide, Coil, Picasso)**

Use `CometChat.fetchPresignedUrl()` to get a temporary URL that works with third-party libraries:
...
| Using OkHttp directly | `SecureMediaHelper.createSecureOkHttpClientBuilder()` |
| Using Glide, Coil, Picasso | `CometChat.fetchPresignedUrl()` |
| Playing video/audio with MediaPlayer | Use `SecureMediaHelper.getSecureHeaders()` |
