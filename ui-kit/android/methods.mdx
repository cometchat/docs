...
| **setEnableThreadSubscription**      | `Boolean`                     | *Deprecated in v6.1.0.* Global switch for the thread subscription feature. Default `true` from v6.1.0 (`false` on v6.0.x, where it is the opt-in). Kept as the only app-wide opt-out — pass `false` to turn the feature off everywhere. To hide a single surface, prefer the per-screen visibility methods. See [Thread Subscription](/ui-kit/android/guide-threaded-messages#thread-subscription) |
...
Synchronous, UI-safe checks for whether a feature is available. Use them to gate custom entry points; the built-in components already check them internally.

| Method | Description |
| --- | --- |
| `CometChatUIKit.isPinMessageEnabled()` | Whether the Pin Message feature is enabled for the app. |
| `CometChatUIKit.isSaveMessageEnabled()` | Whether the Save Message feature is enabled for the app. |
| `CometChatUIKit.isPinConversationEnabled()` | Whether the Pin Conversation feature is enabled for the app. |
| `CometChatUIKit.isThreadSubscriptionEnabled()` | Whether thread subscription is enabled for the app. `true` by default from v6.1.0; on v6.0.x it reports whether `UIKitSettings.setEnableThreadSubscription(true)` was passed. |

```kotlin
if (CometChatUIKit.isPinMessageEnabled()) {
    // show your "Pinned messages" entry point
}
```
