...
description: "Troubleshoot CometChat Android UI Kit setup, initialization, login, theming, permissions, messaging, calling, and build issues."
---
...
| Blank screen after login | Component rendered before `init()` or `login()` completes | Ensure `init()` → `login()` order completes before rendering any UI Kit component. See [Methods](/ui-kit/android/v5/methods) |
...
| Auth Key exposed in production | Using Auth Key instead of Auth Token | Switch to [`loginWithAuthToken()`](/ui-kit/android/v5/methods#login-using-auth-token) for production. Generate tokens server-side via the REST API |
...
| Custom colors not applying | Overriding wrong theme attributes | Check the [Color Resources](/ui-kit/android/v5/color-resources) page for the correct attribute names |
...
| Bubble style not applying | Custom style linked to wrong hub | Confirm the style is linked inside the correct hub (`CustomIncomingMessageBubbleStyle` or `CustomOutgoingMessageBubbleStyle`). See [Message Bubble Styling](/ui-kit/android/v5/message-bubble-styling) |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Call buttons not appearing | `com.cometchat:calls-sdk-android` not installed | Add the calls SDK dependency to your `build.gradle` — the UI Kit auto-detects it. See [Integration](/ui-kit/android/v5/getting-started) |
...
| Call fails immediately | Calls SDK version mismatch | Ensure `com.cometchat:calls-sdk-android` version is compatible with your UI Kit version. Check the [Integration](/ui-kit/android/v5/getting-started) page for version pairing |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| UI text not translated | Wrong language code | Use a `Language.Code` constant from the supported languages list in [Localize](/ui-kit/android/v5/localize) |
...
| String overrides not appearing | Key name mismatch in `strings.xml` | Verify the key name matches exactly (e.g., `cometchat_chats`). See [Localize](/ui-kit/android/v5/localize) |
...
| Date/time labels unchanged | `DateTimeFormatterCallback` not set | Add `setDateTimeFormatterCallback(...)` to the same `UIKitSettings` instance passed to `CometChatUIKit.init()`. See [Methods](/ui-kit/android/v5/methods#dateformatter) |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| No sound plays | `CometChatSoundManager` created with invalid `Context` | Ensure you pass a valid `Context` when instantiating `CometChatSoundManager`. See [Sound Manager](/ui-kit/android/v5/sound-manager) |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Event listener not firing | Subscribed to wrong event class or method | Check the [Events](/ui-kit/android/v5/events) page for exact event class names and callback methods |
...
| Custom message not appearing in Message List | No `MessageTemplate` registered | Custom messages and custom interactive messages require a registered [MessageTemplate](/ui-kit/android/v5/message-template) that defines how to render them |
