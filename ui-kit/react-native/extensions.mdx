...
| Enablement | **Dashboard first** — enable the extension in the CometChat dashboard, then wire any UI. Code alone silently does nothing |
| Auto-rendered | Most extensions render inside `CometChatMessageList` once enabled (e.g. polls, stickers, link preview) — **emit no client code for those**, or you get duplicate UI |
| Needs client code | Only where a component is documented for it (e.g. `CometChatCreatePoll`) |
| NOT extensions in v5 | **Reactions** and **mentions** are core messaging — the legacy dashboard extensions for them are deprecated |
| Related | [Core](/ui-kit/react-native/core-features) · [Message List](/ui-kit/react-native/message-list) |
