...
| Purpose | Visual representation for outgoing voice and video calls with call status and end call controls. |
| Data props | `call` |
| Primary output | `onEndCallButtonPressed(call)` — user cancelled the outgoing call |
| Other actions | `onError` — [details](#actions-and-events) |
| View slots | `TitleView` · `SubtitleView` · `AvatarView` · `EndCallView` — [details](#custom-view-slots) |
| UI events | `ccCallEnded` · `ccCallFailed` |
