...
| Purpose | Scrollable, searchable list of members in a specific group with roles, scopes, online status, and member management actions. |
| Data props | `group` · `groupMemberRequestBuilder` · `searchRequestBuilder` · `searchKeyword` |
| Primary output | `onItemPress(member)` — opens the member's profile/actions |
| Other actions | `onItemLongPress` · `onBack` · `onSelection` · `onSubmit` · `onError` · `onEmpty` · `onLoad` — [details](#actions-and-events) |
| View slots | `ItemView` · `LeadingView` · `TitleView` · `SubtitleView` · `TrailingView` · `LoadingView` · +3 more — [details](#custom-view-slots) |
| UI events | `ccGroupMemberKicked` · `ccGroupMemberBanned` · `ccGroupMemberScopeChanged` |
| Styling | `style` prop — [tokens and overrides](#styling) |
| Prerequisites | `CometChatUIKit.init()` completed and a user logged in |
| Stitching | Fetch group -> pass to GroupMembers, MessageHeader, MessageList, MessageComposer (with `CometChatMessageHeader` · `CometChatMessageList` · `CometChatMessageComposer`) |
...
<Warning>
  **This signature takes no argument, unlike the other list components.** `CometChatUsers`,
  `CometChatGroups` and `CometChatConversations` all declare
  `onError?: (e: CometChat.CometChatException) => void`, and so does `CometChatGroupMembers` in the
  **React** UI Kit — this component on React Native is the exception.

  The error object **is** delivered at runtime: the callback is forwarded to the shared internal list,
  which invokes it as `onError(error)`. Only the public type omits the parameter, so TypeScript rejects
  `onError={(e) => …}` with `TS2322` for a value that is actually there.

  If you need the error today, cast at the call site:
  ...
  Otherwise handle the failure visually with the [ErrorView](#errorview) slot, which needs no error object.
</Warning>
