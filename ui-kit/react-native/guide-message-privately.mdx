...
| Event | `CometChatUIEvents.openChat` emitted by the UIKit |
...
2. It emits a `CometChatUIEvents.openChat` event with the user object
3. Your app listens for this event and navigates to a private chat screen
...
### 1. Listen for the openChat Event

In your group chat screen, register a `CometChatUIEvents.openChat` listener. When it fires, navigate to a private chat screen with the user.
...
const LISTENER_ID = 'message_privately_listener';

export default function GroupChatScreen({ group }: { group: CometChat.Group }) {
  const navigation = useNavigation<any>();

  useEffect(() => {
    CometChatUIEventHandler.addUIEventListener(LISTENER_ID, {
    ...
    return () => {
      CometChatUIEventHandler.removeUIEventListener(LISTENER_ID);
    };
  }, [navigation]);
...
### 2. Group Chat Screen with Message List

Render the full group chat. The "Message Privately" option is available by default on every message in a group conversation.
...
| Event emitted | `CometChatUIEvents.openChat` with `{ user: CometChat.User }` |
| Listen for event | `CometChatUIEventHandler.addUIEventListener` |
