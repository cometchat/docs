...
| Purpose | Voice and video call buttons that initiate calls to users or groups. |
| Data props | `user` · `group` |
| Primary output | Emits the `ccOutgoingCall` UI event — there is no success callback, only `onError` |
| UI events | `ccCallRejected` · `ccCallFailed` · `ccOutgoingCall` |
...
| Stitching | Button press initiates call → OutgoingCall screen displayed (with `CometChatOutgoingCall`) |
