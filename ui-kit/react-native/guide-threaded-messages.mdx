  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <CometChatMessageList
        user={user}
        group={group}
        onThreadRepliesPress={handleThreadRepliesPress}
      />
...
### Turning the feature off app-wide

The two props above hide one surface each. To switch the feature off **everywhere** in one call, use
the exported `ThreadSubscriptionConfig`:

```tsx lines
import { ThreadSubscriptionConfig } from "@cometchat/chat-uikit-react-native";

// Before rendering any chat UI, e.g. right after CometChatUIKit.init()
ThreadSubscriptionConfig.setEnabled(false);
```

It is **on** by default, matching the React UI Kit. The gate and the per-surface props are ANDed — the
bell checks `!!parentMessage && ThreadSubscriptionConfig.isEnabled() && threadSubscriptionVisibility` —
so a control appears only when the gate is on *and* its own prop does not hide it.
`ThreadSubscriptionConfig.isEnabled()` reads the current value.
