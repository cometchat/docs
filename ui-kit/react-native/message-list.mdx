...
Each message in the batch is single-kind, so it renders the matching plural bubble: [Images Bubble](/ui-kit/react-native/images-bubble), [Videos Bubble](/ui-kit/react-native/videos-bubble), [Audios Bubble](/ui-kit/react-native/audios-bubble), or [Files Bubble](/ui-kit/react-native/files-bubble). Recorded voice notes are never batched with files — they render their own [Voice Note Bubble](/ui-kit/react-native/voice-note-bubble).
