...
| `hideThreadSubscriptionOption` | `false` | Subscribe to thread / Unsubscribe from thread |
...
`hideThreadSubscriptionOption` hides the option on one list. To switch the feature off **everywhere** —
the message list and the thread header, in one call — use `ThreadSubscriptionConfig`:
...
It is **on** by default, matching React. The global switch and the per-instance props are ANDed, so
the option appears only when this is enabled *and* the list does not hide it. `ThreadSubscriptionConfig
.isEnabled()` reads the current value.
...
Hides the "Subscribe to thread" / "Unsubscribe from thread" option. Unlike pin and save, it is not gated on a Dashboard setting — the option shows by default, so this prop is how you hide it.
