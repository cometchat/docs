...
<CometChatReactions
  ...
  alignment="right"
  onReactionPress={(reaction) => console.log(reaction.getReaction())}
/>
```
...
```ts
type QuickReactionsProps = React.ComponentProps<typeof CometChatQuickReactions>;
```
</Note>
...
<CometChatReactionList
  message={message}
  selectedReaction="👍"
  onListEmpty={() => setSheetOpen(false)}
/>
```
