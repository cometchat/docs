...
| Custom message types | Needs FOUR overrides on a `DataSourceDecorator`: `getAllMessageTemplates` · `getMessageTemplate` (render) **and** `getAllMessageTypes` · `getAllMessageCategories` (fetch — the list builds `setTypes`/`setCategories` from them). Register after `login()` — [details](#filtering-messages) |
