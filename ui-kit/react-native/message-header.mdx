...
`CometChatMessageHeader` can add a **Pinned Messages** entry to its ⋮ overflow menu — despite the prop name, it is a menu item, not a separate button in the header. It is **off by default**, and needs **both** props: `showPinnedMessagesButton` to add the entry, and `onPinnedMessagesPress` to handle the tap. Setting only one does nothing.
...
| `showPinnedMessagesButton` | Adds the **Pinned Messages** item to the ⋮ overflow menu. Off by default, and only takes effect together with `onPinnedMessagesPress` | `showPinnedMessagesButton?: boolean` |
