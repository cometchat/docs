...
description: "Troubleshoot CometChat React Native UI Kit setup, initialization, login, theming, permissions, messaging, calling, and build issues."
---
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Android build fails with Maven error | `@react-native-async-storage/async-storage` resolved to v3 | Pin it to `^2.1.2` (`npm i @react-native-async-storage/async-storage@^2.1.2`) and rebuild. v3 needed a `local_repo` Maven entry; v2 does not, and v3 also breaks the Calls SDK peer range. See [Integration](/ui-kit/react-native/react-native-cli-integration#step-2-install-the-ui-kit) |
...
| WebRTC errors | Missing peer dependencies | Install `react-native-webrtc`, `react-native-background-timer`, `react-native-performance@^5.1.2`, `react-native-url-polyfill@^2.0.0` |
| `ERESOLVE` when installing the Calls SDK | `async-storage`, `react-native-performance` or `react-native-url-polyfill` resolved above the declared peer range | Pin them: `@^2.1.2`, `@^5.1.2`, `@^2.0.0` |
