---
title: "Users"
description: "Display CometChat React Native UI Kit users with avatars, online status, search, filtering, selection callbacks, and chat navigation."
---
...
| Data props | `usersRequestBuilder` · `searchRequestBuilder` · `searchKeyword` |
| Primary output | `onItemPress(user)` — opens a 1:1 chat with the user |
| Other actions | `onItemLongPress` · `onBack` · `onSelection` · `onError` · `onEmpty` · `onLoad` · `onSubmit` — [details](#actions-and-events) |
...
| Stitching | onItemPress emits CometChat.User -> pass to MessageHeader, MessageList, MessageComposer (with `CometChatMessageHeader` · `CometChatMessageList` · `CometChatMessageComposer`) |
