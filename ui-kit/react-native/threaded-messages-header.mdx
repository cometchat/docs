...
`CometChatThreadHeader` does **not** render the subscription bell. On React Native the bell belongs to [CometChatMessageHeader](/ui-kit/react-native/message-header#thread-subscription) — the landed design places it in the thread screen's top bar rather than in the reply-count row.
...
<CometChatMessageHeader user={user} group={group} parentMessage={parentMessage} />
...
See [CometChatMessageHeader → Thread Subscription](/ui-kit/react-native/message-header#thread-subscription) for the bell's props, and the [Thread Subscription guide](/ui-kit/react-native/guide-thread-subscription) for the full feature.
