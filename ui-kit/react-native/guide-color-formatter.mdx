...
const COLOR_REGEX = /\{color=(#[0-9a-fA-F]{3,6}|[a-zA-Z]+)\}([\s\S]*?)\{\/color\}/g;
...
const HEX_BY_NAME = (value: string) =>
  value.startsWith("#") ? value : NAMED[value.toLowerCase()] ?? "";
