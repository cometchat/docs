...
<Note>
**Requires `@cometchat/chat-uikit-react-native` 5.5.1 or later.** This guide is built on `formatRawText()` and `composer.replaceSelection()`, and registers `textFormatters` on Pinned, Saved, Search and Message Information — none of which exist in 5.5.0. On 5.5.0 the toolbar button does not compile and four of the five surfaces in Step 4 silently render the raw marker.
</Note>
...
<CometChatCompactMessageComposer
  user={user} // or group={group} — one or the other, never both
...
// user={user} or group={group} on the conversation-scoped components — one, never both
<CometChatMessageList user={user} textFormatters={[colorFormatter]} />
<CometChatConversations textFormatters={[colorFormatter]} />
<CometChatSearch textFormatters={[colorFormatter]} />
<CometChatPinnedMessages user={user} textFormatters={[colorFormatter]} />
