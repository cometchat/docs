...
| Data props | `conversationsRequestBuilder` · `datePattern` |
| Primary output | `onItemPress(conversation)` — opens the selected chat |
| Other actions | `onItemLongPress` · `onBack` · `onSelection` · `onError` · `onEmpty` · `onLoad` · `onSearchBarClicked` · `onSearchTextChanged` · `onSubmit` — [details](#actions-and-events) |
| View slots | `ItemView` · `LeadingView` · `TitleView` · `SubtitleView` · `TrailingView` · `LoadingView` · +4 more — [details](#custom-view-slots) |
| UI events | `ccConversationDeleted` |
| Styling | `style` prop — [tokens and overrides](#styling) |
| Prerequisites | `CometChatUIKit.init()` completed and a user logged in |
| Stitching | onItemPress emits CometChat.Conversation -> extract User/Group via getConversationWith() -> navigate to Messages screen with user or group prop (with `CometChatMessageHeader` · `CometChatMessageList` · `CometChatMessageComposer`) |
