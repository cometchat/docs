...
| Field | Value |
| --- | --- |
| Components | `CometChatMentionsFormatter`, `CometChatMessageTemplate` |
| Package | `@cometchat/chat-uikit-react-native` |
| Common renames | `ListItemView` → `ItemView`, `TailView` → `TrailingView`, `EmptyStateView` → `EmptyView`, `ErrorStateView` → `ErrorView` |
| Style changes | `[component]Style` → `style` (e.g., `conversationsStyle` → `style`), all use `DeepPartial<>` types |
| Migration guide | [Upgrading from V4](/ui-kit/react-native/upgrading-from-v4) |
