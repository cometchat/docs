...
<CometChatSavedMessages
  ...
  onItemPress={(message, source) => openConversation(message, source)}
/>
```

Props: `limit`, `onBack`, `onItemPress`, `hideUnsaveMessageOption`, `ItemView`, `title`, `style`.
Note there is no `user` or `group` prop — the list is account-wide by design.
