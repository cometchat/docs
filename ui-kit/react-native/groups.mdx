...
| Data props | `groupsRequestBuilder` · `searchRequestBuilder` · `searchKeyword` |
| Primary output | `onItemPress(group)` — opens the selected group chat |
...
| UI events | `ccGroupCreated` · `ccGroupDeleted` · `ccGroupLeft` · `ccGroupMemberScopeChanged` · `ccGroupMemberBanned` · `ccGroupMemberKicked` · `ccGroupMemberUnbanned` · `ccGroupMemberJoined` · `ccGroupMemberAdded` · `ccOwnershipChanged` |
| Styling | `style` prop — [tokens and overrides](#styling) |
| Prerequisites | `CometChatUIKit.init()` completed and a user logged in |
| Stitching | onItemPress emits CometChat.Group -> pass to MessageHeader, MessageList, MessageComposer (with `CometChatMessageHeader` · `CometChatMessageList` · `CometChatMessageComposer`) |
