...
```json
{
  ...
  "props": {
    ...
    "callbacks": {
      "onBack": "() => void — ACCEPTED BUT NEVER CALLED in 5.5.1; the screen renders no back control",
...
<Warning>
**Accepted but never called in 5.5.1.** The screen renders no back control of its own — its header is
a centred title — and nothing in the component invokes this callback. Give the screen a navigator
that provides its own back affordance, or a `BubbleView`/host wrapper that does. The prop is part of
the public type, so passing it is harmless; just do not rely on it to close the screen.
</Warning>
...
Intended for the back affordance. **Not called in 5.5.1** — see [Actions and Events](#onback).
