...
- **Native peer deps are required**, not optional: `react-native-gesture-handler`,
  `react-native-safe-area-context`, `@react-native-async-storage/async-storage`, and (for the
  action sheets/bottom sheets) `@gorhom/bottom-sheet` + `react-native-reanimated`. Install and
