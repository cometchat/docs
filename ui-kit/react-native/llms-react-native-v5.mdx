...
- **Native peer deps are required**, not optional. Verified against
  ...
  `@react-native-async-storage/async-storage@^2.1.2`, `dayjs` (+ `punycode` on Expo). Install and
...
- **Pin `@react-native-async-storage/async-storage` to `^2.1.2`.** Unpinned it resolves to v3, which
  needs a `local_repo` Maven entry in `android/build.gradle` AND is outside the range the Calls SDK
  declares — so adding calling later fails with `ERESOLVE`. The Calls SDK also needs
  `react-native-performance@^5.1.2` and `react-native-url-polyfill@^2.0.0` pinned for the same reason.
