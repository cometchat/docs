...
```json
{
  ...
  "props": {
    "data": {
      ...
      "messagesRequestBuilder": { "type": "CometChat.MessagesRequestBuilder", "default": "undefined", "note": "Base request for the fetch. The component copies it and re-asserts setPinnedOnly(true), setSavedOnly(false) and the user/group scope; the limit prop wins over the builder's own setLimit()" },
...
<Note>
**`messagesRequestBuilder` and `textFormatters` require `@cometchat/chat-uikit-react-native` 5.5.1 or later.** Neither prop exists in 5.5.0. Everything else on this page works on either version.
</Note>
...
Pass a `messagesRequestBuilder` to control which pinned messages are fetched — a search term, a
category or type filter, or the page size. Call `setPinnedOnly(true)` on the builder: it is what
scopes the request to pinned messages, and without it the request is an ordinary history read.
...
<CometChatPinnedMessages
  ...
  messagesRequestBuilder={new CometChat.MessagesRequestBuilder()
    .setPinnedOnly(true) // required — scopes the fetch to pinned messages
    .setLimit(30)}
/>
```
...
<Note>
The component works on a **copy** of your builder and re-asserts `setPinnedOnly(true)`,
`setSavedOnly(false)` and the `user` / `group` conversation scope on it, so those are safe even if
you omit them — but keep `setPinnedOnly(true)` in your code to make the intent explicit. Your own
instance is never mutated, so the same builder can also go to
[`CometChatSavedMessages`](/ui-kit/react-native/saved-messages).
</Note>
...
Base request used to fetch the pinned messages — a search term, a category or type filter, or the
...
The component copies the builder and re-asserts `setPinnedOnly(true)` on the copy, so your instance is never
...
Text formatters used when rendering the pinned rows. Pass the same array you give
