...
description: "Integrate CometChat React Native UI Kit with React Native CLI using app credentials, package setup, initialization, login, and rendering."
---
...
<Warning>
**Required before `pod install`.** The UI Kit is a Swift pod that depends on `SPTPersistentCache` and `DVAssetLoaderDelegate`, neither of which defines a module. On a static-library build — the React Native default — `pod install` fails with:
...
Add these two lines inside your target in `ios/Podfile`:
...
<Note>
If `pod install` fails with `Unicode Normalization not appropriate for ASCII-8BIT`, CocoaPods needs a UTF-8 locale. Re-run it as `LANG=en_US.UTF-8 pod install`.
</Note>
...
npm install react-native-performance@^5.1.2
npm install react-native-url-polyfill@^2.0.0
```

<Warning>
Pin `react-native-performance` and `react-native-url-polyfill`. Their latest versions (6.x and 4.x) are
outside the ranges the Calls SDK declares, so an unpinned install fails with `ERESOLVE`.
`react-native-svg` and `@react-native-async-storage/async-storage` are Calls SDK peers too — you already
