...
| --- | --- | --- | --- |
| `getConversationStarters` | `() => Promise<string[]>` | Yes | Returns the suggestions to render. |
...
<CometChatConversationStarter
  ...
  onSuggestionClicked={(text) => sendMessage(text)}
/>
```
...
| Prop | Type | Required | Description |
| --- | --- | --- | --- |
| `getSmartReplies` | `() => Promise<string[]>` | Yes | Returns the replies to render. |
...
Its SDK signature is `Promise<Object>`, but the component's `getSmartReplies` prop expects `Promise<string[]>`. Map the result with `Object.values()` or no chips render. `CometChat.getConversationStarter()` needs no such mapping — it already returns `Promise<Array<string>>`.
</Warning>
...
| Prop | Type | Required | Description |
| --- | --- | --- | --- |
| `getConversationSummary` | `() => Promise<string>` | Yes | Returns the summary text to display. |
