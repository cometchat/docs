---
title: "Core"
description: "Explore CometChat React Native UI Kit core messaging features, including media sharing, receipts, typing indicators, and more."
---
...
| Scope | Messaging capabilities that are **ON by default** in v5 — no dashboard step, no extension to enable |
| Included by default | Text · media & file attachments · **reactions** · **mentions** · typing indicators · delivery & read receipts · threaded replies · message editing & deletion |
| Where they render | `CometChatMessageList` and `CometChatMessageComposer` render these themselves |
| Turning one OFF | every one has a `hide*` prop on its component (e.g. `hideReactionOption`, `hideReplyInThreadOption`) — see [Message List](/ui-kit/react-native/message-list) |
| Common mistake | Enabling the legacy **Reactions** or **Mentions** dashboard extension. Both are already core in v5 and those extensions are scheduled for deprecation — enabling them is unnecessary |
| Related | [Message List](/ui-kit/react-native/message-list) · [Message Composer](/ui-kit/react-native/message-composer) · [Extensions](/ui-kit/react-native/extensions) |
...
| Component | Role |
| --- | --- |
| [MessageComposer](/ui-kit/react-native/message-composer#multi-attachment-staging-tray) | Multi-select picker, an attachment tray that uploads before send, and the composer text as the batch caption. |
| [MessageList](/ui-kit/react-native/message-list#multi-attachment-batch-grouping) | Renders received attachments with per-type bubbles and groups a multi-type send as one visual batch. |
...
Keep important messages in reach. Pinning highlights a message for **everyone** in the conversation; saving bookmarks it **privately** for the acting user, across all of their conversations. Both come with message options, bubble indicators, and dedicated full-screen views.

| Components | Functionality |
| --- | --- |
| [MessageList](/ui-kit/react-native/message-list) | Provides the Pin/Unpin and Save/Unsave options and shows the bubble indicators. |
| [PinnedMessages](/ui-kit/react-native/pinned-messages) | Full-screen list of a conversation's pinned messages. |
| [SavedMessages](/ui-kit/react-native/saved-messages) | Full-screen, private list of the user's saved messages. |
| [MessageHeader](/ui-kit/react-native/message-header) | Built-in pinned-messages button, via `showPinnedMessagesButton`. |

Pin and Save must be enabled for your app in the CometChat Dashboard. The UI Kit reads both at login and re-reads them on reconnection, so the options appear without any extra wiring.

See the [Pin & Save Messages guide](/ui-kit/react-native/guide-pin-and-save-messages) for end-to-end wiring.
...
| Components | Functionality |
| --- | --- |
| [Conversations](/ui-kit/react-native/conversations) | Provides the Pin/Unpin conversation option, the row indicator, and pinned-first ordering. Hide it with `pinConversationOptionVisibility`. |

Pin Conversations must be enabled for your app in the CometChat Dashboard. Ordering is server-owned — the list already arrives with pinned conversations first, so no client-side sorting is required.
...
Let users subscribe to or unsubscribe from a thread to control whether its replies notify them. Users are subscribed automatically when they send the parent message, reply in the thread, or are @mentioned in it — this feature lets them see and change that.

| Components | Functionality |
| --- | --- |
| [MessageList](/ui-kit/react-native/message-list) | Provides the Subscribe to thread / Unsubscribe from thread option in the message options. Hide it with `hideThreadSubscriptionOption`. |
...
Enabled by default, and unlike Pin & Save it has no dashboard setting. To remove it app-wide, call `ThreadSubscriptionConfig.setEnabled(false)` during initialization.

See the [Thread Subscription guide](/ui-kit/react-native/guide-thread-subscription) for setup and behavior.
