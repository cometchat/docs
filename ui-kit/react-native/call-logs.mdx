...
| Purpose | Scrollable list of call history showing missed, received, and outgoing calls. |
| Data props | `callLogRequestBuilder` · `datePattern` |
| Primary output | `onItemPress(call)` — opens the selected call log entry |
| Other actions | `onItemLongPress` · `onCallIconPress` · `onBack` · `onError` · `onLoad` · `onEmpty` — [details](#actions-and-events) |
...
| Stitching | onItemPress initiates call to participant, onCallIconPress triggers call-back (with `CometChatOutgoingCall`) |
...
Pass a `CometChatCalls.CallLogRequestBuilder` to `callLogRequestBuilder`. Pass the builder instance — not the result of `.build()`.
...
function FilteredCallLogs() {
  const callLogRequestBuilder = new CometChatCalls.CallLogRequestBuilder()
    .setLimit(20)
...
| Recipe | Code |
| --- | --- |
| Limit to 20 per page | `new CometChatCalls.CallLogRequestBuilder().setLimit(20)` |
| Only cancelled calls | `new CometChatCalls.CallLogRequestBuilder().setCallStatus("cancelled")` |
| Only incoming calls | `new CometChatCalls.CallLogRequestBuilder().setCallDirection("incoming")` |
| Only outgoing calls | `new CometChatCalls.CallLogRequestBuilder().setCallDirection("outgoing")` |
| Calls with recordings | `new CometChatCalls.CallLogRequestBuilder().setHasRecording(true)` |
| Filter by user UID | `new CometChatCalls.CallLogRequestBuilder().setUid("user_uid")` |
