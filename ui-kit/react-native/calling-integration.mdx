...
| Extra deps | `react-native-webrtc` · `react-native-background-timer` · `react-native-performance@^5.1.2` · `react-native-url-polyfill@^2.0.0` (plus `react-native-svg` and `@react-native-async-storage/async-storage@^2.1.2`, already installed with the UI Kit). **Not** `@react-native-community/netinfo` or `react-native-callstats` — those were v4 peers and are neither declared nor referenced by v5 |
