...
| Field | Value |
| --- | --- |
| Package | `@cometchat/chat-sdk-react-native` v4.x |
| Enablement | Dashboard — campaigns are configured and sent from the CometChat dashboard, not from the client |
| Client surface | `CometChatNotificationFeed` renders campaign messages in-app; `addNotificationFeedListener` receives them |
| Prerequisites | SDK initialized and a user logged in |
| Related | [Notification Feed](/ui-kit/react-native/notification-feed) · [Campaigns (SDK)](/sdk/react-native/campaigns) |
...
<Note>
Before proceeding, ensure you have completed the [UI Kit integration](/ui-kit/react-native/react-native-cli-integration) and the [Dashboard Setup](/campaigns#setup-flow) for Campaigns.
</Note>
