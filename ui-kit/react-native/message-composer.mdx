...
| Purpose | Rich text input for composing and sending text, media, attachments, mentions, voice notes, and custom messages. |
| Data props | `user` · `group` · `parentMessageId` · `initialComposertext` |
| Primary output | `onSendButtonPress(message)` — fires before the message is sent |
| Other actions | `onTextChange` · `onError` — [details](#actions-and-events) |
| View slots | `HeaderView` · `AuxiliaryButtonView` · `SendButtonView` — [details](#custom-view-slots) |
| UI events | `ccMessageSent` · `ccMessageEdited` · `ccMessageLiveReaction` |
...
1. **Pick** — choose one or more files from the attachment (`+`) menu. In [`CometChatCompactMessageComposer`](/ui-kit/react-native/compact-message-composer), images and files copied to the clipboard can also be pasted straight into the composer — they stage into the tray as tiles, just like a pick (subject to the same count and size limits).
...
4. **Send** — the staged files fan out into media messages that share a single batch, and the [message list](/ui-kit/react-native/message-list#multi-attachment-batch-grouping) renders them as one connected group of [plural bubbles](/ui-kit/react-native/message-bubble-styling#image-bubble).
...
type StagedFile = React.ComponentProps<typeof CometChatAttachmentTray>["attachments"][number];
...
function CustomTray({
  ...
  return <CometChatAttachmentTray attachments={staged} onRemove={onRemove} onRetry={onRetry} />;
}
```
