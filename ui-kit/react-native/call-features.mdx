...
| Field | Value |
| --- | --- |
| Package | `@cometchat/calls-sdk-react-native` (see [Calling Integration](/ui-kit/react-native/calling-integration)) |
| Components | `CometChatCallButtons` · `CometChatIncomingCall` · `CometChatOutgoingCall` · `CometChatOngoingCall` · `CometChatCallLogs` |
| Call buttons | `CometChatMessageHeader` renders them **by default** — do not also mount `CometChatCallButtons` there, or they double up |
| Incoming calls | Mount `CometChatIncomingCall` **once above your navigator**. Inside a single screen, calls only ring while that screen is open |
| 1:1 vs group | 1:1 rings; groups are **join**-based (there is no group-ring primitive) |
| Call logs | `CometChatCallLogs` is the **list**. Detail/history/participant views were removed in v5 — build them from `CometChatCalls.CallLogRequestBuilder` in the calls SDK |
| Testing | Requires **two real devices** — simulators capture no camera or microphone |
| Related | [Calling Integration](/ui-kit/react-native/calling-integration) · [Call Logs](/ui-kit/react-native/call-logs) |
