...
The `CometChatIncomingCall` is a [Component](/ui-kit/react-native/01-components-overview.md#components) that serves as a visual representation when the user receives an incoming call, such as a voice call or video call, providing options to answer or decline the call.
