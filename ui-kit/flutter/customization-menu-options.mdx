...
| Purpose | Customize CometChat Flutter UI Kit message options, long-press menus, composer attachments, and template-level actions. |
| Key classes | `CometChatMessageOption` · `CometChatConversations` · `CometChatMessageTemplate` · `CometChatMessageComposer` · `CometChatMessageList` |
| Related | [Message List](/ui-kit/flutter/message-list) · [Message Template](/ui-kit/flutter/message-template) |
  ...
  options: (loggedInUser, messageObject, context, group, additionalConfigurations) {
    ...
    return [
      CometChatMessageOption(
        ...
        onItemClick: (message, state) => translateMessage(message),
      ),
      ...defaults,
    ];
  },
  ...
  options: (loggedInUser, messageObject, context, group, additionalConfigurations) {
    return [
      CometChatMessageOption(
        ...
        onItemClick: (message, state) => bookmarkMessage(message),
      ),
      CometChatMessageOption(
        id: 'share',
        title: 'Share',
        icon: Icon(Icons.share, size: 24),
        onItemClick: (message, state) => shareMessage(message),
      ),
    ];
  },
)
