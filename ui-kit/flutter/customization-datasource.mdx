...
| Purpose | Customize how messages are rendered, what options appear on bubbles, and what formatters are used — all through the central MessageTemplateUtils registry. |
| Key classes | `CometChatMessageTemplate` · `CometChatMessageList` · `CometChatEmailFormatter` · `CometChatMentionsFormatter` · `CometChatPhoneNumberFormatter` |
| v5 → v6 | `CometChatUIKit.getDataSource()` no longer exists. v6 exposes `MessageTemplateUtils`; emitting the v5 registration path does not compile. |
| Related | [Message Template](/ui-kit/flutter/message-template) · [Message List](/ui-kit/flutter/message-list) |
