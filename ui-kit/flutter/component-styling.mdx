...
| Purpose | Style CometChat Flutter UI Kit components with ThemeData extensions, component-level style objects, message bubbles, and call UI. |
| Key classes | `CometChatAvatarStyle` · `CometChatOutgoingMessageBubbleStyle` · `CometChatIncomingMessageBubbleStyle` · `CometChatMessageListStyle` · `CometChatBadgeStyle` · `CometChatCollaborativeBubbleStyle` · `CometChatMessageReceiptStyle` |
| Constraints | A kit widget paints its own surface; the app `ThemeData` does not reach inside it. Pass the widget's style object (`CometChatMessageListStyle`, `CometChatConversationsStyle`, …) to change its background. |
| Related | [Theming In CometChat Flutter UI Kit V6](/ui-kit/flutter/theme-introduction) · [Customizing Message Bubbles](/ui-kit/flutter/message-bubble-styling) |
