...
| Purpose | Deliver targeted, rich notifications to users via an in-app notification feed powered by the CometChat Cards renderer. |
| Key classes | `CometChatNotificationFeed` |
| Key methods | `getNotificationFeedItem()` · `markPushNotificationClicked()` · `markPushNotificationDelivered()` |
| Related | [Notification Feed](/ui-kit/flutter/notification-feed) |
