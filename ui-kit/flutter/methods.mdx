...
| Purpose | Use CometChatUIKit methods for initialization, login, logout, users, groups, conversations, messages, calls, and session handling. |
| Key classes | `CometChatUIKit` · `CometChatAIAssistantEvents` · `CometChatSubscriptionType` · `CometChatAIAssistantEventsListener` · `CometChatCardActionEvent` · `CometChatUIEventListener` · `CometChatUIEvents` |
| Key methods | `createUser()` · `init()` · `login()` · `loginWithAuthToken()` · `logout()` · `sendCardMessage()` |
| Related | [Events](/ui-kit/flutter/events) · [Core Features](/ui-kit/flutter/core-features) |
