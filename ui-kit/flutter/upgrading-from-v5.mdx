...
| Purpose | Upgrade from CometChat Flutter UI Kit V5 to V6 with package changes, initialization updates, BLoC architecture, and migration steps. |
| Key classes | `CometChatUIKit` · `CometChatUIKitChatAIFeatures` |
| Key methods | `getDataSource()` |
| Constraints | Snippets marked *Before* are v5 and will not compile against v6 — that is deliberate. |
| Related | [MessageTemplateUtils & Data Source](/ui-kit/flutter/customization-datasource) · [Message Template](/ui-kit/flutter/message-template) |
...
// V6 — one import covers all chat widgets
import 'package:cometchat_chat_uikit/cometchat_chat_uikit.dart';
```

The separate `cometchat_uikit_shared` package is gone — everything it exported now lives in
`cometchat_chat_uikit`.

#### Calling widgets moved too

The standalone `cometchat_calls_uikit` **package** no longer exists. Call widgets now ship
inside `cometchat_chat_uikit`, behind a second barrel:

```dart
// V5 — a separate package
import 'package:cometchat_calls_uikit/cometchat_calls_uikit.dart';

// V6 — a second barrel inside the same package
import 'package:cometchat_chat_uikit/cometchat_calls_uikit.dart';
```

Remove `cometchat_calls_uikit` from your `pubspec.yaml` when you migrate.

<Warning>
In V5 the calls package re-exported the chat kit, so one import gave you both. **V6's calls
barrel does not.** It exports the Chat and Calls SDKs plus the call widgets, but not the chat
widgets — so a screen that uses both (for example `CometChatCallButtons` inside a
`CometChatMessageHeader`) needs both imports:
