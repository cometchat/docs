...
Fires after the follow/unfollow toggle **succeeds**, with the parent message id and the new subscribed state. Use it to keep your own thread list in step — an unfollow also hard-deletes the thread's row server-side, so drop it locally.

The bell renders only when `UIKitSettings.enableThreadSubscription` is `true`; it is `false` by default. Set `threadSubscriptionVisibility: false` to hide it when the header above already carries one.
...
CometChatThreadedHeader(
  parentMessage: parentMessage,
  loggedInUser: loggedInUser,
  onThreadSubscriptionChange: (parentMessageId, subscribed) {
...
| `threadSubscriptionVisibility` | `bool?` | `true` | Show the follow/unfollow bell. Requires `UIKitSettings.enableThreadSubscription`. |
| `onThreadSubscriptionChange` | `Function(int, bool)?` | `null` | Called after a successful toggle with the parent message id and the new state |
