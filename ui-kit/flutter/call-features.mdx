...
<Note>
These are the Calls SDK's own floors (`minSdkVersion 26`, `platform :ios, '15.1'` as of
...
the maximum across the dependency graph, so a lower value is a build error rather than a warning.
</Note>
