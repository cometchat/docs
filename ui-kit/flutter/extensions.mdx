...
| Purpose | Use CometChat Flutter UI Kit extensions for stickers, polls, collaborative tools, reactions, smart replies, and link previews. |
| Key classes | `CometChatCollaborativeBubble` · `CometChatCollaborativeWebView` · `CometChatCreatePoll` · `CometChatLinkPreviewBubble` · `CometChatPollsBubble` · `CometChatStickerBubble` · `CometChatStickerKeyboard` |
| Related | [Core Features](/ui-kit/flutter/core-features) |
...
- The `..extensions` and `..aiFeature` setters on `UIKitSettingsBuilder`

<Note>
`CometChatUIKitChatAIFeatures` is still exported by `cometchat_chat_uikit` and is not a
compile error, but it no longer does anything: the `..aiFeature` builder setter it fed was
removed, and `getDefaultAiFeatures()` returns an empty list. Drop the call rather than
keeping it.
</Note>
