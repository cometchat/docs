...
| Name | Type | Description |
| --- | --- | --- |
| onSelection(_:) | ([Group]?) -> Void | Called with the selected groups whenever the selection changes |
...
| hideSearch | Bool | Hides search bar |
| onSelection(_:) | ([GroupMember]?) -> Void | Calls the closure once, immediately, with the members selected at that moment. To handle a confirmed selection, set `onSelectedItemProceed` |
| set(onEmpty:) | () -> Void | Triggered when list is empty |
...
| setMenus | set(options:) | Custom menu items, as `[CometChatPopupMenu.MenuItem]`. For a fully custom trailing view use `set(trailView:)` instead. |
| setSubtitleView | set(subtitleView:) | Custom subtitle view |
