...
```json
{
  ...
  "compositionExample": {
    ...
    "components": ["CometChatGroups", "CometChatMessageHeader", "CometChatMessageList", "CometChatMessageComposer"],
    "flow": "User taps on a group → onItemClick fires → push your own chat screen with the selected group"
  },
  "types": {
    "Group": {
      "guid": "String",
...
| `subtitle` | `(Group?) -> UIView` | Member count / subtitle |
...
groups.set(trailingView: { group in
...
Add buttons to the navigation bar by assigning `rightBarButtonItem` or `leftBarButtonItem` (each an array of `UIBarButtonItem`). While groups are selected in selection mode, the kit temporarily shows its own checkmark button in their place.
...
groups.rightBarButtonItem = [button]
...
groups.statusIndicatorStyle = statusIndicatorStyle
```
...
groups.emptyStateView = emptyView          // or: groups.set(emptyView: emptyView)
```
