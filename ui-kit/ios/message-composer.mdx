...
messageComposer.setAttachmentOptions { user, group, controller in
    ...
    return [customComposerAction1, customComposerAction2]
}
```
</Tab>
</Tabs>
...
`CometChatCompactMessageComposer` is a compact message composer component with built-in rich text formatting support. It provides a streamlined input experience with an expandable text area and a formatting toolbar.
...
| previewCornerRadius | Corner radius for the edit/reply preview. | `style.previewCornerRadius = .init(cornerRadius: CometChatSpacing.Radius.r1)` |
| previewBorderColor | Border color for the edit/reply preview. | `style.previewBorderColor = CometChatTheme.borderColorHighlight` |
| previewBorderWidth | Border width for the edit/reply preview. | `style.previewBorderWidth = 0` |
...
| set(maxLines:) | Sets the maximum number of visible lines before scrolling. | `set(maxLines: 5)` |
| set(textFormatter:) | Sets custom text formatters (e.g., mentions). | `set(textFormatter: [CometChatTextFormatter])` |
| set(onSendButtonClick:) | Sets a callback for the send button click. | `set(onSendButtonClick: { message in })` |
| set(onError:) | Sets a callback for error handling. | `set(onError: { error in })` |
| set(onTextChangedListener:) | Sets a callback for text changes. | `set(onTextChangedListener: { text in })` |
| set(attachmentOptions:) | Sets custom attachment options. | `set(attachmentOptions: { user, group, controller in return [...] })` |
...
| edit(message:) | Puts the composer in edit mode for a given message. | `edit(message: BaseMessage)` |
| reply(message:) | Puts the composer in reply mode for a given message. | `reply(message: BaseMessage)` |
...
The Rich Text Formatting feature allows users to format messages with various text styles. When enabled, a formatting toolbar appears below the text input. This feature is available in the `CometChatCompactMessageComposer`.
