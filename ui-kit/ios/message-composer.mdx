...
```json
{
  ...
  "props": {
    ...
    "behavior": {
      ...
      "enableMultipleAttachments": "Bool (default: true) - multi-select picker + attachment tray; false reverts to single-attachment send",
...
```swift
composer.enableMultipleAttachments = false
```
...
| `enableMultipleAttachments` | `Bool` | `true` | Multi-select attachment picking with a preview tray; `false` reverts to the legacy single-attachment, send-immediately behavior |
