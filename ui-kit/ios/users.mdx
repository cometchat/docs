---
title: "Users"
description: "Display CometChat iOS UI Kit users with avatars, online status, search, filtering, selection callbacks, and conversation navigation."
---
...
```json
{
  ...
  "props": {
    "data": {
      "userRequestBuilder": {
        ...
        "default": "UsersRequest.UsersRequestBuilder().set(limit: 30)",
        "note": "Custom request builder for filtering users; setter is set(userRequestBuilder:)"
      },
  ...
  "compositionExample": {
    ...
    "components": ["CometChatUsers", "CometChatMessageHeader", "CometChatMessageList", "CometChatMessageComposer"],
    "flow": "User taps on a user → onItemClick fires → push your own chat screen with the selected user"
  },
  "types": {
    "User": {
      "uid": "String",
...
| `subtitle` | `(User?) -> UIView` | Status / subtitle text |
...
users.set(trailingView: { user in
...
Add buttons to the navigation bar by assigning `rightBarButtonItem` or `leftBarButtonItem` (each an array of `UIBarButtonItem`). In selection mode the kit takes over the right-hand slot for its own checkmark button as soon as a user is selected, and custom right-hand buttons do not come back.
...
users.rightBarButtonItem = [button]
...
users.statusIndicatorStyle = statusIndicatorStyle
```

### userRequestBuilder

Custom request builder for filtering users. Set with
`set(userRequestBuilder:)`, or pass one to `CometChatUsers(usersRequestBuilder:)`
at construction.
...
let users = CometChatUsers()
users.set(userRequestBuilder: friendsBuilder)
```
...
let users = CometChatUsers()
users.set(userRequestBuilder: onlineBuilder)
```
...
// build it first, then set it. set(emptyView:) is the equivalent builder form.
...
users.emptyStateView = emptyView          // or: users.set(emptyView: emptyView)
```
