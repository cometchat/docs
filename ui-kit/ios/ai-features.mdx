...
```json
{
  ...
  "features": [
    ...
    {"name": "suggestedMessages", "description": "AI-powered conversation starters and smart replies", "component": "CometChatMessageList", "enabledByDefault": false}
  ],
  "dashboardSetup": {
...
| Smart Replies | Suggests responses based on context | `CometChatMessageList` |
...
Conversation Summary then appears in the composer's AI options automatically. Conversation Starters and Smart Replies also need to be turned on in code: both are off by default on `CometChatMessageList` (`enableConversationStarters`, `enableSmartReplies`). See [Message List → AI Features](/ui-kit/ios/message-list#ai-features).
...
1. User receives a text message
2. After a short delay (10 seconds by default), the message list asks CometChat AI for replies based on the conversation
3. Smart reply suggestions appear at the bottom of the message list
4. User taps a suggestion to insert it into the composer

<Note>
Smart Replies is enabled under **AI** in the dashboard. The **Smart Replies** entry under **Extensions** is the older extension, which the v5 UI Kit does not use by default.
</Note>

### Implementation

Smart replies are off by default. Turn them on with `enableSmartReplies` on the `CometChatMessageList` inside your messages screen:
...
// Your messages screen — see /ui-kit/ios/ios-conversation
class MessagesVC: UIViewController {
    ...
    private lazy var messageListView: CometChatMessageList = {
        ...
        listView.enableSmartReplies = true           // off by default
        listView.set(smartRepliesDelayDuration: 2)   // seconds; default 10
...
class AIEnabledTabBarController: UITabBarController {
    ...
    private func setupTabs() {
        // Chats - summary works once enabled in the dashboard; starters and smart replies
        // also need their enable flags on CometChatMessageList
...
- [Message List](/ui-kit/ios/message-list) - Where conversation starters and smart replies appear
