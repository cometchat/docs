...
```json
{
  ...
  "props": {
    ...
    "visibility": {
      ...
      "hideThreadSubscriptionOption": { "type": "Bool", "default": false },
    ...
    "behavior": {
      ...
      "enableThreadSubscription": { "type": "Bool", "default": false, "note": "Offered on parent messages only, never on a reply" },
...
let messageList = CometChatMessageList()
messageList.enableThreadSubscription = true
```
...
Hides the subscribe/unsubscribe option in message actions. Independent of `enableThreadSubscription` — use this to suppress the option on one screen while the feature stays on elsewhere. Ignored while `enableThreadSubscription` is `false`, since nothing renders in that case anyway.
...
let messageList = CometChatMessageList()
messageList.hideThreadSubscriptionOption = true
```
