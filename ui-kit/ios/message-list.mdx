...
Hides the subscribe/unsubscribe option in message actions — use this to suppress the option on one screen while it stays available elsewhere. Set it on an `AdditionalConfiguration` and pass that to [CometChatMessageHeader](/ui-kit/ios/message-header).

ANDed with the feature gate: to remove thread subscription from your app entirely rather than from one screen, call `CometChatThreadSubscriptionConfig.setEnabled(false)` instead — see [Turning the Feature Off](/ui-kit/ios/guide-threaded-messages#turning-the-feature-off).
...
let configuration = AdditionalConfiguration()
configuration.hideThreadSubscriptionOption = true

let messageHeader = CometChatMessageHeader()
messageHeader.additionalConfiguration = configuration
```
