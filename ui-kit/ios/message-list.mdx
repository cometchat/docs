---
title: "Message List"
description: "Display CometChat iOS UI Kit messages with text, media, reactions, threads, real-time updates, filters, and message callbacks."
---
...
```json
{
  ...
  "props": {
    ...
    "visibility": {
      ...
      "hidePinMessageOption": { "type": "Bool", "default": false },
      "hideSaveMessageOption": { "type": "Bool", "default": false },
      "hideThreadSubscriptionOption": { "type": "Bool", "default": false, "note": "On AdditionalConfiguration — set via getAdditionalConfiguration()" },
    ...
    "behavior": {
      ...
      "enablePinMessage": { "type": "Bool", "default": false, "note": "Also requires CometChat.isPinMessageEnabled()" },
      "enableSaveMessage": { "type": "Bool", "default": false, "note": "Also requires CometChat.isSaveMessageEnabled()" },
  ...
  "compositionExample": {
    "description": "MessageList is typically used in your own chat screen alongside MessageHeader and MessageComposer",
...
class ChatViewController: UIViewController {
    ...
    private func openThreadView(for message: BaseMessage) {
        // A thread is its OWN screen, not the root conversation VC — see
        // /ui-kit/ios/guide-threaded-messages, where ThreadedMessagesVC exposes
        // `var parentMessage: BaseMessage?` as a property (there is no set(parentMessage:)).
...
Media messages that carry multiple attachments render with dedicated per-type bubbles — a media grid for images/videos, playback rows for audio files, and a connected card stack for documents (see [Message Bubble Styling](/ui-kit/ios/message-bubble-styling) for their styles).

<Frame>
  <img src="/images/ios_imagesbubble.png" alt="Message list rendering a multi-attachment media message as a media grid of images with a +N overflow badge, shown for both incoming and outgoing messages" />
</Frame>

When a single composer send was split into several messages (one per attachment type), the list **groups them visually as one batch**: the avatar and sender name appear only on the first message of the batch, the timestamp and read receipt only on the last, while reactions remain per message. Grouping is driven by the batch metadata (`batchId`, `batchIndex`, `batchSize`) the composer stamps on each message.

The behavior is on by default and can be toggled:

```swift
let messageList = CometChatMessageList()
messageList.set(enableMultipleAttachments: true)   // default
```

Setting it to `false` renders every media message with the classic single-attachment bubbles instead.
...
messageList.set(onThreadRepliesClick: { [weak self] message, template in
    ...
    // The thread gets its own VC — see /ui-kit/ios/guide-threaded-messages.
    // `parentMessage` is a property on ThreadedMessagesVC, not a setter, and it carries
    // the user/group context, so there is nothing else to pass in.
    let threadVC = ThreadedMessagesVC()
    threadVC.parentMessage = message
...
let cometChatMessages = MessagesVC()   // your own VC — see /ui-kit/ios/ios-conversation
    .set(user: user)
...
Enables the pin and unpin options in message actions. The feature must **also** be enabled for your app — the kit checks `CometChat.isPinMessageEnabled()` — and pinning is further gated by the user's scope.
...
let messageList = CometChatMessageList()
messageList.enablePinMessage = true
```
...
Enables the save and unsave options in message actions. The feature must **also** be enabled for your app — the kit checks `CometChat.isSaveMessageEnabled()`.
...
let messageList = CometChatMessageList()
messageList.enableSaveMessage = true
```
...
Hides the pin option in message actions. Independent of `enablePinMessage` — use this to suppress the option on one screen while the feature stays on elsewhere.
...
Hides the save option in message actions. Independent of `enableSaveMessage` — use this to suppress the option on one screen while the feature stays on elsewhere.
...
let messageList = CometChatMessageList()
messageList.hideSaveMessageOption = true
```
...
Hides the subscribe/unsubscribe option in message actions — use this to suppress the option on one screen while it stays available elsewhere.

Unlike the other option flags, this one lives on `AdditionalConfiguration` rather than directly on the component. Reach it through `getAdditionalConfiguration()`, which returns the list's own configuration object — the message actions read it each time they are built, so setting it at any point before the user opens them takes effect.

The option is offered on **parent messages only** — never on a reply inside a thread, because a subscription is always rooted at the thread's parent. It is not gated on reply count: subscribing to a message that has no replies yet is supported. It renders in one-on-one conversations as well as groups.

Thread subscription is enabled by default from **v5.2.0**, so this flag is what removes the option on one screen. On **v5.1.22–v5.1.23** the option appears only once the feature is switched on for the app.
...
See the [Thread Subscription guide](/ui-kit/ios/guide-threaded-messages#thread-subscription) for the full feature, including keeping your own subscription control in sync.
...
// emptyStateView is a UIView you assign — build it first.
let label = UILabel()
label.text = "Start a conversation!"
label.textAlignment = .center
label.textColor = .secondaryLabel

messageList.emptyStateView = label          // or: messageList.set(emptyView: label)
```
