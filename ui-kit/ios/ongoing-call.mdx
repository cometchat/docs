---
title: "Ongoing Call"
description: "Display CometChat iOS UI Kit ongoing calls with video, call controls, participant management, recording, screen sharing, and callbacks."
---
...
<Tab title="Swift">
```swift lines
// Use the SDK Call's `sessionID` (capital D, optional) from the initiated or received call, e.g. call.sessionID ?? ""
...
| **Session Id**   | Sets the session ID for CometChatOngoingCall. Pass the SDK `Call.sessionID` (note the capital D); it is optional, so unwrap it first. | `.set(sessionId: String)` |
