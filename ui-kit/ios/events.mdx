...
```json
{
  ...
  "eventCategories": [
    ...
    {
      "name": "cardEvents",
      "listenerClass": "CometChatCardEvents",
      "protocol": "CometChatCardEventListener",
      "events": [
        {"name": "ccCardActionClicked", "payload": "(BaseMessage, CometChatCardActionEvent)", "description": "Action element tapped inside a card bubble"}
      ]
    }
  ]
}
...
class MyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        CometChatCardEvents.addListener(listenerId, self)
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        CometChatCardEvents.removeListener(listenerId)
    }
}
```
...
Listen for thread subscription changes so any subscription control you host stays in agreement with the kit's action-sheet option, without a refetch.
...
class ThreadedMessagesVC: UIViewController {
    ...
    // listener — so use a distinct id per screen.
    private let threadListenerID = "threaded-messages-vc-\(UUID().uuidString)"

    override func viewDidLoad() {
        super.viewDidLoad()
        CometChatThreadEvents.addListener(threadListenerID, self)
    }

    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        CometChatThreadEvents.removeListener(threadListenerID)
    }
}
...
extension ThreadedMessagesVC: CometChatThreadEventListener {
    ...
    func ccThreadSubscriptionChanged(parentMessageId: Int, isSubscribed: Bool) {
        ...
        print(isSubscribed ? "Subscribed to thread" : "Unsubscribed from thread")
    }
}
```
...
```swift lines
// The local user subscribed to or unsubscribed from a thread
...
The kit emits this **only on success**, so every surface keeps showing the state the server still holds if a toggle fails.

</Note>

### Thread Events Reference

| Event | Description |
|-------|-------------|
| `ccThreadSubscriptionChanged` | The local user subscribed to or unsubscribed from a thread. `parentMessageId` identifies the thread; `isSubscribed` is the new state. |
...
extension MessageEventsViewController: CometChatMessageEventListener {
    ...
    func ccMessagePinned(message: BaseMessage, status: MessageStatus) {
        // There is no separate unpinned callback — read the field to tell which happened
        print(message.pinnedAt != 0 ? "Pinned" : "Unpinned")
    }
    
    func ccMessageSaved(message: BaseMessage, status: MessageStatus) {
        print(message.savedAt != 0 ? "Saved" : "Unsaved")
    }
...
// Message pinned or unpinned by the local user
CometChatMessageEvents.ccMessagePinned(message: pinnedMessage, status: .success)

// Message saved or unsaved by the local user
CometChatMessageEvents.ccMessageSaved(message: savedMessage, status: .success)
```
...
Listen for taps on interactive elements (buttons, links) inside card bubbles. The UI Kit renders card bubbles automatically (`CometChatCardBubble`) and emits `ccCardActionClicked` when a user taps an action inside one — so a single listener handles every card action across your app. The source `message` is a `CardMessage` for standalone [card messages](/sdk/ios/campaigns#card-messages) and an `AIAssistantMessage` for cards embedded in AI agent replies.
...
class CardEventsViewController: UIViewController {

    override func viewDidLoad() {
        super.viewDidLoad()
        CometChatCardEvents.addListener("card-listener", self)
    }

    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        CometChatCardEvents.removeListener("card-listener")
    }
}

extension CardEventsViewController: CometChatCardEventListener {

    func ccCardActionClicked(message: BaseMessage, action: CometChatCardActionEvent) {
        // `message` is a CardMessage (standalone) or AIAssistantMessage (agent card)
        print("Action: \(action.action), element: \(action.elementId), message: \(message.id)")
        // Handle the tap — navigate, open a URL, call an API, etc.
    }
}
```
...
|----------|------|-------------|
| `action` | `CometChatCardAction` | The action the tapped element is configured to perform. |
| `elementId` | `String` | Identifier of the card element that was tapped. |
| `cardJson` | `String` | The raw card schema JSON of the rendered card. |

### Card Events Reference

| Event | Description |
|-------|-------------|
| `ccCardActionClicked` | Triggered when a user taps an action element inside a card bubble. Provides the source `message` (`CardMessage` or `AIAssistantMessage`) and a `CometChatCardActionEvent`. |
