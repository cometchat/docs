...
```json
{
  ...
  "props": {
    ...
    "threads": {
      "parentMessage": { "type": "BaseMessage?", "default": "nil", "note": "Set to put the header in thread mode" },
      "enableThreadSubscription": { "type": "Bool", "default": false, "note": "Also requires parentMessage" }
    },
    "style": {
...
Enables the subscribe/unsubscribe bell in the header's trailing area, letting users opt in to notifications for a thread's replies. The bell **also** requires `parentMessage` — setting this flag alone on a conversation header renders nothing, so turning the feature on globally cannot put a thread control on a non-thread screen. It renders in one-on-one threads as well as groups.
...
messageHeader.set(enableThreadSubscription: true)
```
...
Hides the subscribe/unsubscribe bell while leaving the feature on — for a screen that already draws its own control and would otherwise show two.
