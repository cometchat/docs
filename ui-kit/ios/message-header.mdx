...
Hides the subscribe/unsubscribe bell while leaving the feature on — for a screen that already draws its own control in the navigation bar. To turn the feature off across your whole app instead, call `CometChatThreadSubscriptionConfig.setEnabled(false)` — see [Turning the Feature Off](/ui-kit/ios/guide-threaded-messages#turning-the-feature-off).
