...
```json
{
  ...
  "props": {
    ...
    "callbacks": {
      ...
      "onAiChatHistoryClicked": "(User) -> Void  (assignable property)",
      "onAiNewChatClicked": "(User) -> Void  (assignable property)"
    },
    "visibility": {
      ...
      "hideThreadSubscriptionButton": { "type": "Bool", "default": false },
      "disableTyping": { "type": "Bool", "default": false }
    },
    "threads": {
      "parentMessage": { "type": "BaseMessage?", "default": "nil", "note": "Set to put the header in thread mode; the subscribe bell renders only in thread mode" }
    },
    "style": {
  ...
  "methods": {
    ...
    "menuCustomization": {
      "set(options:)": "[CometChatPopupMenu.MenuItem]? - Sets custom menu options"
    }
  },
  "events": [],
  "sdkListeners": [
  ...
  "compositionExample": {
    "description": "MessageHeader is typically used at the top of your own chat screen",
...
`CometChatMessageHeader` displays the recipient's information at the top of the chat screen. It's typically used in your own chat screen alongside `CometChatMessageList` and `CometChatMessageComposer`.
...
<Frame>
  <img src="/images/7ae625d7-headerTrail-b0c07329c262e97434036abff2c0132d.png" alt="CometChatMessageHeader with custom trailView showing video call, voice call, and bookmark buttons" />
</Frame>
...
|-----------|------|-------------|
| `options` | `[CometChatPopupMenu.MenuItem]?` | The menu items shown in the header overflow menu |
...
// On CometChatMessageHeader, `set(options:)` takes the ARRAY directly — it is not a closure.
// (Conversations, Users, Groups and Group Members each take a closure returning their own
// option type; the header does not. Passing a closure here will not compile.)
let viewProfileOption = CometChatPopupMenu.MenuItem(
    title: "View Profile",
    icon: UIImage(systemName: "person.circle") ?? UIImage(),
    action: { [weak self] in self?.showProfile() }
)

let muteOption = CometChatPopupMenu.MenuItem(
    title: "Mute Notifications",
    icon: UIImage(systemName: "bell.slash") ?? UIImage(),
    action: { [weak self] in self?.toggleMute() }
)

messageHeader.set(options: [viewProfileOption, muteOption])
```
...
messageHeader.avatarStyle = avatarStyle
```
...
Set `parentMessage` to put the header in **thread mode**, which renders the subscribe/unsubscribe bell in its trailing area. Without a parent message there is no thread to follow, so a conversation header never shows a thread control.

Thread subscription is enabled by default from **v5.2.0**; on **v5.1.22–v5.1.23** the bell renders only once the feature is switched on for the app.

This is the bell to use on a thread screen: every CometChat platform places it in the thread screen's top bar, and on iOS this header in thread mode is what puts it there. [CometChatThreadedMessageHeader](/ui-kit/ios/threaded-messages-header#thread-subscription) carries a fallback bell on its reply-count bar for a screen with no top bar; when your screen renders both, hide that one with its own `hideThreadSubscriptionButton`.
...
let messageHeader = CometChatMessageHeader()
messageHeader.set(parentMessage: parentMessage)
```

Hide it on a particular screen with [`hideThreadSubscriptionButton`](#hidethreadsubscriptionbutton). See the [Thread Subscription guide](/ui-kit/ios/guide-threaded-messages#thread-subscription) for the full feature.

### hideThreadSubscriptionButton

*Available since v5.1.22*

Hides the subscribe/unsubscribe bell while leaving the feature on — for a screen that already draws its own control in the navigation bar.

This is the bell to use on a thread screen: every CometChat platform places the subscription bell in the thread screen's top bar, and on iOS this header in thread mode is what puts it there. [CometChatThreadedMessageHeader](/ui-kit/ios/threaded-messages-header#thread-subscription) carries a fallback bell on its reply-count bar, for a thread screen that has no top bar; when your screen renders both components, hide that one rather than this one.
...
let messageHeader = CometChatMessageHeader()
messageHeader.set(parentMessage: parentMessage)
messageHeader.set(hideThreadSubscriptionButton: true)
```

See the [Thread Subscription guide](/ui-kit/ios/guide-threaded-messages#thread-subscription) for the full feature.
...
messageHeader.onAiNewChatClicked = { [weak self] user in
...
The thread's root message the subscription bell acts on. Setting it puts the header in **thread mode**; leaving it `nil` keeps the header in its normal conversation mode with no bell.
...
messageHeader.statusIndicatorStyle = statusIndicatorStyle
```
...
messageHeader.typingIndicatorStyle = typingIndicatorStyle
```
