...
<Warning>
If your app uses a `SceneDelegate`, also declare `var window: UIWindow?` on your `AppDelegate`. The Calls SDK reads the app delegate's `window`, and without it the app crashes with `-[AppDelegate window]: unrecognized selector` when a call starts. See [Calls SDK setup](/calls/ios/setup#declare-a-window-on-your-appdelegate).
</Warning>
