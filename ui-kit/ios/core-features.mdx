...
On by default — unlike pin and save, it is not gated on a dashboard setting. Each surface is removed independently: the action-sheet option through the message list's `getAdditionalConfiguration().hideThreadSubscriptionOption`, and the bell with `hideThreadSubscriptionButton` on [CometChatMessageHeader](/ui-kit/ios/message-header).
