...
class ThreadSubscriptionViewController: UIViewController {
    ...
    // Read the current state synchronously — no network call
    func isSubscribed(parentMessageId: Int) -> Bool {
        CometChat.threadSubscriptionState(forParentMessageId: parentMessageId) == .SUBSCRIBED
    }
}
```
