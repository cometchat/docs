...
On by default — there is no dashboard flag for this feature, so `CometChatThreadSubscriptionConfig.setEnabled(false)` is the only way to remove it. See [Turning the Feature Off](/ui-kit/ios/guide-threaded-messages#turning-the-feature-off).
