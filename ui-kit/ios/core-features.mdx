...
```json
{
  "category": "messaging",
  "features": [
    {"name": "instantMessaging", "description": "Send and receive text messages in real-time", "component": "CometChatMessageList", "enabledByDefault": true},
  ...
  "relatedComponents": ["CometChatMessageList", "CometChatConversations", "CometChatMessageList", "CometChatMessageComposer"]
}
```
</Accordion>
...
<Frame>
  <img src="/images/ios_multi_attachments_mobile.png" alt="Multi-attachment message list showing an image grid, a file card, and an audio bubble with caption, plus the composer staging tray with mixed image, video, and file tiles" />
</Frame>
...
class MediaChatViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // The message composer includes multi-attachment media sharing:
        // - Multi-select photos and videos from the library
        // - Camera capture
        // - Multiple documents and audio files
        // - Clipboard paste (images, videos, documents, audio)
        // Picks stage in a preview tray, upload with progress, and send
        // as one batch with the composer text as the caption.
...
|------|------------------|
| Images | [Image Bubble](/ui-kit/ios/message-bubble-styling#image-bubble) · multiple: [Media Grid Bubble](/ui-kit/ios/message-bubble-styling#media-grid-bubble) |
| Videos | [Video Bubble](/ui-kit/ios/message-bubble-styling#video-bubble) · multiple: [Media Grid Bubble](/ui-kit/ios/message-bubble-styling#media-grid-bubble) |
| Audio | [Audio Bubble](/ui-kit/ios/message-bubble-styling#audio-bubble) (voice notes) · [Audio Files Bubble](/ui-kit/ios/message-bubble-styling#audio-files-bubble) |
| Files | [File Bubble](/ui-kit/ios/message-bubble-styling#file-bubble) · multiple: [Files Bubble](/ui-kit/ios/message-bubble-styling#files-bubble) |
...
class ThreadedMessagesViewController: UIViewController {
    ...
    func openThread(for parentMessage: BaseMessage, user: User) {
        ...
        messageList.set(parentMessageId: parentMessage.id)
...
Enabled by default from **v5.2.0** — unlike pin and save, it is not gated on a dashboard setting. On **v5.1.22–v5.1.23** the feature is opt-in and neither surface renders until it is switched on for the app. Each surface is then removed independently: the action-sheet option through the message list's `getAdditionalConfiguration().hideThreadSubscriptionOption`, and the bell with `hideThreadSubscriptionButton` on [CometChatMessageHeader](/ui-kit/ios/message-header).
...
class ThreadSubscriptionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // The "Subscribe to thread" / "Unsubscribe from thread" option is added
        // to the message action sheet on parent messages only. To hide it on one
        // screen, set the flag on the list's own configuration object.
        messageListView.getAdditionalConfiguration().hideThreadSubscriptionOption = true
    }

}

// Stay in step with a toggle made anywhere in the kit.
extension ViewController: CometChatThreadEventListener {
    func ccThreadSubscriptionChanged(parentMessageId: Int, isSubscribed: Bool) {
        // update your own control, if you draw one
    }
}
```
