...
On by default — unlike pin and save, it is not gated on a dashboard setting. Remove a surface with `hideThreadSubscriptionOption` on [CometChatMessageList](/ui-kit/ios/message-list) or `hideThreadSubscriptionButton` on [CometChatMessageHeader](/ui-kit/ios/message-header).
