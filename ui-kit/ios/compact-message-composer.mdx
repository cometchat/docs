...
| `applyAttributes(_:to:skippingMentions:)` / `removeAttributes(_:from:skippingMentions:)` | Add or strip attributes over a range. |
| `applyAttributesToSelection(_:skippingMentions:)` / `removeAttributesFromSelection(_:skippingMentions:)` | The same over the current selection; each returns whether there was a selection to act on. |
...
An action that restyles a range will happily run over a mention, which should keep the styling the mentions formatter gives it. The mutating methods take `skippingMentions`, which defaults to `true`, so mention ranges inside the target range are left untouched without any work on your part.

Pass `skippingMentions: false` to opt out. To make the decision yourself, read `mentionRanges` — the ranges are read at call time, so re-read them after a mutation rather than caching, as they shift with the text.
...
CometChatRichTextToolbarAction(
    ...
    // Mentions are skipped by default and keep their own styling.
...
Only `#rgb` and `#rrggbb` are valid on the wire. Eight-digit `#rrggbbaa` is rejected deliberately — a fully transparent run would be an invisible message — so a colour that cannot be sent is not applied locally either.
