...
Attributes you apply are composer-local. The composer serializes to markdown on send, which has no representation for arbitrary attributes, so anything beyond the built-in formats is dropped from the sent message.
...
CometChatRichTextToolbarAction(
    ...
    input.applyAttributesToSelection([.backgroundColor: UIColor.systemYellow])
}
```
</Tab>
</Tabs>
