...
description: "Configure CometChat iOS UI Kit threaded message headers with parent message context, reply lists, composer layout, and back actions."
---
...
| hideThreadSubscriptionButton | Hides this component's own bell. Set it when the screen also renders [CometChatMessageHeader](/ui-kit/ios/message-header#thread-subscription), which carries the navigation-bar bell. | `set(hideThreadSubscriptionButton: true)` |

### Thread Subscription

*Available since v5.1.22*

The subscription bell belongs to [CometChatMessageHeader](/ui-kit/ios/message-header#thread-subscription) — put it in the thread screen's navigation bar, which is where every CometChat platform places it. Thread subscription is enabled by default from **v5.2.0**; on **v5.1.22–v5.1.23** the bell renders only once the feature is switched on for the app.

A thread screen usually renders both components: the message header as the navigation bar, and this component directly beneath it for the parent message and reply count.

```swift lines
import CometChatUIKitSwift

// Navigation bar — `parentMessage` puts it in thread mode and renders the bell
let messageHeaderView = CometChatMessageHeader()
messageHeaderView.set(parentMessage: parentMessage)
...
This component carries a **fallback** bell of its own on the reply-count bar, so a thread screen with no top bar still has the control. When your screen renders both components, hide it — the top-bar bell is then the only one:
