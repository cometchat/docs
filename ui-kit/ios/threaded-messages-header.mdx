...
The header renders a subscribe/unsubscribe bell as a trailing action on its reply-count bar. It appears only on a **sent parent message**, in one-on-one threads as well as groups. The feature is on by default; turn it off across your whole app with `CometChatThreadSubscriptionConfig.setEnabled(false)` — see [Turning the Feature Off](/ui-kit/ios/guide-thread-subscription#turning-the-feature-off).
