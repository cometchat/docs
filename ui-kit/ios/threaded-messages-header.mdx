...
`CometChatThreadedMessageHeader` has no filters of its own. Filter the thread's replies on the `CometChatMessageList` that shows them, with `set(messagesRequestBuilder:)`.

Scope the list to the thread first with `set(user:parentMessage:)` or `set(group:parentMessage:)`, then pass the builder. The list sets the builder's `uid`/`guid` and parent message ID from what it was given, so you don't set them yourself. If the list has no parent message, the builder is scoped to the whole conversation instead of the thread.

**Example**

In this example, we are filtering the thread's replies to those that contain the keyword "payment":

<Tabs>
<Tab title="Swift">
```swift lines
// MARK: - Filter thread replies by search keyword
let messageRequestBuilder = MessagesRequest.MessageRequestBuilder()
    .set(searchKeyword: "payment")

let messageList = CometChatMessageList()
messageList.set(user: user, parentMessage: parentMessage)   // or set(group:parentMessage:)
...
Show only specific types of messages in the thread. Call `set(messagesRequestBuilder:)` after `set(user:parentMessage:)`: in a one-on-one thread, a builder passed before it has its `types` and `categories` replaced with the defaults.
