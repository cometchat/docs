...
```json
{
  ...
  "props": {
    ...
    "callbacks": {
      ...
      "onBack": "() -> Void",
            "onSelectedItemProceed": "([GroupMember]) -> Void  (assignable property)",
    ...
    "viewSlots": {
      ...
      "emptyStateView": "UIView  (assign a view; inherited from CometChatListBase)",
  ...
  "compositionExample": {
    ...
    "components": ["CometChatGroups", "CometChatGroupMembers", "CometChatMessageHeader", "CometChatMessageList", "CometChatMessageComposer"],
...
<Note>
`CometChatGroupMembers` declares its own `init()`, which suppresses the inherited
`UIViewController` initializers — so the group is always supplied through
`set(group:)`, as shown above.

From UI Kit 5.2.0 a convenience initializer is also available:
...
groupMembers.onSelectedItemProceed = { [weak self] selectedMembers in
...
| `onSelectedItemProceed` | Fires when the user confirms a multi-selection | Multi-select members |
...
groupMembers.set(options: { group, member in
    ...
    let messageOption = CometChatGroupMemberOption(
        ...
        onClick: { member, group, section, option, controller in
            let messages = MessagesVC()   // your own VC — see /ui-kit/ios/ios-conversation
...
Add buttons to the navigation bar by assigning `rightBarButtonItem` (an array of `UIBarButtonItem`). In selection mode the kit replaces these with its own Done button.
...
class GroupMembersViewController: UIViewController {
    ...
    private func setupGroupMembers() {
        ...
        groupMembers.rightBarButtonItem = [addButton]
    ...
    @objc func addMemberTapped() {
        // The UI Kit ships no add-members component — build your own picker
        // (CometChatUsers with selectionMode) and add via the SDK:
        //   CometChat.addMembersToGroup(guid:groupMembers:bannedUIDs:onSuccess:onError:)
        // See /sdk/ios/group-add-members
        let picker = AddMembersVC(group: group)   // your own screen
...
let groupMembers = CometChatGroupMembers()
    .set(groupMemberRequestBuilder: adminBuilder)
    .set(group: group)
```
...
// emptyStateView is a UIView you assign (inherited from CometChatListBase).
let label = UILabel()
label.text = "No members in this group"
label.textAlignment = .center

groupMembers.emptyStateView = emptyView          // or: groupMembers.set(emptyView: emptyView)
```
...
groupMembers.onSelectedItemProceed = { selectedMembers in
    print("Selected \(selectedMembers.count) members")
