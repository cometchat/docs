...
class MessagesVC: UIViewController {
    ...
    // The message header is this screen's top bar, so hide the system one every time the
    // screen appears — viewDidLoad runs once, and returning from a pushed screen would
    // otherwise leave two headers.
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        navigationController?.setNavigationBarHidden(true, animated: animated)
    }
    
    // Hide it again once on screen: opened while a list's search is active, UIKit keeps
    // the bar through viewWillAppear.
