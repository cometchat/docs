...
## Calls SDK v5 (direct)
The UI Kit's call screens sit on `CometChatCallsSDK@5`. Read these when you drive a session
yourself (join a session, in-call actions, custom call UI) or the kit's call components don't
cover the need. The site-wide `/docs/llms.txt` does not link these pages directly.
- Setup: [Overview](/calls/ios/overview) · [Setup](/calls/ios/setup) · [Authentication](/calls/ios/authentication) · [Migration Guide](/calls/ios/migration-guide-v5)
- Session: [SessionSettingsBuilder](/calls/ios/session-settings) · [Join Session](/calls/ios/join-session) · [Actions](/calls/ios/actions) · [Events](/calls/ios/events)
- Ringing and push: [Ringing](/calls/ios/ringing) · [VoIP Calling](/calls/ios/voip-calling) · [Background Handling](/calls/ios/background-handling)
- In-call features: [Call Layouts](/calls/ios/call-layouts) · [Audio Modes](/calls/ios/audio-modes) · [Participant Management](/calls/ios/participant-management) · [Screen Sharing](/calls/ios/screen-sharing) · [Recording](/calls/ios/recording) · [Transcription & Closed Captions](/calls/ios/transcription) · [Raise Hand](/calls/ios/raise-hand) · [In-Call Chat](/calls/ios/in-call-chat) · [Share Invite](/calls/ios/share-invite) · [Picture-in-Picture](/calls/ios/picture-in-picture) · [Idle Timeout](/calls/ios/idle-timeout)
- Custom UI: [Custom Control Panel](/calls/ios/custom-control-panel) · [Custom Participant List](/calls/ios/custom-participant-list)
- History and help: [Call Logs](/calls/ios/call-logs) · [Troubleshooting](/calls/ios/troubleshooting)
