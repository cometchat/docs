...
description: "Troubleshoot CometChat iOS UI Kit setup, CocoaPods, Swift Package Manager, initialization, theming, permissions, and real-time updates."
---
...
4. Add `https://github.com/cometchat/cometchat-uikit-ios` with **Up to Exact Version** `5.1.22`
5. Add `https://github.com/cometchat/chat-sdk-ios` with **Up to Exact Version** `4.1.9` — the Chat SDK version must match the one the UI Kit was built against (see the version table in [Getting Started](/ui-kit/ios/getting-started))
...
| Call logs show "Oops! Looks like something went wrong" | No Calls SDK session. Log in with `CometChatUIKit.login`, or call `CometChatCalls.login(authToken:)` after `CometChat.login` — see [Calling Integration](/ui-kit/ios/calling-integration#calls-sdk-session) |
| Call logs show the previous user's history | The Calls SDK session outlived the chat one. Log out with `CometChatUIKit.logout`, or call `CometChatCalls.logout()` alongside `CometChat.logout` |
| Login fails after adding the Calls SDK | A failed Calls SDK login fails `CometChatUIKit.login`. If the app does not use calling, opt out with `UIKitSettings.disableCalling()` |
...
| Smart replies not appearing | Enable Smart Replies under **AI** in the dashboard and set `enableSmartReplies = true` on `CometChatMessageList`. Suggestions follow an incoming text message after a delay (10 seconds by default) |
