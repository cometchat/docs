...
Thread subscription is **enabled by default from v5.2.0**. On **v5.1.22–v5.1.23** it is opt-in: nothing on this page renders until the feature is switched on for the app, and on those versions it must be switched on before `CometChatUIKit.init` runs. From v5.2.0 the gate reads on from process start, so a surface built before `init` still sees the feature as enabled.
</Note>
...
From **v5.2.0** both are shown by default — unlike pin and save, the feature is not gated on a dashboard setting. On **v5.1.22–v5.1.23** neither appears until the feature is switched on for the app. Once it is on, each surface is hidden independently, per instance, so you can offer one and not the other.
