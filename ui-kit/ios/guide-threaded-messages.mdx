...
`CometChatThreadedMessageHeader` has no back button, so the thread screen needs its own exit. Pin a [CometChatMessageHeader](/ui-kit/ios/message-header) above it, as the chat screen does. Its back button pops the screen when pushed and dismisses it when presented:

```swift lines
let messageHeaderView = CometChatMessageHeader()
if let group = group { messageHeaderView.set(group: group) } else if let user = user { messageHeaderView.set(user: user) }
messageHeaderView.set(controller: self)   // required: the back button acts on this controller

// Optional: replace the default pop-or-dismiss
messageHeaderView.set(onBack: { [weak self] in
    self?.navigationController?.popViewController(animated: true)
})
```

Hide the system navigation bar on the thread screen, so there is a single back button:
...
If you keep the system navigation bar visible instead, its back button is the exit and you can leave the message header out.
...
| Display threaded messages | `CometChatMessageList.set(user:parentMessage:)` / `set(group:parentMessage:)` |
...
<CardGroup cols={2}>
  <Card title="Thread Subscription" icon="bell" href="/ui-kit/ios/guide-thread-subscription">
    Let users subscribe to or unsubscribe from a thread's replies
  </Card>
