...
- **Init:** `CometChatUIKit.init(uiKitSettings:result:)`
- **Login:** `CometChatUIKit.login(uid:result:)` or `CometChatUIKit.login(authToken:result:)`
- **Logout:** `CometChatUIKit.logout(user:result:)`
- **Create user:** `CometChatUIKit.create(user:result:)`
- **Send message:** `CometChatUIKit.sendTextMessage(message:)`, `sendMediaMessage(message:)`, `sendCustomMessage(message:)` — no completion; the outcome arrives via `CometChatMessageEvents.ccMessageSent`
