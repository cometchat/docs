...
<Warning>
**Call logs need a Calls SDK session.** From v5, the Calls SDK authenticates the call-history request with its own session rather than your chat token. Without it, this component shows *"Oops! Looks like something went wrong"* — even while chat and calling work. Either:

1. **Recommended** — log in with `CometChatUIKit.login`, which starts that session for you, and `CometChatUIKit.logout` to end it.
2. If your app logs in with `CometChat.login` directly, call `CometChatCalls.login(authToken:)` yourself once it succeeds, and `CometChatCalls.logout()` on sign-out.

See [Calling Integration](/ui-kit/ios/calling-integration#calls-sdk-session).
</Warning>
...
cometChatCallLogs.set(onError: { error in
    // error is typed Any. Fetching call logs fails with a CometChatCallException;
    // looking up the tapped call's user or group fails with a CometChatException.
    if let error = error as? CometChatCallException {
        print(error.errorDescription)
    } else if let error = error as? CometChatException {
        print(error.errorDescription)
    }
})
```
...
let button = UIBarButtonItem(
    ...
    style: .plain,
    target: self,
    action: #selector(newCallTapped)
)
callLogs.rightBarButtonItem = [button]
...
```swift lines
let callLogs = CometChatCallLogs()
callLogs.set(onItemClick: { [weak self] item in
    // The tapped item is typed Any; cast it to CallLog (not Call)
    guard let callLog = item as? CallLog else { return }
