...
| Dependencies | `CometChatSDK 4.1.9` (installed automatically) |
...
target 'YourApp' do
  pod 'CometChatUIKitSwift', '5.1.22'
...
<Warning>
**Pin the Chat SDK to the exact version the UI Kit requires.** `CometChatUIKitSwift` ships as a prebuilt binary compiled against one specific `CometChatSDK` version — for UI Kit `5.1.22` that is `CometChatSDK 4.1.9` (see the version table at the top of this page).
