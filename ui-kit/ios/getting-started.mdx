...
| `.headerBased` | Token is sent via HTTP headers, preventing exposure in server logs. More secure for production. |

<Note>
If you're building custom media handling outside the UI Kit components, you'll need to handle secure media access yourself. See [Secure Media Access](/ui-kit/ios/methods#secure-media-access) for implementation details.
</Note>
