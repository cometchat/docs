...
| 5 | `textFormatters` on list components | `CometChatTextFormatter[]` | `CometChatTextFormatter[]` | Prop removed in 7.0.0 and **back in 7.2.0** on `CometChatConversations`, `CometChatMessageList` and `CometChatThreadHeader` — but formatter classes must be ported to the v7 `CometChatTextFormatter` API. On 7.0.x–7.1.x the prop is unavailable |
...
| `textFormatters` | `CometChatTextFormatter[]` | 7.2.0 | Prop name is back, but formatter classes must be ported to the v7 `CometChatTextFormatter` API |
...
<Note>
`textFormatters` was removed in 7.0.0 and the prop is **back in 7.2.0** — but the class behind it was rewritten, so formatter classes must be ported to the v7 `CometChatTextFormatter` API. v7 declares `id`, `getRegex()` and `format()` as abstract members, so a v6 subclass does not compile; in plain JavaScript it fails silently instead. v6-only hooks such as `formatMessageForSending()`, `setClasses()` and `setStyle()` no longer exist. See [Text Formatters](/ui-kit/react/plugins/text-formatters) for the v7 interface. On 7.0.x–7.1.x the prop is unavailable and formatting must go through a plugin.
</Note>
