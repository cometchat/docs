...
| 5 | `textFormatters` on list components | `CometChatTextFormatter[]` | `CometChatTextFormatter[]` | Removed in 7.0.0, **restored in 7.2.0** on `CometChatConversations`, `CometChatMessageList` and `CometChatThreadHeader`. On 7.0.x–7.1.x use the plugin system |
...
| `textFormatters` | `CometChatTextFormatter[]` | 7.2.0 | Carried over from v6. Dropped in 7.0.0 and restored in 7.2.0 |
...
<Note>
`textFormatters` was removed in 7.0.0 and **restored in 7.2.0**. If you are landing on 7.2.0 or later, keep your v6 `textFormatters` — no migration needed. On 7.0.x–7.1.x it is unavailable and formatting must go through a plugin.
</Note>
