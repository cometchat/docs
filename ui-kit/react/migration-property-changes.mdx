...
**The rule: in v7 a formatter works on text, never on the message.** `format(text)` goes in, a string comes out. Composer-side formatters still get live-input access — the composer assigns `inputElementReference`, calls `initializeComposerTracking()`, fans keystrokes to `onKeyUp()`/`onKeyDown()`, and provides `getCaretPosition()`/`setCaretPosition()`. What no longer reaches a formatter is the **message object and the logged-in user**, so every v6 member carrying that context is gone — along with the v6 hooks the kit used to call, which v7 replaced or dropped.

That splits a v6 formatter into one part that carries over and two that do not:

| What your v6 formatter does | On v7 |
| --- | --- |
| Transforms text by overriding `customLogicToFormatText()` | **Works unchanged.** Add `readonly id` for TypeScript |
| **Calls** a removed helper | **Throws** `TypeError: … is not a function` — loud, easy to find |
| **Overrides** a removed hook, or **reads** a removed context field | **Silently does nothing.** No error; the code just never runs |

**What still works.** v7's `format()` delegates to `getFormattedText()` and then to `customLogicToFormatText()` — the same override point v6 used. `id` is the only abstract member, so a v6 subclass fails to compile with `TS2515: … does not implement inherited abstract member id` until you add one:
...
Plain JavaScript runs without an `id`, but declare one anyway — it is how the pipeline identifies your formatter.

**Removed helpers — these throw when called.** Delete the calls.
...
<Warning>
**Removed hooks and context — these fail silently.** Your method or field is still on your class, so nothing throws. It is simply never called or never populated.

**Hooks nothing calls any more:** `onRegexMatch()` · `registerEventListeners()` · `formatMessageForSending()` · `getFormattedTextForEntity()`

**Renamed live-input hook:** `formatTextOnKeyUp()` and `debouncedFormatTextOnKeyUp()` are gone — v7 calls `formatText()` instead, and only when you override it. Move that logic to `formatText()`. `onKeyUp()` and `onKeyDown()` are unchanged and still called.

**Context fields nothing populates any more:** `messageObject` · `loggedInUser` · `user` · `group` · `composerId` · `messageBubbleAlignment` · `classes` · `cssClassMapping` · `textStyle` · `timeoutID`
...
This is the category to audit, because nothing surfaces it. Two common cases from the v6 guides:

- **The v6 URL formatter pattern** — [that guide](/ui-kit/react/v6/url-formatter-guide) has you override `onRegexMatch()` and `registerEventListeners()`. In v7 neither is called: the formatter compiles, formats nothing, and attaches no click handlers. Move the transform into `customLogicToFormatText()` and attach click handling yourself.
- **`formatMessageForSending()`** — in v6 the composer called this just before sending so you could attach metadata or tags to the outgoing message. v7 has no equivalent. Move that logic into your own send path, setting it on the message before `CometChatUIKit.sendTextMessage()`.
</Warning>
