...
If your Next.js app is running inside an `<iframe>`, you need to ensure the CometChat UI Kit mounts into the correct document context. Wrap your entire application in `CometChatFrameProvider` in `pages/_app.js` (or `_app.tsx`) using one of the following approaches:

### Option 1: Dynamic Import (Disable SSR)

```jsx
// pages/_app.js
import dynamic from 'next/dynamic'

const CometChatFrameProvider = dynamic(
  () => import('@cometchat/ui-kit').then(mod => mod.CometChatFrameProvider),
  { ssr: false }
)
...
function MyApp({ Component, pageProps }) {
  ...
  // During SSR, render the page without the provider
  if (!isBrowser) {
