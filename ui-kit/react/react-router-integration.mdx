...
The UI Kit ships with a base stylesheet (CSS variables + component styles). Import it once at app startup (for example in `app/root.tsx`).
...
Some React Router templates render on the server by default. CometChat UI Kit relies on browser APIs, so make sure initialization and UI rendering happen in the browser (see the React Router chat experience guides for a no-SSR route pattern).
...
2. Confirm you see successful init/login logs in the browser console.
3. If you see SSR-related errors, follow the React Router chat experience guides for a no-SSR route pattern.
