...
| SDK peer dep | `@cometchat/chat-sdk-javascript` ^4 (v7.2.1 requires ^4.2.0) |
...
```bash
npm install @cometchat/chat-uikit-react@7 @cometchat/chat-sdk-javascript@^4
...
| `@cometchat/chat-sdk-javascript` | `^4.x` | `^4` — peer dep; v7.2.1 requires `^4.2.0` |
| `react` | `>=18` | `>=18 <21` |
...
Bump your SDK first. v7.2.1 declares `@cometchat/chat-sdk-javascript` `^4.2.0`; staying on a 4.1.x pin leaves npm to resolve a peer conflict. The authoritative range is the `peerDependencies` block of the UI Kit version you install.
</Note>
...
| Text formatters on lists | `textFormatters` prop | Still `textFormatters` — removed in 7.0.0, restored in 7.2.0 on Conversations, MessageList and ThreadHeader |
