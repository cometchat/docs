...
| Bundled deps | `dompurify` (new in v7) and `@cometchat/cards-react` (already a dependency since 6.5.0) — installed automatically, not peer deps |
...
| `@cometchat/cards-react` | Bundled (since 6.5.0) | Bundled — do **not** install it yourself |
...
ChatConfigurator.enable((dataSource) => new MyDecorator(dataSource));
```
...
| Text formatters on lists | `textFormatters` prop | Back in 7.2.0 on Conversations, MessageList and ThreadHeader. A v6 formatter carries over when its transform lives in `customLogicToFormatText()` or `getFormattedText()` (add `readonly id` in TypeScript); transforms in `onRegexMatch()`, and hooks such as `formatMessageForSending()`, are never called in v7 — see [Porting a v6 formatter](/ui-kit/react/migration-property-changes#porting-a-v6-formatter) |
...
<CometChatConversations showSearchBar={false} onItemClick={handleClick} />
```
