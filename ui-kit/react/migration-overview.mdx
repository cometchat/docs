...
| Text formatters on lists | `textFormatters` prop | Prop name returns in 7.2.0 on Conversations, MessageList and ThreadHeader, but formatter classes must be ported to the v7 `CometChatTextFormatter` API — see [Text Formatters](/ui-kit/react/plugins/text-formatters) |
