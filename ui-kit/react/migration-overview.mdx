...
You can still use `CometChatUIKit.init()` + `login()` and compose individual providers. See [CometChatProvider — Individual Providers](/cometchat-provider#approach-2-individual-providers-advanced).
...
See [Plugins Overview](/plugins/overview) for the full guide.
...
See [Event System](/event-system) for the full event catalog.
...
For the full prop-by-prop reference, see [Property Changes](/migration-property-changes).
...
<CardGroup cols={2}>
  <Card title="Property Changes" icon="list" href="/migration-property-changes">
    Full prop-by-prop migration reference for every component
  </Card>
  <Card title="CometChatProvider" icon="box" href="/cometchat-provider">
