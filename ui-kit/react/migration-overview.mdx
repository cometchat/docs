...
| SDK peer dep | `@cometchat/chat-sdk-javascript` ^4.2.0 (required by 7.2.x) |
| New bundled deps | `dompurify`, `@cometchat/cards-react` — installed automatically, not peer deps |
| Removed dep | `rxjs` (no longer needed) |
| Stylesheet | Delete the v6 `css-variables.css` import — v7 loads its own styles |
...
```bash
npm install @cometchat/chat-uikit-react@7 @cometchat/chat-sdk-javascript@^4.2.0
npm uninstall rxjs  # No longer needed for CometChat
```

| Package | v6 | v7 |
| --- | --- | --- |
| `@cometchat/chat-uikit-react` | `^6.x` | `^7.2.1` |
| `@cometchat/chat-sdk-javascript` | `^4.x` | `^4.2.0` — peer dep, required by 7.2.x |
| `react` | `>=18` | `>=18 <21` |
| `rxjs` | Required | Not needed — uninstall it |
| `@cometchat/calls-sdk-javascript` | Optional | Optional (unchanged) |
| `dompurify` | — | Bundled — do **not** install it yourself |
| `@cometchat/cards-react` | — | Bundled — do **not** install it yourself |

<Note>
`dompurify` and `@cometchat/cards-react` are regular dependencies of the UI Kit, so npm pulls them in automatically. They are not peer dependencies and do not belong in your `package.json`.

Bump your SDK first. 7.2.x declares `@cometchat/chat-sdk-javascript` `^4.2.0`, so staying on a 4.1.x pin fails with `ERESOLVE` — `^4` is not sufficient. Installing `^4.2.0` is safe on every v7 release, including 7.0.x and 7.1.x, whose ranges it also satisfies.
</Note>

**Delete your v6 stylesheet import.** v6 required you to import the base stylesheet yourself:

```tsx title="v6 — remove this"
...
| v6 | v7 |
| --- | --- |
| `await CometChatUIKit.getLoggedinUser()` | `await CometChat.getLoggedinUser()` (SDK), or `CometChatUIKit.getLoggedInUser()` (sync, after login) |
| `CometChatUIKitLoginListener.getLoggedInUser()` | `CometChatUIKit.getLoggedInUser()` |

<Warning>
`CometChatUIKit.getLoggedInUser()` is **synchronous** in v7 and returns `CometChat.User | null` directly — not a Promise. Note the capital **I**: v6's async method was spelled `getLoggedinUser()`, and that spelling no longer exists on `CometChatUIKit`, so v6 code carried over unchanged throws `TypeError: CometChatUIKit.getLoggedinUser is not a function` at runtime (TypeScript flags it at compile time).

Await the SDK's `CometChat.getLoggedinUser()` when you need the asynchronous form, or drop the `await` and call `CometChatUIKit.getLoggedInUser()` once login has completed. `CometChatUIKitLoginListener` no longer exists in v7.
</Warning>
...
```tsx title="v7 — init doubles as the session check"
const existingUser = await CometChatUIKit.init(settings);
if (!existingUser) {
  await CometChatUIKit.login("uid");
}
render(<App />);
...
useCometChatEvents((event) => {
  // ui:message/sent fires at each stage — check status, or you will
  // handle the same message twice (once with no ID yet, once confirmed).
  if (event.type === "ui:message/sent" && event.status === "success") { ... }
  if (event.type === "message/edited") { ... }
}, []); // deps is required — omitting it throws at runtime
```
...
| Text formatters on lists | `textFormatters` prop | Back in 7.2.0 on Conversations, MessageList and ThreadHeader. Display-only v6 formatters still work (add `readonly id` in TypeScript); calls to `setStyle`/`setClasses` now throw, and a `formatMessageForSending` override silently stops running — see [Text Formatters](/ui-kit/react/plugins/text-formatters) |
