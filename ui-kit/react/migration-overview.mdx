...
<Note>
The media bubbles are renamed to their multi-attachment v7 components: `CometChatImageBubble` → `CometChatImagesBubble`, `CometChatVideoBubble` → `CometChatVideosBubble`, `CometChatFileBubble` → `CometChatFilesBubble`, and `CometChatAudioBubble` → `CometChatAudiosBubble` (attached audio files) or `CometChatVoiceNoteBubble` (recorded voice notes).
</Note>
