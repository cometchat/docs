  ...
  "props": {
    "entity": {
      ...
      "parentMessage": {
        ...
        "default": "undefined",
        "note": "Enables thread mode — fetches replies to this message. Preferred over parentMessageId; holding the whole parent also lets the list reflect live thread-subscription state on realtime replies."
      },
      "parentMessageId": {
        "type": "number",
        "default": "undefined",
        "note": "Deprecated — pass parentMessage instead. Still honoured (as the parent's id) when parentMessage is absent."
      }
    },
    "data": {
    ...
    "formatting": {
      "textFormatters": { "type": "CometChatTextFormatter[]", "default": "undefined", "since": "7.2.0", "note": "Merged into text and caption bubbles." }
    },
    "viewSlots": {
...
| `ui:thread/subscription-changed` | `{ parentMessageId, subscribed }` | The user toggles the subscribe/unsubscribe option, or an auto-subscription fires (the user sends a reply, or is @mentioned in one) — mirrored to the parent whether or not the thread panel is open |
...
When pinning and saving are enabled for your app (the `features.ux.messages.pinned.enabled` and `features.ux.messages.saved.enabled` app settings), `CometChatMessageList` automatically adds **Pin**, **Unpin**, **Save**, and **Unsave** to the message context menu — no props required. The **Pin/Unpin option is shown to every member** — the client does not gate it by role. The **server** enforces who may pin or unpin in that conversation; a denied attempt is rejected and surfaces a permission toast (localized via the `action_permission_denied` key). Saving is per-user and always available.
...
Enables thread mode. When set, the component fetches and displays replies to this parent message instead of the main conversation feed. Prefer this over [`parentMessageId`](#parentmessageid): holding the whole parent message lets the list read the thread's live subscription state and stamp it onto realtime-arriving replies (a socket-delivered reply carries no flag of its own). Re-pass it when the parent updates in real time.
...
**Deprecated — pass [`parentMessage`](#parentmessage) instead.** Enables thread mode by id. Still honoured (as the parent's id) when `parentMessage` is absent, so existing integrations keep working; new code should pass the whole `parentMessage`.
...
Custom display formatters merged into the text and caption bubbles rendered by the list.
...
<Note>
Available from **7.2.0**. The prop existed in v6, was dropped in 7.0.0 in favour of the plugin system, and returned in 7.2.0. A v6 formatter carries over when its transform lives in `customLogicToFormatText()` or `getFormattedText()`; in TypeScript, add `readonly id = "…"`, since `id` is now abstract. A transform in `onRegexMatch()` never runs in v7, and members v7 removed (`setStyle()`, `setClasses()` and the rest) throw when called — inside the display pipeline that error is caught and your formatter is skipped rather than surfaced. See [Porting a v6 formatter](/ui-kit/react/migration-property-changes#porting-a-v6-formatter) and [Text Formatters](/ui-kit/react/plugins/text-formatters). On 7.0.x–7.1.x the prop is not available.

Register the same formatters on `CometChatMessageComposer` as well. If you format only in the composer, the author sees the styled result while readers see the raw marker text — see the [Custom Text Formatter guide](/ui-kit/react/guide-custom-text-formatter).
</Note>
