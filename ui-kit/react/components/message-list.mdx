...
When several attachments are sent together from the [composer's staging tray](/ui-kit/react/components/message-composer#multi-attachment-staging-tray), they arrive as multiple media messages — one per media kind — that all share a single `batchId`. The message list detects consecutive messages carrying the same `batchId` and renders them as one visually connected group instead of separate, independently-chromed bubbles.
...
Each message in a batch is assigned a `batchPosition`, which the [plural bubbles](/ui-kit/react/components/images-bubble) use to adjust their corner radius so the group reads as a single stack:
