...
{/* TODO: add a screenshot of a grouped multi-attachment batch (e.g. images + a file sent together, showing shared avatar on top and receipt on bottom). */}
...
Each message in a batch is assigned a `batchPosition`, which the [media bubbles](/ui-kit/react/components/image-bubble) use to adjust their corner radius so the group reads as a single stack:
