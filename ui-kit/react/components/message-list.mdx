...
<Note>
Available from **7.2.0**. The prop existed in v6, was dropped in 7.0.0 in favour of the plugin system, and returned in 7.2.0. A v6 formatter that overrides `customLogicToFormatText()` still works here — in TypeScript, add `readonly id = "…"`, since `id` is now abstract. Calls to `setStyle()` or `setClasses()` throw and must be removed, and a `formatMessageForSending()` override no longer runs at all. See [Porting a v6 formatter](/ui-kit/react/migration-property-changes#porting-a-v6-formatter) and [Text Formatters](/ui-kit/react/plugins/text-formatters). On 7.0.x–7.1.x the prop is not available.
