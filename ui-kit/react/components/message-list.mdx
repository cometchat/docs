...
<Note>
Available from **7.2.0**. The prop existed in v6, was dropped in 7.0.0 in favour of the plugin system, and returned in 7.2.0 — but the formatter class was rewritten in v7, so a v6 formatter cannot be passed here unchanged. Port it to the v7 `CometChatTextFormatter` API first: see [Text Formatters](/ui-kit/react/plugins/text-formatters). On 7.0.x–7.1.x the prop is not available.
