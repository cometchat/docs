...
1. **Pick** — choose files from the attachment (`+`) menu, or drag-and-drop them onto the composer (unless [`disableDragAndDrop`](#disabledraganddrop) is set).
2. **Stage & upload** — each file becomes a tile in the tray and begins uploading directly to storage right away, showing its own progress. The user can keep typing or add more files while uploads run.
...
4. **Send** — on send, the staged files fan out into media messages that share a single batch, so the message list renders them as a connected group of [plural bubbles](/ui-kit/react/components/images-bubble) (see [Batch send](#batch-send)).
