...
<Warning>
**The drop target is the composer only.** Drag-and-drop is wired to the composer element, not to the whole chat view. A file dropped on the message list, the message header, or anywhere else on the page is not handled by the UI Kit — the browser takes over and navigates away to open the file, losing unsent state.
