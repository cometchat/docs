...
```json
{
  ...
  "props": {
    ...
    "editReply": {
      "messageToEdit": { "type": "CometChat.TextMessage | CometChat.MediaMessage | null", "default": "null", "note": "MediaMessage accepted from 7.1.0, when media captions became editable." },
    ...
    "attachments": {
      ...
      "hideAttachmentOptions": { "type": "CometChatAttachmentHideOptions", "default": "undefined", "keys": ["image", "video", "audio", "file", "polls", "collaborativeDocument", "collaborativeWhiteboard"], "note": "All keys optional booleans. Stickers are not a key — use hideStickersButton." },
...
<Warning>
**The drop target is the composer only.** Drag-and-drop is wired to the composer element, not to the whole chat view. A file dropped on the message list, the message header, or anywhere else on the page is not handled by the UI Kit — the browser takes over — opening the file or downloading it, depending on the type — and any unsent state is lost.

If your layout invites dropping onto the wider chat area, catch it yourself: add `onDragOver`/`onDrop` handlers on your container that call `event.preventDefault()`, so a stray drop is a no-op instead of a navigation.
</Warning>
...
| | |
| --- | --- |
| Type | `CometChat.TextMessage \| CometChat.MediaMessage \| null` |
| Default | `null` |

`CometChat.MediaMessage` is accepted from **7.1.0**, when media captions became editable. On 7.0.x only `CometChat.TextMessage` is accepted.
...
<Note>
**Extension-backed options are always shown — the extension decides whether they work.** The attachment menu lists **Polls**, **Collaborative Document** and **Collaborative Whiteboard** whenever they are not hidden by `hideAttachmentOptions`; **Stickers** is a separate composer button governed by [`hideStickersButton`](#hidestickersbutton), not an entry in this menu. The composer does **not** check whether the matching extension is enabled in the [CometChat Dashboard](/ui-kit/react/extensions), so an option stays visible even when its extension is off, and using it then fails. Enable the extension — [Polls](/fundamentals/polls), [Collaborative Document](/fundamentals/collaborative-document), [Collaborative Whiteboard](/fundamentals/collaborative-whiteboard), [Stickers](/fundamentals/stickers) — or hide the option yourself. See [Extensions](/ui-kit/react/extensions) for the full list the composer integrates.

The three menu options are also hidden automatically in a **thread** composer (when `parentMessageId` is set), because they do not support a parent message.
</Note>
...
Hide specific attachment options in the default layout. Replaces the seven individual `hide*Option` booleans from v6.
...
Every key is an optional `boolean`, defaulting to `false`. Set a key to `true` to hide that option; omitted keys leave the option visible.

| Key | Hides | v6 equivalent |
| --- | --- | --- |
| `image` | Image / photo attachment | `hideImageAttachmentOption` |
| `video` | Video attachment | `hideVideoAttachmentOption` |
| `audio` | Audio file attachment | `hideAudioAttachmentOption` |
| `file` | Generic file attachment | `hideFileAttachmentOption` |
| `polls` | Polls (requires the Polls extension) | `hidePollsOption` |
| `collaborativeDocument` | Collaborative Document (requires the extension) | `hideCollaborativeDocumentOption` |
| `collaborativeWhiteboard` | Collaborative Whiteboard (requires the extension) | `hideCollaborativeWhiteboardOption` |
...
<Note>
Stickers are **not** a key on this object — hide them with the separate [`hideStickersButton`](#hidestickersbutton) prop, since the sticker picker is its own composer button rather than an entry in the attachment menu.
</Note>
...
Drag-and-drop is also disabled **while a message is being edited**, regardless of this prop. Attachments cannot be added to an existing message, so the drop overlay does not appear and dropped files are discarded. The drop is still cancelled rather than passed to the browser, so editing in progress is never lost to a navigation. Drop behaviour resumes once the edit is sent or cancelled.
...
| Drag-and-drop overlay | `.cometchat-message-composer__drop-overlay` |
