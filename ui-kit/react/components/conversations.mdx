...
```json
{
  ...
  "props": {
    ...
    "formatting": {
      "textFormatters": { "type": "CometChatTextFormatter[]", "default": "undefined", "since": "7.2.0", "note": "Applied to the last-message subtitle." }
    },
    "viewSlots": {
...
Custom display formatters applied to the **last-message subtitle** of each conversation item.
...
```tsx
<CometChatConversations textFormatters={[new ColorFormatter()]} />
```
...
A formatter only applies where you register it. Register the same formatter on the message list and composer too, or the preview here will render differently from the message itself — see the [Custom Text Formatter guide](/ui-kit/react/guide-custom-text-formatter).
</Note>
