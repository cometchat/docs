...
```json
{
  ...
  "props": {
    ...
    "formatting": {
      "textFormatters": { "type": "CometChatTextFormatter[]", "default": "undefined", "since": "7.2.0", "note": "Applied to the parent message bubble text." }
    },
    "viewSlots": {
  ...
  "events": {
    "emitted": [
      {
        "name": "ui:thread/subscription-changed",
        "payload": "{ parentMessageId, subscribed }",
        "description": "Fires when the user subscribes to or unsubscribes from the thread via the bell"
      }
    ],
    "received": [
      {
...
- A thread subscription (subscribe/unsubscribe) bell
...
| --- | --- | --- |
| `ui:thread/subscription-changed` | `{ parentMessageId, subscribed }` | The user subscribes to or unsubscribes from the thread via the subscription bell |

This is the same channel the message-list subscribe/unsubscribe option publishes on, so a flip in either place — or an automatic subscription — is reflected everywhere. See [Threaded Messages → Thread Subscription](/ui-kit/react/guide-threaded-messages#thread-subscription).
...
The header shows a bell that lets the user subscribe to or unsubscribe from the thread, so they can receive updates about new replies even when they aren't actively viewing it. It appears in both 1:1 and group threads. The bell reflects the current subscription state and toggles it on click, with an optimistic UI update and a toast on failure.

The bell renders automatically in both 1:1 and group threads. To remove it, set `hideThreadSubscriptionToggle`; to react to changes, pass `onThreadSubscriptionChange`.
...
Hide the subscribe/unsubscribe (subscription) bell.
...
```tsx
<CometChatThreadHeader parentMessage={message} textFormatters={[new ColorFormatter()]} />
```
...
Register the same formatters on the thread's `CometChatMessageList` so the replies format identically — see the [Custom Text Formatter guide](/ui-kit/react/guide-custom-text-formatter).
</Note>
