...
This is the **multi-attachment replacement** for `CometChatImageBubble`. The message list always renders it for image messages in v7. The legacy `CometChatImageBubble` remains available as a deprecated standalone component you can use directly, but the list no longer uses it.
...
<iframe
  ...
  title="CometChat Images Bubble — 3 Image Grid"
...
| --- | --- | --- |
| Used when | Standalone use only (deprecated) | Always, for image messages in the list |
