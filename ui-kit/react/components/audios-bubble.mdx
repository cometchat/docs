...
This is the **multi-attachment replacement** for `CometChatAudioBubble` when dealing with attached audio files. The message list always renders it for audio messages that do **not** have `audioType: "voice_note"` in their metadata (voice notes render as a [Voice Note Bubble](/ui-kit/react/components/voice-note-bubble) instead). The legacy `CometChatAudioBubble` remains available as a deprecated standalone component you can use directly, but the list no longer switches to it.
...
| --- | --- | --- |
| Used when | Standalone use only (deprecated) | Always, for audio messages without `audioType: "voice_note"` |
