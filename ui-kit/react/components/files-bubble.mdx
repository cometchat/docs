...
This is the **multi-attachment replacement** for `CometChatFileBubble`. The message list always renders it for file messages in v7. The legacy `CometChatFileBubble` remains available as a deprecated standalone component you can use directly, but the list no longer uses it.
...
<iframe
  ...
  title="CometChat Files Bubble — Two Files"
...
| --- | --- | --- |
| Used when | Standalone use only (deprecated) | Always, for file messages in the list |
