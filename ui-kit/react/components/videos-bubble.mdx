...
This is the **multi-attachment replacement** for `CometChatVideoBubble`. The message list always renders it for video messages in v7. The legacy `CometChatVideoBubble` remains available as a deprecated standalone component you can use directly, but the list no longer uses it.
...
<iframe
  ...
  title="CometChat Videos Bubble — Grid With Caption"
  allow="clipboard-write"
></iframe>
...
| --- | --- | --- |
| Used when | Standalone use only (deprecated) | Always, for video messages in the list |
