...
| Event Type | Payload | When |
| --- | --- | --- |
| `thread/subscription-changed` | `{ parentMessageId: number, subscribed: boolean }` | The server acknowledged a subscribe/unsubscribe on a thread |

This is the network-acknowledged event. For the optimistic local flip published the moment a user clicks the bell, see [`ui:thread/subscription-changed`](#thread) below.
