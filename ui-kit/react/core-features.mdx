...
Share multiple images, videos, audio files, and documents in a single send. The attachment menu opens a multi-select picker — or paste and drag-and-drop files straight into the composer, and the picks stage in a preview tray above the input with per-file upload progress, remove, and retry. They send together as one batch, with the composer text carried as an optional caption — all out of the box.

<Frame>
  <img src="/images/multi_attachments_react.png" />
</Frame>

| Components                                              | Functionality                                                                                                                                                                     |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Message Composer](/ui-kit/react/components/message-composer#multi-attachment-staging-tray) | A multi-select picker and a staging tray that uploads files before send, with per-file progress, remove, and retry. The composer text becomes the batch caption. |
| [Message List](/ui-kit/react/components/message-list#multi-attachment-batch-grouping)       | Renders received attachments as per-type bubbles and groups a multi-type send as one visually connected batch.                                                    |
