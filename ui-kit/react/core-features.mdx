...
The UI Kit also subscribes a user automatically when they send a message (to that message's own thread), when they post a reply in a thread, and when they are @mentioned in a reply — so people keep hearing about threads they are actually part of. A deliberate unsubscribe is remembered. See the [Threaded Messages guide](/ui-kit/react/guide-threaded-messages#thread-subscription) for the full behaviour, the `hide*` props, and the `useThreadSubscription` hook.
...
Each feature is switched on per app, and each carries a cap on how many items a user may pin or save:
