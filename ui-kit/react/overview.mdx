...
The versions above are correct as of `@cometchat/chat-uikit-react@7.2.x`, and `@cometchat/chat-sdk-javascript` `^4.2.0` also satisfies every earlier 7.x release, so it is safe whichever v7 version you are on. If you are pinned to SDK 4.1.x for unrelated reasons, note that 7.0.3 and later require 4.1.12 or newer. The authoritative range is always the `peerDependencies` block of the version you install — check it with `npm view @cometchat/chat-uikit-react peerDependencies`.
</Note>
