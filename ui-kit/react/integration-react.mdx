...
<Note>
For production, use the `authToken` prop instead of `authKey` + `uid`. Generate auth tokens server-side via the [REST API](/rest-api/chat-apis). Never ship auth keys in client code.
</Note>
