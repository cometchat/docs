...
The plugin's `renderBubble` is a thin forwarder to `CometChatCollaborativeDocumentBubble` — a self-extracting component that reads the document URL from the message's extension metadata at `@injected.extensions.document.document_url`.
