...
All formatters extend the abstract `CometChatTextFormatter` class. `id` is its only abstract member — everything else has a working default, so a v6 formatter that only transforms text carries over with an `id` added. See [Porting a v6 formatter](/ui-kit/react/migration-property-changes#porting-a-v6-formatter) for the cases that do need changes.
...
abstract class CometChatTextFormatter {
  ...
  /** Regex pattern for detecting formattable content. Defaults to the first of `regexPatterns`. */
  getRegex(): RegExp;

  /** Transform the input text. Default stores state and delegates to
      getFormattedText() -> customLogicToFormatText(), so a v6-style formatter
      need only override customLogicToFormatText. */
  format(text: string): string;

  /** The display transform. Default: identity. The usual override point. */
