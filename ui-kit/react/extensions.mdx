---
title: "Extensions - CometChat React UI Kit"
description: "Explore CometChat React UI Kit v6 extensions: stickers, polls, link previews, message translation, and more. Learn how Dashboard-enabled extensions auto-integrate into your chat components."
sidebarTitle: "Extensions"
journey_stage: "feature-discovery"
step_number: 3
total_steps: 8
next_page: "/ui-kit/react/ai-features"
prev_page: "/ui-kit/react/core-features"
required_components: ["/ui-kit/react/message-list", "/ui-kit/react/message-composer"]
---

<Info>
📍 **Step 3 of 8: Feature Discovery** — [← Core Features](/ui-kit/react/core-features) | [Components →](/ui-kit/react/components-overview)
</Info>
...
**If you want stickers** → enable in Dashboard → Extensions → Stickers, auto-integrates with [MessageComposer](/ui-kit/react/message-composer) + [MessageList](/ui-kit/react/message-list)

**If you want polls** → enable in Dashboard → Extensions → Polls, auto-integrates with [MessageComposer](/ui-kit/react/message-composer) + [MessageList](/ui-kit/react/message-list)

**If you want link previews** → enable in Dashboard → Extensions → Link Preview, auto-integrates with [MessageList](/ui-kit/react/message-list)

**If you want message translation** → enable in Dashboard → Extensions → Message Translation, auto-integrates with [MessageList](/ui-kit/react/message-list)

**If you want profanity filter** → enable in Dashboard → Extensions → Profanity Filter — works automatically, no component changes needed
</Tip>
...
|---------|---------------------|--------------------------|
| Stickers | [MessageComposer](/ui-kit/react/message-composer), [MessageList](/ui-kit/react/message-list) | Yes (Dashboard → Extensions → Stickers) |
| Polls | [MessageComposer](/ui-kit/react/message-composer), [MessageList](/ui-kit/react/message-list) | Yes (Dashboard → Extensions → Polls) |
| Link Preview | [MessageList](/ui-kit/react/message-list) | Yes (Dashboard → Extensions → Link Preview) |
| Message Translation | [MessageList](/ui-kit/react/message-list) | Yes (Dashboard → Extensions → Message Translation) |
| Profanity Filter | Automatic (no component) | Yes (Dashboard → Extensions → Profanity Filter) |
| Image Moderation | Automatic (no component) | Yes (Dashboard → Extensions → Image Moderation) |
| Data Masking | Automatic (no component) | Yes (Dashboard → Extensions → Data Masking) |
...
```mermaid
graph TD
    A["What extension do you need?"] --> B["🎨 Rich Media"]
    A --> C["🛡️ Moderation"]
    A --> D["🌐 Translation"]
    
    B --> B1["Stickers → Dashboard → Extensions"]
    B --> B2["Polls → Dashboard → Extensions"]
    B --> B3["Link Preview → Dashboard → Extensions"]
    
    C --> C1["Profanity Filter → automatic"]
    C --> C2["Image Moderation → automatic"]
    C --> C3["Data Masking → automatic"]
    
    D --> D1["Message Translation → Dashboard → Extensions"]
```

## Built-in Extensions

<Warning>
**Dashboard Setup Required:** All extensions must be enabled in your CometChat Dashboard at Dashboard → Extensions before they appear in the UI Kit. Each extension has its own toggle.
</Warning>
...
<CardGroup cols={3}>
  ...
  <Card title="Core Features" icon="puzzle-piece" href="/ui-kit/react/core-features">
    Explore core chat features
  </Card>
