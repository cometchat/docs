...
function SubscribeButton({ parentMessageId }: { parentMessageId: number }) {
  const { isSubscribed, toggle } = useThreadSubscription(parentMessageId);
  return (
...
Subscription changes are also broadcast on the [event bus](/ui-kit/react/event-system): `ui:thread/subscription-changed` (the optimistic local flip) and `thread/subscription-changed` (the server acknowledgement).
