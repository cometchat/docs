...
The same skill adds **voice & video calling** too — run `/cometchat add voice and video calls to my app` and pick **Ringing** (incoming/outgoing call screens with system push) or **Session** (a shared meeting link) mode. And when you add chat, it asks **how** you want to customize: **Visually** — opening CometChat's Visual Builder in your browser and writing the styled result straight into your app — or **In code**. Either path bootstraps the kit from a generated `cometchat-settings.json` via file-based initialization.
