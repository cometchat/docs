...
| Primary output | `createGroup()` / `joinGroup()` → `Promise<CometChat.Group>`; `leaveGroup()` / `deleteGroup()` → `Promise<boolean>` |
| Constraints | Three group types on the wire — `public`, `private`, `password` — exposed as four constants: `GROUP_TYPE.PROTECTED` and `GROUP_TYPE.PASSWORD` both resolve to `"password"`. Only that type accepts a password argument. GUIDs are immutable. |
