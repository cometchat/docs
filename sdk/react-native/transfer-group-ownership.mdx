...
description: "Transfer CometChat group ownership to another member in React Native apps before the current owner leaves the group."
---
...
| Primary output | `Promise<string>` — the new owner's UID; rejects with `CometChatException` |
| Constraints | Owner-only, and the target must already be a member. The former owner stays in the group as an admin. |
| Related | [Change Member Scope](/sdk/react-native/group-change-member-scope) · [Leave a Group](/sdk/react-native/leave-group) · [Delete a Group](/sdk/react-native/delete-group) |
