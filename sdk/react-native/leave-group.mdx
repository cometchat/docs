...
| Key Methods | `addGroupListener()`, `leaveGroup()`, `removeGroupListener()` |
| Key Classes | `Action`, `CometChatException`, `Group`, `GroupListener`, `User` |
...
| Constraints | The owner cannot leave without transferring ownership first. Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Join a Group](/sdk/react-native/join-group) · [Transfer Ownership](/sdk/react-native/transfer-group-ownership) · [Delete a Group](/sdk/react-native/delete-group) |
