...
description: "Send and receive ephemeral real-time messages with the CometChat React Native SDK for live reactions and temporary indicators."
---
...
| Key Classes | `MessageListener`, `TransientMessage` |
...
| Primary output | `sendTransientMessage()` returns **`void`** — fire-and-forget, nothing to await and no delivery guarantee |
| Constraints | Transient messages are never persisted: no history, no receipts, no offline delivery. Only online recipients see them. Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Typing Indicators](/sdk/react-native/typing-indicators) · [Send a Message](/sdk/react-native/send-message) · [Real-Time Listeners](/sdk/react-native/real-time-listeners) |
