...
| Key classes | `CometChat.ConversationsRequestBuilder` · `CometChat.Conversation` |
| Request builder | `new CometChat.ConversationsRequestBuilder().setLimit(n).build()` then `.fetchNext()` |
| Primary output | `Promise<Conversation[]>` — an **empty array means the end of the list**, it does not throw |
| Key methods | `CometChat.getConversation(with, type)` · `CometChat.deleteConversation(with, type)` · `CometChat.tagConversation(...)` |
| Prerequisites | `init()` then `login()` resolved |
| Constraints | Scope with `.setConversationType("user")` or `"group"` for a 1:1-only or groups-only app. In the UI Kit, pass the **builder instance** to `conversationsRequestBuilder`, **not** the result of `.build()` — the kit builds it internally |
| Related | [Receive Messages](/sdk/react-native/receive-messages) · [Conversations (UI Kit)](/ui-kit/react-native/conversations) |
