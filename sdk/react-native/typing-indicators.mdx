...
| Key methods | `CometChat.startTyping(typingNotification)` · `CometChat.endTyping(typingNotification)` |
| Key classes | `CometChat.TypingIndicator` |
| Primary output | **`void`** for both — **do not `await` them** |
| Listeners registered | `onTypingStarted` / `onTypingEnded` on `MessageListener` |
| Prerequisites | `init()` then `login()` resolved |
| Constraints | `endTyping` must be called **explicitly** — there is no client-side auto-expiry, so a missed call leaves the peer showing "typing…" indefinitely. Debounce `startTyping`; do not call it per keystroke |
