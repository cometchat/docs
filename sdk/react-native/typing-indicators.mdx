...
description: "Send and receive CometChat typing indicators in React Native apps for user and group conversations with real-time listeners."
---
...
| Key Methods | `addMessageListener()`, `endTyping()`, `removeMessageListener()`, `startTyping()` |
| Key Classes | `MessageListener`, `TypingIndicator` |
...
| Primary output | `startTyping()` / `endTyping()` return **`void`** — fire-and-forget, not Promises |
| Constraints | Do not `await` them. They are transient: no persistence, and only online recipients receive them. Always pair `startTyping()` with `endTyping()`. Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Transient Messages](/sdk/react-native/transient-messages) · [Send a Message](/sdk/react-native/send-message) · [Real-Time Listeners](/sdk/react-native/real-time-listeners) |
| Full reference | [`TypingIndicator`](/sdk/reference/auxiliary#typingindicator) · [`CometChatException`](/sdk/reference/auxiliary#cometchatexception) |
