...
description: "Delete CometChat messages in React Native apps by message ID and listen for real-time deletion events."
---
...
| Key Methods | `addMessageListener()`, `deleteMessage()` |
| Key Classes | `BaseMessage`, `CometChatException`, `MessageListener` |
...
| Primary output | `Promise<CometChat.BaseMessage>` — the message in its deleted state (a tombstone, not removal); rejects with `CometChatException` |
| Constraints | The message object survives with `getDeletedAt()` set — render a deleted-state bubble rather than dropping it from the list. Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Edit a Message](/sdk/react-native/edit-message) · [Send a Message](/sdk/react-native/send-message) · [Receive Messages](/sdk/react-native/receive-messages) |
