...
| Key methods | `CometChat.deleteMessage(messageId)` |
| Primary output | `Promise<BaseMessage>` — resolves a **tombstone**, not void; rejects with `CometChatException` |
| Listeners registered | `onMessageDeleted` on `MessageListener` |
| Prerequisites | `init()` then `login()` resolved; the user must own the message or have moderation scope |
| Constraints | **Soft delete** — the message does not vanish from the conversation. Peers receive `onMessageDeleted` and the UI Kit renders a "message deleted" bubble. Do not assume the id disappears from history |
| Related | [Edit Message](/sdk/react-native/edit-message) · [Receive Messages](/sdk/react-native/receive-messages) |
