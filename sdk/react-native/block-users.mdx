...
| Key methods | `CometChat.blockUsers(uids)` · `CometChat.unblockUsers(uids)` |
| Primary output | `Promise<Object>` — a per-UID success map; rejects with `CometChatException` |
| Prerequisites | `init()` then `login()` resolved |
| Constraints | Both take an **array of UIDs**, not a single uid — passing a bare string is the common mistake. Blocking is one-directional and takes effect immediately for new messages |
| Related | [Retrieve Users](/sdk/react-native/retrieve-users) · [User Management](/sdk/react-native/user-management) |
