...
| Key methods | `CometChat.login(uid, authKey)` *(dev)* · `CometChat.login(authToken)` *(production)* · `CometChat.logout()` · `CometChat.getLoggedinUser()` |
| Primary output | `login` → `Promise<CometChat.User>`; `getLoggedinUser` → `Promise<User \| null>` — **`null` is the normal "no session" answer, not an error** |
| Prerequisites | `CometChat.init()` must have **resolved**. Calling login first fails silently |
| Constraints | `login` is **variadic and untyped** in the type definitions, so TypeScript will **not** catch a wrong call — the only two valid shapes are `(uid, authKey)` and `(authToken)`. Note the method is `getLogged**in**User` (lowercase `in`). **Never ship the Auth Key** — production mints a short-lived auth token server-side |
| Logout order | **Unregister the push token before `logout()`**, or the device keeps receiving the previous user's notifications |
| Related | [Setup](/sdk/react-native/setup-sdk) · [User Management](/sdk/react-native/user-management) |
