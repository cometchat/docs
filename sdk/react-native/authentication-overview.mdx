...
| Key Methods | `createUser()`, `getLoggedinUser()`, `login()`, `logout()` |
| Key Classes | `CometChatException`, `User` |
| Primary output | `login()` → `Promise<CometChat.User>`; `getLoggedinUser()` → `Promise<CometChat.User | null>`; `logout()` → `Promise<Object>` |
| Constraints | Auth Key is **development only** — it can create and log in any user. Production must mint an Auth Token server-side. `login()` rejects if `init()` has not resolved. |
| Related | [Setup SDK](/sdk/react-native/setup-sdk) · [User Management](/sdk/react-native/user-management) · [Connection Status](/sdk/react-native/connection-status) |
