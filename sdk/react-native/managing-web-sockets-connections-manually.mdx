...
description: "Learn how to manage WebSocket connections in the CometChat React Native SDK, including connect, disconnect, and ping."
---
...
| Key Methods | `connect()`, `disconnect()`, `getLoggedinUser()`, `init()`, `login()`, `ping()` |
| Key Classes | `AppSettings`, `AppSettingsBuilder`, `CometChatException` |
...
| Primary output | `connect()`, `disconnect()` and `ping()` all return **`void`** — they are fire-and-forget, not awaitable |
| Constraints | Manual mode requires `autoEstablishSocketConnection(false)` at `init()`. Calling `connect()` before `login()` resolves has no effect. |
| Related | [Setup SDK](/sdk/react-native/setup-sdk) · [Connection Status](/sdk/react-native/connection-status) · [Authentication](/sdk/react-native/authentication-overview) |
