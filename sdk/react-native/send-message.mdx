...
| Key methods | `CometChat.sendMessage(message)` · `CometChat.sendMediaMessage(message)` · `CometChat.sendCustomMessage(message)` · `CometChat.sendInteractiveMessage(message)` · `CometChat.sendTransientMessage(message)` |
| Key classes | `CometChat.TextMessage` · `CometChat.MediaMessage` · `CometChat.CustomMessage` · `CometChat.InteractiveMessage` |
| Primary output | `Promise<BaseMessage>` — resolves to the **sent** message (with `id`, `sentAt`); rejects with `CometChat.CometChatException` |
...
| Prerequisites | `CometChat.init()` resolved, then `CometChat.login()` resolved. Sending before login rejects |
| Constraints | **There is no `sendCardMessage()`** — card and interactive messages are **receive-only** from the client; do not infer one from the three `send*` methods that do exist. `sendTransientMessage` returns **void** and is **not persisted** |
| Receive counterpart | Sent messages reach peers via `CometChat.addMessageListener` — see [Receive Messages](/sdk/react-native/receive-messages) |
| Related | [Upload Files](/sdk/react-native/upload-files) · [Edit Message](/sdk/react-native/edit-message) · [Delete Message](/sdk/react-native/delete-message) · [Threaded Messages](/sdk/react-native/threaded-messages) |
