...
A **Card Message** is a message with `category: "card"` — a rich, structured card rendered from Card Schema JSON.

<Note>
Card messages are **receive-only**. They **cannot be sent from the SDK** — they are created and sent exclusively via the **Platform (REST) API** or the **Dashboard Bubble Builder**. The SDK only receives them, through the `onCardMessageReceived` callback of the `MessageListener`.
</Note>

To handle incoming card messages, see [Receive Messages → Card Messages](/sdk/react-native/receive-messages#card-messages). For the full `CardMessage` field reference, see [Campaigns → Card Messages](/sdk/react-native/campaigns#card-messages).
