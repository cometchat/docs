...
| Key Classes | `BaseMessage`, `CometChatException`, `CometChatHelper`, `MessageListener`, `REACTION_ACTION`, `ReactionCount` |
...
| Primary output | `addReaction()` / `removeReaction()` → `Promise<CometChat.BaseMessage>` — the message with its updated reaction set |
| Constraints | Reactions resolve to the **message**, not a reaction object. Enumerate with `ReactionsRequest` (`fetchNext()` → `Promise<Reaction[]>`). Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Send a Message](/sdk/react-native/send-message) · [Receive Messages](/sdk/react-native/receive-messages) · [Real-Time Listeners](/sdk/react-native/real-time-listeners) |
| Full reference | [`Reaction`](/sdk/reference/auxiliary#reaction) · [`ReactionCount`](/sdk/reference/auxiliary#reactioncount) · [`ReactionEvent`](/sdk/reference/auxiliary#reactionevent) · [`BaseMessage`](/sdk/reference/messages#basemessage) |
