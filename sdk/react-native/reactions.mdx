...
| Key methods | `CometChat.addReaction(messageId, reaction)` · `CometChat.removeReaction(messageId, reaction)` |
| Primary output | `Promise<BaseMessage>` — resolves the **updated** message with its reaction list |
| Listeners registered | `onMessageReactionAdded` / `onMessageReactionRemoved` on `MessageListener` |
| Prerequisites | `init()` then `login()` resolved |
| Constraints | Reactions are **core messaging in v5 — there is no extension to enable.** The dashboard's legacy Reactions extension is already part of core and scheduled for deprecation; enabling it is unnecessary. The UI Kit's `CometChatMessageList` renders reactions itself |
| Related | [Receive Messages](/sdk/react-native/receive-messages) · [Message List (UI Kit)](/ui-kit/react-native/message-list) |
