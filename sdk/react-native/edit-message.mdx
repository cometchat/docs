...
| Key methods | `CometChat.editMessage(message)` |
| Primary output | `Promise<BaseMessage>` — resolves the **edited** message with `editedAt` set |
| Listeners registered | `onMessageEdited` on `MessageListener` |
| Prerequisites | `init()` then `login()` resolved; the user must own the message |
| Constraints | Pass the **modified message object**, not just the new text. Peers are notified via `onMessageEdited` — update your local copy from the listener, not optimistically, or the two diverge |
| Related | [Delete Message](/sdk/react-native/delete-message) · [Send Messages](/sdk/react-native/send-message) |
