...
| Key Classes | `BaseMessage`, `CometChatException`, `MediaMessage`, `MessageListener`, `TextMessage` |
...
| Primary output | Moderation is applied server-side; `sendMessage()` still resolves to the sent message, which then transitions `PENDING` → `APPROVED` / `DISAPPROVED` |
| Constraints | A disapproved message never reaches recipients — the sender's own promise still resolves. Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Send a Message](/sdk/react-native/send-message) · [Receive Messages](/sdk/react-native/receive-messages) · [AI Agents](/sdk/react-native/ai-agents) |
