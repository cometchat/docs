...
| Key Methods | `createUser()`, `updateCurrentUserDetails()`, `updateUser()` |
...
| Primary output | `createUser()` / `updateUser()` / `updateCurrentUserDetails()` → `Promise<CometChat.User>` |
| Constraints | `createUser()` and `updateUser()` need the **Auth Key** and are development-only — do not ship them in a client. `updateCurrentUserDetails()` is the safe client-side call. |
| Related | [Authentication](/sdk/react-native/authentication-overview) · [Retrieve Users](/sdk/react-native/retrieve-users) · [User Presence](/sdk/react-native/user-presence) |
