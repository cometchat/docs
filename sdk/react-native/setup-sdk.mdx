...
title: "Setup"
sidebarTitle: "Setup"
description: "Install, configure, initialize, and log in users with the CometChat React Native SDK using app settings and auth keys."
---
...
| Field | Value |
| --- | --- |
| Package | `@cometchat/chat-sdk-react-native` + `@react-native-async-storage/async-storage` (required peer — the SDK persists session state through it) |
| Import | `import { CometChat } from "@cometchat/chat-sdk-react-native";` |
| Key methods | `CometChat.init(appId, appSettings)` · `CometChat.initFromSettings(settings)` |
| Key classes | `CometChat.AppSettingsBuilder` · `CometChat.CometChatSettings` |
| Primary output | `Promise<boolean>` — resolves `true`; rejects with `CometChatException` |
| Prerequisites | App ID + Region from the dashboard. Nothing else — this is the entry point |
| Constraints | **`init()` must RESOLVE before `login()`**, and login before any send or fetch. Out of order fails **silently** — a blank screen, not an exception. Prefer **`initFromSettings`**: it persists `integrationSource` for attribution, where the classic `init` does not. In `initFromSettings`, presence lives at `chatSDK.presenceSubscription.type`, not top-level |
| Related | [Authentication](/sdk/react-native/authentication-overview) · [Key Concepts](/sdk/react-native/key-concepts) |
