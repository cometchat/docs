...
| Primary output | `init()` → `Promise<boolean>` — resolves `true` once the SDK is ready |
| Constraints | Every other SDK call rejects until `init()` resolves. `init()` is idempotent but must not race — await it once at app startup, not per screen. |
| Related | [Authentication](/sdk/react-native/authentication-overview) · [Connection Status](/sdk/react-native/connection-status) · [Key Concepts](/sdk/react-native/key-concepts) |
