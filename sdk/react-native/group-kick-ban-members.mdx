...
| Key classes | `CometChat.BannedMembersRequestBuilder` — lists banned members |
| Primary output | `Promise<boolean>` for all three; rejects with `CometChatException` |
| Request builder | `new CometChat.BannedMembersRequestBuilder(guid).setLimit(n).build()` then `.fetchNext()` |
| Prerequisites | The logged-in user must be group **admin or owner** — these are scope-gated |
| Constraints | **Ban is half a round trip.** `CometChatGroupMembers` in the UI Kit lists **active** members only, and v5 ships no banned-members component — so an app that offers ban and nothing else leaves the admin unable to see or reverse it. Always ship `BannedMembersRequestBuilder` + `unbanGroupMember` alongside `banGroupMember`. Kick is reversible (the user may rejoin); ban is not until unbanned |
| Related | [Retrieve Group Members](/sdk/react-native/retrieve-group-members) · [Add Members](/sdk/react-native/group-add-members) |
