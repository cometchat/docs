...
| Key methods | `CometChat.markAsRead(message)` · `CometChat.markAsDelivered(message)` · `CometChat.markAsUnread(message)` · `CometChat.markConversationAsRead(conversation)` · `CometChat.getMessageReceipts(messageId)` |
| Primary output | `markAsRead` is **untyped** (`any`) — treat it as fire-and-forget. `getMessageReceipts` → `Promise<MessageReceipt[]>` |
| Listeners registered | `onMessagesDelivered` · `onMessagesRead` · `onMessagesDeliveredToAll` · `onMessagesReadByAll` on `MessageListener` |
| Prerequisites | `init()` then `login()` resolved |
| Constraints | Delivery is usually automatic; **read is not** — call `markAsRead` when the message is actually visible, not when it arrives, or read counts are wrong. Use `markConversationAsRead` to clear a whole thread in one call |
| Related | [Receive Messages](/sdk/react-native/receive-messages) · [All Real Time Listeners](/sdk/react-native/real-time-listeners) |
