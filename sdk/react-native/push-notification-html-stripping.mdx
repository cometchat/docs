...
| Primary output | A helper concern — no SDK call; affects the notification payload text only |
| Constraints | Applies to the push payload, not to the message stored by the SDK — `getText()` still returns the original. |
| Related | [Send a Message](/sdk/react-native/send-message) · [Receive Messages](/sdk/react-native/receive-messages) |
| Full reference | [`TextMessage`](/sdk/reference/messages#textmessage) · [`BaseMessage`](/sdk/reference/messages#basemessage) |
...
<CardGroup cols={2}>
<Card title="Push Notifications" icon="bell" href="/notifications/react-native-push-notifications">
