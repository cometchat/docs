...
| Key Methods | `addMessageListener()`, `fetchPrevious()`, `fetchNext()`, `getUnreadMessageCount()`, `getLastDeliveredMessageId()`, `getMessageDetails()`, `getUnreadMessageCountForAllGroups()`, `getUnreadMessageCountForAllUsers()`, `getUnreadMessageCountForGroup()`, `getUnreadMessageCountForUser()`, `removeMessageListener()` |
...
| Primary output | `fetchNext()` / `fetchPrevious()` → `Promise<CometChat.BaseMessage[] | []>`; listeners return `void` and deliver via callbacks |
| Constraints | `fetchPrevious()` walks backwards through history; `fetchNext()` walks forward. Mixing them on one request object interleaves pages confusingly. Every `add*Listener()` needs a matching `remove*Listener()` with the same listener ID — unremoved listeners leak and fire after unmount. |
| Related | [Send a Message](/sdk/react-native/send-message) · [Message Filtering](/sdk/react-native/additional-message-filtering) · [Delivery & Read Receipts](/sdk/react-native/delivery-read-receipts) |
