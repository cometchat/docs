...
| Key methods | `addMessageListener` · `addUserListener` · `addGroupListener` · `addCallListener` · `addLoginListener` · `addConnectionListener` — each with a matching `remove*Listener(id)` |
| Primary output | **`void`** for every `add*Listener` — not a subscription object |
| Listeners registered | messages + typing + receipts + reactions (`MessageListener`) · presence (`UserListener`) · joins/leaves/kicks/bans/scope (`GroupListener`) · call lifecycle (`CallListener`) · token expiry (`LoginListener`) · socket state (`ConnectionListener`) |
| Prerequisites | `init()` then `login()` resolved |
| Constraints | **Register in an effect, remove in its cleanup.** The SDK — not React — holds the listener, so it survives unmount and navigation. A leaked listener produces duplicate messages that compound over the app's lifetime |
| Related | [Receive Messages](/sdk/react-native/receive-messages) · [Connection Status](/sdk/react-native/connection-status) |
