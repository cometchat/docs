...
<Tab title="Swift">
```swift
CometChat.pinMessage(messageId: messageId) { _ in
    // pinned
} onError: { error in
    if error.errorCode == "ERR_PINNED_MESSAGES_LIMIT_EXCEEDED",
       let limit = error.errorParams?["limit"] as? Int {
        print("This conversation allows at most \(limit) pinned messages")
    }
}
```
</Tab>
...
For the two limit errors, read the actual cap from `errorParams` rather than hard-coding a number — the limit is server-owned and may change.
