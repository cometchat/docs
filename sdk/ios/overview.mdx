...
When your app handles media files (images, videos, audio, documents), CometChat uses a File Access Token (FAT) to authenticate requests. The `setSecureMediaMode()` method lets you configure how this token is transmitted.

| Mode | Description |
| ---- | ----------- |
| `.embedded` | Token is appended to the URL as a query parameter (`?fat=xxx`). This is the default behavior. |
| `.headerBased` | Token is sent via HTTP headers, preventing exposure in server logs and browser history. More secure. |

To enable header-based mode, add `.setSecureMediaMode(.headerBased)` during initialization:
...
let appSettings = AppSettings.AppSettingsBuilder()
    ...
    .setRegion(region: "us")
    .autoEstablishSocketConnection(true)
    .setSecureMediaMode(.headerBased)  // Enable secure header-based mode
    .build()
...
<Note>
If you're using CometChat UIKit, secure media is handled automatically. This configuration is only needed when loading media manually in custom implementations.
</Note>

For detailed implementation guidance on loading images, videos, and files securely, see [Secure Media Access](/sdk/ios/secure-media-access).
