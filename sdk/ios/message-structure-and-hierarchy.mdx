...
Card Messages are **receive-only**: they are created via the Platform REST API or the Dashboard Bubble Builder and delivered to clients like any other message. See [Send a Message](/sdk/ios/send-message#card-message) for the `CardMessage` fields and [Receive Messages](/sdk/ios/receive-message#real-time-messages) for handling incoming card messages.
