...
description: "Fetch, filter, tag, and search CometChat conversations with the iOS SDK for one-on-one and group chats."
---
...
Tags are your own free-form labels. Pinning a conversation is a separate feature — use `pinConversation` and `ConversationRequestBuilder.set(pinnedBy:)` rather than a `"pinned"` tag.

```swift
let tags = ["priority", "archived"]
var conversationRequest = ConversationRequest.ConversationRequestBuilder(limit: 50)
  .setTags(tags)
  .build()
```
