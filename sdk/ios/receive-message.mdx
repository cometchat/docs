...
extension ViewController: CometChatMessageDelegate {
  ...
  func onCardMessageReceived(cardMessage: CardMessage) {
    print("CardMessage received successfully: " + cardMessage.stringValue())
  }
}
```
</Tab>
...
- (void)onCardMessageReceivedWithCardMessage:(CardMessage *)cardMessage {
    NSLog(@"CardMessage received: %@",[cardMessage stringValue]);
}
...
`onCardMessageReceived(cardMessage:)` is triggered when a card message (category `"card"`) is received. Card Messages are **receive-only** — they are created via the Platform REST API or the Dashboard Bubble Builder, not sent from the SDK (see [Send a Message → Card Message](/sdk/ios/send-message#card-message)).

The `CardMessage` payload exposes:

| Method | Returns | Description |
| ------ | ------- | ----------- |
| `getCard()` | `[String: Any]?` | The raw card schema/payload to render. |
