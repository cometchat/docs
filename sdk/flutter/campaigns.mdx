...
| Purpose | Fetch notification feed items, listen for real-time delivery, mark items as read/delivered, report engagement, and retrieve unread counts in Flutter. |
| Key methods | `addMessageListener()` · `addNotificationFeedListener()` · `getNotificationFeedItem()` · `getNotificationFeedUnreadCount()` · `markFeedItemAsDelivered()` · `markFeedItemAsRead()` · `markPushNotificationClicked()` · `markPushNotificationDelivered()` · `removeNotificationFeedListener()` · `reportFeedEngagement()` |
| Key classes | `CometChatException` · `CometChatCardView` · `CardMessage` · `CometChatCardActionEvent` · `CometChatCardChatWithUserAction` · `CometChatCardOpenUrlAction` · `CometChatCardThemeMode` |
| Listener callbacks | `onCardMessageReceived()` · `onFeedItemReceived()` |
...
| Full reference | [`CometChatException`](/sdk/reference/auxiliary#cometchatexception) · [`CardMessage`](/sdk/reference/messages#cardmessage) |
...
class CardMessageHandler with MessageListener {
  ...
  void onCardMessageReceived(CardMessage cardMessage) {
    debugPrint("Card message received: ${cardMessage.getCard()}");
  }
}
```
</Tab>
...
See [Receive Messages → Receiving Card Messages](/sdk/flutter/receive-messages#receiving-card-messages) for the full handling and rendering reference.
