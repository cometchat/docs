...
description: "Manage CometChat Flutter SDK WebSocket behavior with automatic connection handling, background disconnects, and manual connection mode."
---
...
| Purpose | Manage CometChat Flutter SDK WebSocket behavior with automatic connection handling, background disconnects, and manual connection mode. |
| Key methods | `connect()` · `disconnect()` · `init()` · `ping()` |
| Key classes | `CometChatException` · `CometChatSubscriptionType` |
...
| Related | [Connection Status](/sdk/flutter/connection-status) |
