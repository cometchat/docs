...
| Purpose | Upload files directly to storage with per-file progress, remove, and retry through an UploadFileRequest — then send them as one or more media messages with multiple attachments. |
| Key methods | `createUploadFileRequest()` · `getMaxAttachmentCount()` · `getMaxAttachmentSize()` · `sendMediaMessage()` |
| Key classes | `CometChatException` · `MediaMessage` · `Attachment` · `CometChatConversationType` · `CometChatMessageType` |
| Listener callbacks | `onComplete()` · `onFileError()` · `onFileFailure()` · `onFileProgress()` · `onFileUploaded()` |
...
| Related | [Send A Message](/sdk/flutter/send-message) · [Messaging](/sdk/flutter/messaging-overview) |
| Full reference | [`CometChatException`](/sdk/reference/auxiliary#cometchatexception) · [`MediaMessage`](/sdk/reference/messages#mediamessage) · [`Attachment`](/sdk/reference/auxiliary#attachment) |
