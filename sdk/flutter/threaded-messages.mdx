...
| Purpose | Send, receive, and fetch CometChat threaded messages in Flutter apps using parent message IDs. |
| Key methods | `addMessageListener()` · `sendMessage()` |
| Key classes | `TextMessage` · `CometChatException` · `BaseMessage` · `CustomMessage` · `MediaMessage` · `CometChatConversationType` · `CometChatMessageType` |
| Listener callbacks | `onCustomMessageReceived()` · `onMediaMessageReceived()` · `onTextMessageReceived()` |
...
| Full reference | [`TextMessage`](/sdk/reference/messages#textmessage) · [`CometChatException`](/sdk/reference/auxiliary#cometchatexception) · [`BaseMessage`](/sdk/reference/messages#basemessage) · [`CustomMessage`](/sdk/reference/messages#custommessage) · [`MediaMessage`](/sdk/reference/messages#mediamessage) |
