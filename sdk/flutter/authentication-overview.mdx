...
description: "Authenticate Flutter app users with CometChat using UID login, auth tokens, session checks, logout, and backend user management."
---
...
| Purpose | Authenticate Flutter app users with CometChat using UID login, auth tokens, session checks, logout, and backend user management. |
| Key methods | `getLoggedInUser()` · `login()` · `loginWithAuthToken()` · `logout()` |
...
| Related | [Setup](/sdk/flutter/setup) · [Login Listeners](/sdk/flutter/login-listeners) |
