...
description: "Fetch CometChat users in Flutter apps, get logged-in user details, and filter user lists with request builder options."
---
...
| Purpose | Fetch CometChat users in Flutter apps, get logged-in user details, and filter user lists with request builder options. |
| Key methods | `getLoggedInUser()` · `getOnlineUserCount()` · `getUser()` |
| Key classes | `User` · `CometChatException` · `CometChatUserStatus` |
...
| Related | [User Management](/sdk/flutter/user-management) · [User Presence](/sdk/flutter/user-presence) · [Block Users](/sdk/flutter/block-users) |
