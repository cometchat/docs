...
description: "Send and receive CometChat typing indicators in Flutter apps for one-on-one and group conversations."
---
...
| Purpose | Send and receive CometChat typing indicators in Flutter apps for one-on-one and group conversations. |
| Key methods | `addMessageListener()` · `endTyping()` · `startTyping()` |
| Key classes | `CometChatReceiverType` · `TypingIndicator` |
| Listener callbacks | `onTypingEnded()` · `onTypingStarted()` |
...
| Related | [Transient Messages](/sdk/flutter/transient-messages) · [All Real Time Listeners](/sdk/flutter/real-time-listeners) |
| Full reference | [`TypingIndicator`](/sdk/reference/auxiliary#typingindicator) |
