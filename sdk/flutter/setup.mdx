...
<Warning>
We exclude only the dead `i386` architecture so the app still builds and runs natively on Apple Silicon (M1/M2/M3) simulators. Do **not** add `arm64` to this exclusion — doing so breaks native simulator builds on Apple Silicon Macs.
</Warning>
