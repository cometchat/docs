...
As mentioned in the [Send a Message](/sdk/flutter/v4/send-message) section, you can send a message to a User or a Group by mentioning the receiver (uid/guid) and `receiverType`(user/group).
...
The procedure to receive real-time messages is exactly the same as mentioned in the [Receive Messages](/sdk/flutter/v4/receive-messages) section. Use the `MessageListener` class to listen for incoming thread messages. Check if the received message belongs to the active thread using the `parentMessageId` field.
