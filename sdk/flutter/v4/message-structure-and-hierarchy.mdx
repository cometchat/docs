...
Every message in CometChat belongs to a category (`message`, `custom`, `interactive`, `action`, `call`) and has a specific type within that category. On the SDK side, all messages extend [`BaseMessage`](/sdk/flutter/v4/key-concepts), with subclasses like `TextMessage`, `MediaMessage`, `CustomMessage`, `InteractiveMessage`, `Action`, and `Call`. Understanding this hierarchy helps you handle different message types correctly.
...
<Note>
To know about Interactive messages please [click here](/sdk/flutter/v4/interactive-messages).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Interactive Messages" icon="hand-pointer" href="/sdk/flutter/v4/interactive-messages">
