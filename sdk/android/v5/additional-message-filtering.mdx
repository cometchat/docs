...
We recommend before trying this, you refer to the [Message structure and hierarchy guide](/sdk/android/v5/message-structure-and-hierarchy) to get familiar with the various categories of messages.
...
We recommend before trying this, you refer to the [Message structure and hierarchy guide](/sdk/android/v5/message-structure-and-hierarchy) to get familiar with the various types of messages.
...
The pinned and saved lists paginate forward-only on an internal server cursor. `fetchNext()` works the same from the caller's side, but message-window filters such as `setMessageId()` and `setTimestamp()` do not apply to these two queries.
