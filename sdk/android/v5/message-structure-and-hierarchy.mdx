...
As you can see in the above diagram, every message belongs to a particular category. A message can belong to either one of the 5 categories
...
Card Messages are **receive-only**: they are created via the Platform REST API or the Dashboard Bubble Builder and delivered to clients like any other message. See [Send a Message](/sdk/android/v5/send-message#card-message) for the `CardMessage` fields and [Receive Messages](/sdk/android/v5/receive-messages#real-time-messages) for handling incoming card messages.
