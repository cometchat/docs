...
A user can save a limited number of messages (100 by default). When the limit is exceeded, the SDK surfaces the server error through `onError`, and the applicable limit can be read programmatically from `CometChatException.getErrorParams()` under the `limit` key — never hard-code it.
