...
| Mode | Description |
| ---- | ----------- |
| `LEGACY` | Token is appended to the URL as a query parameter (`?fat=xxx`). This is the default behavior. |
| `HEADER_BASED` | Token is sent via HTTP headers, preventing exposure in server logs and browser history. More secure. |

To enable header-based mode, add `.setSecureMediaMode(SecureMediaMode.HEADER_BASED)` during initialization:
...
val appSettings = AppSettings.AppSettingsBuilder()
    ...
    .setRegion("us")
    .autoEstablishSocketConnection(true)
    .setSecureMediaMode(SecureMediaMode.HEADER_BASED)  // Enable secure header-based mode
    .build()
...
For detailed implementation guidance on loading images, videos, and files securely, see [Token Based File Access](/sdk/android/secure-media-access).
