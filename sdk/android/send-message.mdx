...
When displaying media files (images, videos, audio) in your app, CometChat provides a secure way to load them. By default, media URLs include a File Access Token (FAT) as a query parameter (`?fat=xxx`), which can be exposed in server logs and browser history.

For enhanced security, you can enable **header-based mode** which transmits the token via HTTP headers instead. This is especially recommended for apps handling sensitive content.

[Learn how to implement Token Based File Access](/sdk/android/secure-media-access)
</Info>
