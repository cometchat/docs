...
| Code | Meaning |
| --- | --- |
| `ERR_PERMISSION_DENIED` | The user's role is not allowed to pin or unpin in this conversation. Pin and unpin are gated independently. |
| `ERR_ACTION_NOT_ALLOWED` | Pin Message is not enabled for the app, or the action is otherwise refused. |
| `ERR_PINNED_MESSAGES_LIMIT_EXCEEDED` | The conversation's pin cap was reached. |
| `ERR_MESSAGE_NO_ACCESS` | The user cannot act on this message — an access refusal, or a message still held by moderation. |
...
<Note>
Read the cap from `CometChat.getPinnedMessagesLimit()` — it returns the configured value directly, so you can name the exact number in your own copy without parsing it out of the error text.
</Note>
