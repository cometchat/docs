...
| `ERR_ACTION_NOT_ALLOWED` | Pin Conversation is not enabled for the app, or the action is otherwise refused. |
| `ERR_PERMISSION_DENIED` | The user is not allowed to pin or unpin this conversation. |

Pins are per-user, so pinning a conversation never affects anyone else's list. A **system-pinned** conversation cannot be unpinned by a user.
...
<Note>
Read the cap from `CometChat.getPinnedConversationsLimit()` — it returns the configured value directly, so you can name the exact number in your own copy without parsing it out of the error text.
</Note>
