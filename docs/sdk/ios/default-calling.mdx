...
As mentioned in the [Integration](/docs/sdk/ios/calling-integration) section, Once the call is initiated, there are three options that can be possible:
...
Once the call request is sent and the receiver has accepted the call, both the initiator and the receiver need to call the [startSession()](/docs/sdk/ios/direct-calling#start-call-session) method.
