...
`CometChatUsersWithMessages` is a [Composite Component](/docs/ui-kit/ios/v4/components-overview#composite-components) that seamlessly merges the functionalities of both the [Users](/docs/ui-kit/ios/v4/users) and [Messages](/docs/ui-kit/ios/v4/messages) modules. It empowers users to effortlessly navigate to any individual's chat window by simply clicking on their respective **list item** in the user list.
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [Users](/docs/ui-kit/ios/v4/users)       | `Users` serves as a standalone component designed to establish a screen displaying a list of `users`, providing them with the capability to search for specific individuals. This component operates as a container, inheriting from `CometChatListBase`, and serves as a wrapper for the `CometChatUserList` component. |
| [Messages](/docs/ui-kit/ios/v4/messages) | `Messages` presents the chat interface for both individual `users` and `group` entities. It exhibits a message list containing all interactions where the loggedInUser has sent or received messages from any of these entities.                                                                                         |
...
While the UsersWithMessages component does not have filters, its components do, For more detail on individual filters of its component refer to [Users Filters](/docs/ui-kit/ios/v4/users)[Messages Filters](/docs/ui-kit/ios/v4/messages)
...
You can also customize its component styles. For more details on individual component styles, you can refer [Users Styles](/docs/ui-kit/ios/v4/users) and [Messages Styles](/docs/ui-kit/ios/v4/messages).
...
To find all the details on individual Component advance customization you can refer, [Users Advance](/docs/ui-kit/ios/v4/users#advanced) and [Messages Advance](/docs/ui-kit/ios/v4/messages#advanced).

UsersWithMessages uses advanced-level customization of both Users & Messages components to achieve its default behavior.

1. UsersWithMessages utilizes the [SetMenu](/docs/ui-kit/ios/v4/users) function of the `Users` subcomponent to navigate the [User](/docs/ui-kit/ios/v4/users) to [Messages](/docs/ui-kit/ios/v4/messages)

<Frame>
  <img src="/images/b7ec35f8-uwm_advanced_2_screens-0c0eee0877d03561afd59f690ff481b3.png" />
</Frame>

2. UsersWithMessages utilizes the [SetMenu](/docs/ui-kit/ios/v4/users) function of the `Messages` subcomponent to navigatefrom [Messages](/docs/ui-kit/ios/v4/messages) to Details.
...
All exposed properties of `UsersConfiguration` can be found under [Users](/docs/ui-kit/ios/v4/users). Properties marked with the <Tooltip tip="Not available">🛑</Tooltip> symbol are not accessible within the Configuration Object.
...
All exposed properties of `MessagesConfiguration` can be found under [Messages](/docs/ui-kit/ios/v4/messages). Properties marked with the <Tooltip tip="Not available">🛑</Tooltip> symbol are not accessible within the Configuration Object.
