...
In this guide, we will demonstrate how to add a custom message with a custom-built message bubble to the [Message List](/docs/ui-kit/angular/message-list) component.

<Note>

We recommend that you read the [Key Concepts](/docs/fundamentals/key-concepts) , follow the guidelines, and also take the time to familiarise yourself with the library's [components](/docs/ui-kit/angular/components-overview).

</Note>

In the example below, we are going to simulate a "nudge" behaviour which sends out a custom message when the user clicks on the auxiliary button placed in the [Message Composer](/docs/ui-kit/angular/message-composer) component.

## Custom Auxiliary Button

The first step is to create a new auxiliary button. We'll be making use of [CometChatIcon](/docs/web-elements/icon) web component to create the auxiliary icon.
...
The auxiliary button is then passed to the [ConversationsWithMessages](/docs/ui-kit/angular/conversations-with-messages) component as a prop using the [MessagesConfiguration](/docs/web-shared/messages-configuration).
...
The message request builder is then applied to the [ConversationsWithMessages](/docs/ui-kit/angular/conversations-with-messages) component using [MessagesConfiguration](/docs/web-shared/messages-configuration).
