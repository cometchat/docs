...
The `CometChatIncomingCall` is a [Component](/docs/ui-kit/android/v4/components-overview#components) that serves as a visual representation when the user receives an incoming call, such as a voice call or video call, providing options to answer or decline the call.
...
To apply customized styles to the `Avatar` component in the IncomingCall Component, you can use the following code snippet. For further insights on `Avatar` Styles [refer](/docs/ui-kit/android/v4/avatar#avatarstyle)
