...
The ConversationsWithMessages is a [Composite Component](/docs/ui-kit/android/v4/components-overview#composite-components) encompassing components such as [Conversations](/docs/ui-kit/android/v4/conversations), [Messages](/docs/ui-kit/android/v4/messages), and [Contacts](/docs/ui-kit/android/v4/contacts). Each of these component contributes to the functionality and structure of the overall ConversationsWithMessages component.
...
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [Conversations](/docs/ui-kit/android/v4/conversations) | The `Conversations` component is designed to display a list of either `User` or `Group`. This essentially represents your recent conversation history. |
| [Messages](/docs/ui-kit/android/v4/messages)           | The `Messages` component is designed to manage the messaging interaction for either individual `User` or `Group` conversations.                        |
| [Contacts](/docs/ui-kit/android/v4/contacts)           | The `CometChatContacts` component is specifically designed to facilitate the display and management of both `User` and `Groups`.                       |
...
While the ConversationsWithMessages component does not have its actions, its components - [Conversation](/docs/ui-kit/android/v4/conversations#actions), [Messages](/docs/ui-kit/android/v4/messages#actions), and [Contacts](/docs/ui-kit/android/v4/contacts) - each have their own set of actions.
...
* [ItemClickListener](/docs/ui-kit/android/v4/conversations#1-itemclicklistener) : By overriding the `ItemClickListener` of the Conversation Component, ConversationsWithMessages achieves navigation from [Conversation](/docs/ui-kit/android/v4/conversations) to [Messages](/docs/ui-kit/android/v4/messages) component.
...
While the ConversationsWithMessages component does not have filters, its components do, For more detail on individual filters of its component refer to [Conversations Filters](/docs/ui-kit/android/v4/conversations#filters) and [Messages Filters](/docs/ui-kit/android/v4/messages#filters).
...
The ConversationsWithMessages does not generate its events but its component does. For a full list of these events, you can refer to [Conversations events](/docs/ui-kit/android/v4/conversations#events) and [Messages events](/docs/ui-kit/android/v4/messages#events).
...
You can utilize the `.setUser(user)` function with a [User](/docs/sdk/android/user-management) object as input to ConversationsWithMessages. This will automatically guide you to the [Messages](/docs/ui-kit/android/v4/messages#functionality) component for the designated `User`.
...
You can utilize the `.setGroup(group)` function with a [Group](/docs/sdk/android/groups-overview) object as input to ConversationsWithMessages. This will automatically guide you to the [Messages](/docs/ui-kit/android/v4/messages) component for the designated `Group`.
...
You can find the list of all Functionality customization of individual components in [Conversations](/docs/ui-kit/android/v4/conversations#functionality) , [Messages](/docs/ui-kit/android/v4/messages#functionality), and [Contacts](/docs/ui-kit/android/v4/contacts)
...
To find all the details on individual Component advance customization you can refer, [Conversations Advance](/docs/ui-kit/android/v4/conversations#advanced),[Messages Advance](/docs/ui-kit/android/v4/messages#advanced) and [Contacts Advance](/docs/ui-kit/android/v4/contacts)
...
1. ConversationsWithMessages utilizes the [SetMenu](/docs/ui-kit/android/v4/conversations#setmenu) function of the `Conversations` subcomponent to navigate the user from [Conversations](/docs/ui-kit/android/v4/conversations) to [Contacts](/docs/ui-kit/android/v4/contacts)

   <Frame>
     <img src="/images/0d2524b3-conversationwithmessages_to_contacts_cometchat_screens-ffe1b2deab523d2db18b8a7a3b4f6fc3.png" />
   </Frame>

2. ConversationsWithMessages utilizes the [SetMenu](/docs/ui-kit/android/v4/message-header#setmenu) function of the `Messages` subcomponent to navigate the user from [Messages](/docs/ui-kit/android/v4/messages) to [Details](/docs/ui-kit/android/v4/group-details).
...
[Configurations](/docs/ui-kit/android/v4/components-overview#configurations) offer the ability to customize the properties of each component within a Composite Component.
...
All exposed properties of `ConversationsConfiguration` can be found under [Conversations](/docs/ui-kit/android/v4/conversations). Properties marked with the 🛑 symbol are not accessible within the Configuration Object.
