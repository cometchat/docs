...
The `CometChatCallButtons` is a [Component](/docs/ui-kit/android/components-overview#components) provides users with the ability to make calls, access call-related functionalities, and control call settings. Clicking this button typically triggers the call to be placed to the desired recipient.
...
All exposed properties of `OutgoingCallConfiguration` can be found under [Outgoing Call](/docs/ui-kit/android/outgoing-call). Properties marked with the 🛑 symbol are not accessible within the Configuration Object.
