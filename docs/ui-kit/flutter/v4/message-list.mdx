...
`MessageList` is a [Composite Widget](/docs/ui-kit/flutter/v4/components-overview#composite-components) that displays a list of messages and effectively manages real-time operations. It includes various types of messages such as Text Messages, Media Messages, Stickers, and more.

`MessageList` is primarily a list of the base widget [MessageBubble](/docs/ui-kit/flutter/v4/message-bubble). The MessageBubble Widget is utilized to create different types of chat bubbles depending on the message type.
...
To apply customized styles to the `Avatar` widget in the `CometChatGroups` widget, you can use the following code snippet. For further insights on `Avatar` Styles [refer](/docs/ui-kit/flutter/v4/avatar)
...
[CometChatMessageTemplate](/docs/ui-kit/flutter/v4/message-template) is a pre-defined structure for creating message views that can be used as a starting point or blueprint for creating message views often known as message bubbles. For more information, you can refer to [CometChatMessageTemplate](/docs/ui-kit/flutter/v4/message-template).
...
Assigns the list of text formatters. If the provided list is not null, it sets the list. Otherwise, it assigns the default text formatters retrieved from the data source. To configure the existing Mentions look and feel check out [MentionsFormatter Guide](/docs/ui-kit/flutter/v4/mentions-formatter-guide)
...
From the MessageList, you can navigate to the [MesssageInformation](/docs/ui-kit/flutter/v4/message-information) widget as shown in the image.

If you wish to modify the properties of the [MesssageInformation](/docs/ui-kit/flutter/v4/message-information) Widget, you can use the `MessageInformationConfiguration` object.
