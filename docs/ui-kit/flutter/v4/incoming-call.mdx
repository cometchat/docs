...
The `CometChatIncomingCall` is a [Widget](/docs/ui-kit/flutter/v4/components-overview#components) that serves as a visual representation when the user receives an incoming call, such as a voice call or video call, providing options to answer or decline the call.
