...
The [Init](/docs/ui-kit/flutter/v4/methods#init) method initializes the settings required for CometChat. Please ensure to call this method before invoking any other methods from CometChat UI Kit or CometChat SDK.

<Note>

The Auth Key is an optional property of the `UIKitSettings` Class. It is intended for use primarily during proof-of-concept (POC) development or in the early stages of application development. You can use [Auth Token](/docs/ui-kit/flutter/v4/methods#login-using-auth-token) method to login securely.
...
This straightforward authentication method is ideal for proof-of-concept (POC) development or during the early stages of application development. For production environments, however, we strongly recommend using an [Auth Token](/docs/ui-kit/flutter/v4/methods#login-using-auth-token) instead of an Auth Key to ensure enhanced security.
...
[CometChatUsersWithMessages](/docs/ui-kit/flutter/v4/conversations-with-messages) is a wrapper widget that offers functionality to render both the [Conversations](/docs/ui-kit/flutter/v4/conversations) and [Messages](/docs/ui-kit/flutter/v4/messages) widgets. It also enables opening the [Messages](/docs/ui-kit/flutter/v4/messages) by tapping on any conversation rendered in the list of conversations.
