...
`CometChatCallLogHistory` is a [Component](/docs/ui-kit/react-native/v4/components-overview#components) that shows a paginated list of all the calls between the logged-in user & another user or group. This allows the user to see all the calls with a specific user/group they have initiated/received/missed.
...
If you want to apply customized styles to the `ListItemStyle` component within the `Call Log History` Component, you can use the following code snippet. For more information, you can refer [ListItem Styles](/docs/ui-kit/react-native/v4/list-item#listitemstyle).
