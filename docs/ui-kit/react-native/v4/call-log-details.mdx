...
The `CallLogDetails` is a [Component](/docs/ui-kit/react-native/v4/components-overview#components) that displays all the information related to a call. This component displays information like user/group information, participants of the call, recordings of the call (if available) & history of all the previous calls. Also, it has buttons to start a new call.
...
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Call Log History](/docs/ui-kit/react-native/v4/call-log-history)           | The `Call Log History` component shows a paginated list of all the calls between the logged-in user & another user or group. This allows the user to see all the calls with a specific user/group they have initiated/received/missed. |
| [Call Log Recordings](/docs/ui-kit/react-native/v4/call-log-recording)      | The `Call Log Recordings` component shows a paginated list of recordings of a particular call.                                                                                                                                         |
| [Call Log participants](/docs/ui-kit/react-native/v4/call-log-participants) | The `Call Log Participants` component shows a separate view that displays comprehensive information about Call.                                                                                                                        |
...
If you want to apply customized styles to the `Avatar` component within the `CallLogDetails` Component, you can use the following code snippet. For more information you can refer [Avatar Styles](/docs/ui-kit/react-native/v4/avatar#avatarstyleinterface).
...
| **options**               | `CometChatDetailsOption[]` | Sets the [`CometChatDetailsOption`](/docs/ui-kit/react-native/v4/call-log-details#detailsoption)s |
...
All exposed properties of `GroupsConfiguration` can be found under [Groups](/docs/ui-kit/react-native/v4/groups#functionality). Properties marked with the <Tooltip tip="Not available">🛑</Tooltip> symbol are not accessible within the Configuration Object.
...
All exposed properties of `CallLogParticipantsConfiguration` can be found under [Messages](/docs/ui-kit/react-native/v4/messages#functionality). Properties marked with the <Tooltip tip="Not available">🛑</Tooltip> symbol are not accessible within the Configuration Object.
