...
## **Integration with CometChat Visual Builder (Next.js)**
...
1. **Create new users** on the **[CometChat Dashboard](https://app.cometchat.com)**, **[CometChat SDK Method](/docs/ui-kit/react/methods#create-user)** or **[via the API](https://api-explorer.cometchat.com/reference/creates-user)**.
...
* For **production environments**, it is strongly advised to use an **[Auth Token](/docs/ui-kit/react/methods#login-using-auth-token)** instead of an **Auth Key** to enhance security and prevent unauthorized access.
...
In this step, we’ll render the `CometChatApp` component and specifically disable **Server-Side Rendering (SSR)** for `CometChatNoSSR.tsx`. This targeted approach ensures the CometChat Visual Builder components load only on the client side, while the rest of your application remains fully compatible with SSR.
...
CometChat Visual Builder relies on browser APIs like `window`, `document`, and WebSockets. Since Next.js renders on the server by default, we disable SSR for this component to avoid runtime errors.
...
* **[Builder Configuration File](/docs/ui-kit/react/builder-settings)** – Learn how to customize your integration.
* **[Builder Directory Structure](/docs/ui-kit/react/builder-dir-structure)** – Understand the organization of the builder components.
* **[Advanced Theming](/docs/ui-kit/react/theme)** – Modify themes and UI elements to match your brand.
* **[Additional Customizations](/docs/ui-kit/react/builder-customisations)** – Customise the UI the way you want.
