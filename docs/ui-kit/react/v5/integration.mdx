...
The [Init](/docs/ui-kit/react/v5/methods#init) method initialises the settings required for CometChat. Please ensure to call this method before invoking any other methods from CometChat UI Kit or CometChat SDK.

<Note>

The Auth Key is an optional property of the `UIKitSettings` Class. It is intended for use primarily during proof-of-concept (POC) development or in the early stages of application development. You can use the [Auth Token](/docs/ui-kit/react/v5/methods#login-using-auth-token) method to log in securely.
...
This straightforward authentication method is ideal for proof-of-concept (POC) development or during the early stages of application development. For production environments, however, we strongly recommend using an [Auth Token](/docs/ui-kit/react/v5/methods#login-using-auth-token) instead of an Auth Key to ensure enhanced security.
...
1. [Conversations](/docs/ui-kit/react/v5/conversations) can be rendered by adding the following code snippet to the `App.js` file:
